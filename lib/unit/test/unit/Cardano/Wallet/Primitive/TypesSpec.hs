...
spec = describe "Cardano.Wallet.Primitive.Types" $ do
    ...
    describe "Slot arithmetic" $ do
        ...
        it
            "slotStartTime slotMinBound `Range.isAfter` r => \
        ...
        it "slotRangeFromTimeRange is maximal"
            ...
            $ \(sps, uniformTimeRange) ->
                ...
                in  checkCoverage
                        ...
                        $ cover
                            20
                            (fmap Range.hasLowerBound maybeSlotRange == Just True)
                            "slot range has lower bound"
                        $ cover
                            20
                            (fmap Range.hasUpperBound maybeSlotRange == Just True)
                            "slot range has upper bound"
                        $ cover
                            20
                            (fmap Range.isFinite maybeSlotRange == Just True)
