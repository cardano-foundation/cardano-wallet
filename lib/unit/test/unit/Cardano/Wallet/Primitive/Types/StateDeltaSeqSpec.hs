...
prop_fromStateDeltas_isValid state deltas =
    Seq.isValid
        applyTestDelta
        (Seq.fromStateDeltas applyTestDelta state deltas)
        === True
...
prop_toTransitionList_nextState (TestStateDeltaSeq seq) =
    all
        (\(si, d, sf) -> applyTestDelta si d == sf)
        (Seq.toTransitionList seq)
        === True
...
prop_dropEmptyTransitionWhere_countEmptyTransitionsWhere
    ...
    (applyFun -> f) =
        all
            ((== pred emptyTransitionCount) . Seq.countEmptyTransitionsWhere f)
...
prop_dropEmptyTransitionWhere_headState
    ...
    (applyFun -> f) =
        all
            ((== Seq.headState seq) . Seq.headState)
            (Seq.dropEmptyTransitionWhere f seq)
            === True
...
prop_dropEmptyTransitionWhere_lastState
    ...
    (applyFun -> f) =
        all
            ((== Seq.lastState seq) . Seq.lastState)
            (Seq.dropEmptyTransitionWhere f seq)
            === True
