...
instance Malformed (PathParam ApiTxId) where
    malformed =
      ...
      where
        msg =
            "Invalid tx hash: expecting a hex-encoded value that is 32 bytes in length."
...
instance Malformed (PathParam ApiDRepSpecifier) where
    malformed =
      ...
      where
        msg1 =
            "Invalid DRep key/script hash: expecting a Bech32 encoded value with human readable part of 'drep', 'drep_vkh' or 'drep_script'."
        msg2 =
            "Invalid DRep credential: expecting a first byte '0b00100010' value for key hash or a first byte '0b00100011' value for script hash."
...
instance Malformed (PathParam (ApiT AssetName)) where
    malformed =
      ...
      where
        ...
        msgMalformed =
            "AssetName is not hex-encoded: base16: input: invalid encoding at offset: 0"
        ...
        jsonInvalid =
            first BodyParam
                <$> [
                    ...
                    ,
                        ( "{style = \"random\"}"
                        , "Unexpected 'style = \\'random\\'}', expecting record key literal or }"
                        )
                    ]
        jsonValid =
        ...
        jsonInvalid =
            first BodyParam
                <$> [
                    ...
                    ,
                        ( "{old_passphrase = \"random\"}"
                        , "Unexpected 'old_passphrase = \\'random\\'}', expecting record key literal or }"
                        )
                    ]
        jsonValid =
            first (BodyParam . Aeson.encode)
      ...
      where
        jsonInvalid =
            first BodyParam
                <$> [
                        ( "1020344"
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiWalletPutData(ApiWalletPutData) failed, expected Object, but encountered Number"
                        )
                    ,
                        ( "\"1020344\""
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiWalletPutData(ApiWalletPutData) failed, expected Object, but encountered String"
                        )
                    ,
                        ( "\"slot_number : \"random\"}"
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiWalletPutData(ApiWalletPutData) failed, expected Object, but encountered String"
      ...
      where
        jsonInvalid =
            first BodyParam
                <$> [
                        ( "1020344"
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiWalletPutDataExtended(ApiWalletPutDataExtended) failed, expected Object, but encountered Number"
                        )
                    ,
                        ( "\"1020344\""
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiWalletPutDataExtended(ApiWalletPutDataExtended) failed, expected Object, but encountered String"
                        )
                    ,
                        ( "\"slot_number : \"random\"}"
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiWalletPutDataExtended(ApiWalletPutDataExtended) failed, expected Object, but encountered String"
        ...
        jsonInvalid =
            first BodyParam
                <$> [
                    ...
                    ,
                        ( "{\"payments : [], \"random\"}"
                        , "Unexpected 'random\\'}', expecting :"
                        )
                    , ("join", "Unexpected 'join', expecting JSON value")
                    , ("quit", "Unexpected 'quit', expecting JSON value")
                    ]
        jsonValid =
            (first (BodyParam . Aeson.encode) <$> paymentCases) <> jsonValidAction
        jsonValidAction =
            first (BodyParam . Aeson.encode)
                <$> [
                        ( [aesonQQ| { "action": "join" }|]
      ...
      where
        jsonInvalid =
            first BodyParam
                <$> [
                        ( "1020344"
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiDecodeTransactionPostData(ApiDecodeTransactionPostData) failed, expected Object, but encountered Number"
                        )
                    ,
                        ( "\"hello\""
                        , "Error in $: parsing Cardano.Wallet.Api.Types.ApiDecodeTransactionPostData(ApiDecodeTransactionPostData) failed, expected Object, but encountered String"
                        )
                    , ("{\"transaction\": \"\", \"random\"}", "Unexpected '}', expecting :")
                    ,
                        ( "{\"transaction\": \"lah\", \"decrypt_metadata\":{\"passphrase\": \"Secure Passphrase\"}}"
                        , "Error in $.transaction: Parse error. Expecting Base64-encoded format."
                        )
                    ,
                        ( "{\"transaction\": 1020344, \"decrypt_metadata\":{\"passphrase\": \"Secure Passphrase\"}}"
                        , "Error in $.transaction: parsing 'Base64 ByteString failed, expected String, but encountered Number"
                        )
                    ,
                        ( "{\"transaction\": { \"body\": 1020344 }, \"decrypt_metadata\":{\"passphrase\": \"Secure Passphrase\"}}"
                        , "Error in $.transaction: parsing 'Base64 ByteString failed, expected String, but encountered Object"
                        )
                    ]
        jsonValid =
...
instance
    Malformed
        (BodyParam (ApiWalletMigrationPostData ('Testnet pm) "lenient"))
    where
    malformed = jsonValid ++ jsonInvalid
      where
        jsonInvalid =
...
instance
    Malformed
        (BodyParam (ApiWalletMigrationPostData ('Testnet pm) "user"))
    where
    malformed = jsonValid ++ jsonInvalid
      where
        jsonInvalid =
        ...
        jsonInvalid =
            first BodyParam
                <$> [
                    ...
                    ,
                        ( "\"slot_number : \"random\"}"
                        , "trailing junk after valid JSON: endOfInput"
                        )
                    ]
        jsonValid =
            first (BodyParam . Aeson.encode)
