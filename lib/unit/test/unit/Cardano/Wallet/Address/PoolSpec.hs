...
spec = do
    describe "Pool invariants" $ do
        ...
        it "generated pool always satisfies prop_gap and prop_fresh"
            $ forAll (genUsageForGap $ AddressPool.gap testPool)
            $ \usage ->
        ...
        it
            "nextIndex always points to an unused address"
            prop_next_is_unused
...
prop_updates
    :: (Ord addr, Ord ix, Enum ix) => Pool addr ix -> Property
      ...
      where
        ...
        discoverQuery =
            second AddressPool.addresses
                . runIdentity
                $ AddressPool.discover query pool0
...
genUsageForGap gap = do
  ...
  where
    used =
        flip replicate Used
            <$> oneof [choose (1, 3), choose (gap + 1, 2 * gap)]
