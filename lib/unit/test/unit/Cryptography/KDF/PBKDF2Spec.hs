...
spec = do
    describe "Golden Tests obtained by openssl" $ do
        ...
        it "golden 1" $
            tohex (generateKey (PBKDF2Config SHA256 10000 32 16) "password" Nothing)
            `shouldBe`
            OpenSSLOutput
        ...
        it "golden 2" $
            tohex (generateKey (PBKDF2Config SHA256 5000 32 16) "password" Nothing)
            `shouldBe`
            OpenSSLOutput
        ...
        it "golden 3" $
            tohex (generateKey (PBKDF2Config SHA512 10000 32 16) "password" Nothing)
            `shouldBe`
            OpenSSLOutput
        ...
        it "golden 4" $
            tohex (generateKey (PBKDF2Config SHA512 5000 32 16) "password" Nothing)
            `shouldBe`
            OpenSSLOutput
