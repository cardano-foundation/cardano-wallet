...
import Cardano.Wallet.Primitive.Types
    ( Address (..), ChimericAccount (..), WalletId (..), WalletName (..) )
...
instance IsOurs AnyAddressState Address where
    isOurs (Address addr) s@(AnyAddressState p) = (crc32 addr < p', s)
        where
          p' = floor (fromIntegral (maxBound :: Word32) * p)

instance IsOurs AnyAddressState ChimericAccount where
    isOurs _ s = (False, s)
...
instance GenChange AnyAddressState where
    type ArgGenChange AnyAddressState = ()
    genChange _ = error
