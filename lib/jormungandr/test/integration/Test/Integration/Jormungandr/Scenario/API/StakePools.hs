...
import Test.Integration.Framework.TestData
    ( errMsg405, passphraseMaxLength, passphraseMinLength )
...
spec = do
    ...
    it "STAKE_POOLS_JOIN_01 - Can join a stakepool" $ \ctx -> do
        (_, p:_) <- eventually' $ unsafeRequest @[ApiStakePool] ctx listStakePoolsEp Empty
        w <- emptyWallet ctx
        r <- joinStakePool ctx p (w, "Secure Passphrase")
        expectResponseCode HTTP.status501 r

    it "STAKE_POOLS_JOIN_02 - I cannot join another until I quit\
        \ or maybe I will just re-join another?" $ \ctx -> do
        (_, p1:p2:_) <- eventually' $ unsafeRequest @[ApiStakePool] ctx listStakePoolsEp Empty
        w <- emptyWallet ctx
    ...
    it "STAKE_POOLS_JOIN_02 - I definitely can quit and join another" $ \ctx -> do
        (_, p1:p2:_) <- eventually' $ unsafeRequest @[ApiStakePool] ctx listStakePoolsEp Empty
        w <- emptyWallet ctx
        r1 <- joinStakePool ctx p1 (w, "Secure Passprase")
        expectResponseCode HTTP.status501 r1
    ...
    it "STAKE_POOLS_JOIN_03 - Passphrase must be correct to join" $ \ctx -> do
        (_, p:_) <- eventually' $ unsafeRequest @[ApiStakePool] ctx listStakePoolsEp Empty
        w <- emptyWallet ctx
        r <- joinStakePool ctx p (w, "Incorrect Passphrase")
        expectResponseCode HTTP.status501 r

    describe "STAKE_POOLS_JOIN/QUIT_03 - Passphrase must have appropriate length" $ do
        let pMax = passphraseMaxLength
        let pMin = passphraseMinLength
        let tooShort = "passphrase is too short: expected at\
                \ least 10 characters"
        let tooLong = "passphrase is too long: expected at\
                \ most 255 characters"
        let tests =
        ...
        forM_ tests $ \(expec, passphrase) -> it ("Join: " ++ expec) $ \ctx -> do
            verifyIt ctx joinStakePool passphrase expec

        forM_ tests $ \(expec, passphrase) -> it ("Quit: " ++ expec) $ \ctx -> do
            verifyIt ctx quitStakePool passphrase expec

    it "STAKE_POOLS_JOIN_04 - Byron wallet cannot join stake pool" $ \ctx -> do
        (_, p:_) <- eventually' $ unsafeRequest @[ApiStakePool] ctx listStakePoolsEp Empty
    ...
    it "STAKE_POOLS_QUIT_03 - Passphrase must be correct to quit" $ \ctx -> do
        (_, p:_) <- eventually' $ unsafeRequest @[ApiStakePool] ctx listStakePoolsEp Empty
        w <- emptyWallet ctx
