...
spec = describe "COMMON_NETWORK" $ do
    ...
    it
        "NETWORK_BYRON - Byron wallet has the same tip as network/information"
        $ \ctx -> runResourceT @IO $ do
            ...
            eventually "Wallet has the same tip as network/information" $ do
                ...
                verify
                    ...
                    , expectField
                        (#tip . #slotId . #epochNumber . #getApiT)
                        (`shouldBe` epochNum)
                    , expectField
                        (#tip . #slotId . #slotNumber . #getApiT)
                        (`shouldBe` slotNum)
    ...
    it "NETWORK_CLOCK - Can query network clock" $ \ctx -> do
        ...
        eventually "ntp status = (un)available" $ do
            r <-
                request @ApiNetworkClock
                    ctx
                    Link.getNetworkClock
                    Default
                    Empty
            expectResponseCode @IO HTTP.status200 r
            expectField
