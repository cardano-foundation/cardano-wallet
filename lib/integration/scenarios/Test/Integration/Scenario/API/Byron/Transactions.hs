...
spec = describe "BYRON_TRANSACTIONS" $ do
    describe
        "BYRON_TRANS_ASSETS_CREATE_01 - Multi-asset transaction with ADA"
        $ forM_
    ...
    describe
        ...
        $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
            ...
            let destination = (addrs !! 1) ^. #id
            payload <-
                mkTxPayloadMA @n destination minUTxOValue' [val] fixturePassphrase
    ...
    describe
        "BYRON_TRANS_ASSETS_CREATE_02a - Multi-asset transaction with no ADA"
        $ forM_
            [ (fixtureMultiAssetRandomWallet @n, "Byron wallet")
            , (fixtureMultiAssetIcarusWallet @n, "Icarus wallet")
            ]
    ...
    describe "BYRON_TRANS_ASSETS_LIST_01 - Asset list present"
        ...
        $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
            ...
            r <-
                request @([ApiAsset]) ctx (Link.listByronAssets wal) Default Empty
            verify
                r
                [ expectSuccess
    ...
    describe
        "BYRON_TRANS_ASSETS_LIST_02 - Asset list present when not used"
        $ forM_
    ...
    describe
        "BYRON_TRANS_ASSETS_GET_02 - Asset not present when isn't associated"
        $ forM_
            [ (fixtureMultiAssetRandomWallet @n, "Byron wallet")
            , (fixtureMultiAssetIcarusWallet @n, "Icarus wallet")
            ]
    ...
    describe
        "BYRON_TRANS_ASSETS_GET_02a - Asset not present when isn't associated"
        $ forM_
            [ (fixtureMultiAssetRandomWallet @n, "Byron wallet")
            , (fixtureMultiAssetIcarusWallet @n, "Icarus wallet")
            ]
    ...
    describe
        "BYRON_TRANS_CREATE_01 - Single Output Transaction Byron -> Shelley"
        $ forM_
        ...
        $ \(srcFixture, name) -> it name $ \ctx -> runResourceT $ do
            ...
            let feeEstMin = getFromResponse (#estimatedMin . #toNatural) rFeeEst
            let feeEstMax = getFromResponse (#estimatedMax . #toNatural) rFeeEst
            ...
            verify
                ...
                , expectField (#balance . #total)
                    $ between
                        ( ApiAmount (faucetAmt - feeEstMax - amt)
                        , ApiAmount (faucetAmt - feeEstMin - amt)
                        )
                , expectField
                    (#balance . #available)
                    (.>= ApiAmount (faucetAmt - faucetUtxoAmt))
                ]
    ...
    describe
        "BYRON_TRANS_CREATE_01a - Single Output Transaction Byron -> Byron"
        $ forM_
            [ (emptyRandomWallet, "Byron wallet")
            , (emptyIcarusWallet, "Icarus wallet")
            ]
        $ \(emptyByronWallet, name) -> it name $ \ctx -> runResourceT $ do
            (wByron, wDestByron) <-
                (,) <$> fixtureRandomWallet ctx <*> emptyByronWallet ctx
            ra <-
                request @ApiByronWallet
                    ctx
            ...
            let (ApiAmount feeEstMin) = getFromResponse #estimatedMin rFeeEst
            let (ApiAmount feeEstMax) = getFromResponse #estimatedMax rFeeEst
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            r <-
                request @(ApiTransaction n) ctx ("POST", endpoint) Default payload
            expectResponseCode HTTP.status404 r
            decodeErrorInfo r
    ...
    describe
        "BYRON_TX_LIST_ADDRESS - Transactions can be filtered by address"
        $ forM_
  ...
  where
    ...
    getAddress ctx wal addrIx = do
        ra <-
            request @ApiByronWallet ctx (Link.getWallet @'Byron wal) Default Empty
