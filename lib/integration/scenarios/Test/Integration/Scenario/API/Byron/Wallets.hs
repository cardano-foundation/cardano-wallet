...
spec = describe "BYRON_WALLETS" $ do
    ...
    it "BYRON_LIST_01 - Interleave of Icarus and Random wallets" $ \ctx -> runResourceT $ do
        ...
        r1 <-
            Mnemonics.generateSome Mnemonics.M15 >>= \m ->
                emptyByronWalletWith ctx "icarus" ("ica1", m, pwd)
        r2 <-
            Mnemonics.generateSome Mnemonics.M12 >>= \m ->
    ...
    it
        "BYRON_UPDATE_PASS_03 - Updating passphrase with no password wallets"
        $ \ctx -> runResourceT $ do
    ...
    it
        "BYRON_UPDATE_PASS_04a - Updating passphrase with no password wallets"
        $ \ctx -> runResourceT $ do
            w <- emptyRandomWalletWithPasswd ctx ""
            request @ApiByronWallet ctx (Link.getWallet @'Byron w) Default Empty
                >>= flip verify [expectField #passphrase (`shouldSatisfy` isNothing)]
    ...
    it "BYRON_UPDATE_PASS_04b - Regression test" $ \ctx -> runResourceT $ do
        ...
        w <-
            emptyByronWalletFromXPrvWith ctx "random" ("Random Wallet", key, pwd)
        let payload = updatePassPayload "" "correct-password"
        r <-
            request @ApiByronWallet
                ctx
    ...
    it
        "BYRON_UPDATE_PASS_07 - Updating passphrase with short password wallets"
        $ \ctx -> runResourceT $ do
