...
spec = describe "SHELLEY_WALLETS" $ do
        ...
        $ do
            ...
            forM_ matrix $ \(nameIn, nameOut) -> it nameIn $ \ctx -> runResourceT $ do
                ...
                eventually "listed wallet's state = Ready" $ do
                    r2 <-
                        request @ApiWallet ctx (Link.getWallet @'Shelley wid) Default Empty
                    verify
                        r2
    ...
    it "WALLETS_CREATE_10 - Create wallet with one change address mode on" $ \ctx -> runResourceT $ do
        ...
        let realizeTx wSrc amt dest = do
                ...
                eventually "Transaction is discovered" $ do
                    request @(ApiTransaction n)
                        ctx
                        (Link.getTransaction @'Shelley wOneChangeAddr (ApiTxId txid))
                        Default
                        Empty
                        >>= expectField #status (`shouldBe` ApiT InLedger)
                    request @(ApiTransaction n)
                        ctx
                        (Link.getTransaction @'Shelley wFixture (ApiTxId txid))
                        Default
                        Empty
                        >>= expectField #status (`shouldBe` ApiT InLedger)
        ...
        rPut <-
            request @ApiWallet
                ctx
                ("PUT", "v2/wallets" </> walIdOneChangeAddr)
                Default
                putData
        verify
            rPut
    ...
    it
        "WALLETS_GET_02, WALLETS_DELETE_01 - Deleted wallet is not available"
        $ \ctx -> runResourceT $ do
            w <- emptyWallet ctx
            _ <-
                request @ApiWallet
    ...
    it
        "WALLETS_UPDATE_PASS_01b - passphraseLastUpdate gets updated, mnemonic"
        $ \ctx -> runResourceT $ do
    ...
    it
        "WALLETS_UPDATE_PASS_04 - Deleted wallet is not available, mnemonic"
        $ \ctx -> runResourceT $ do
            (w, mnemonic) <- emptyWalletAndMnemonic ctx
    ...
    describe
        "WALLETS_UPDATE_PASS_05,06 - Transaction after updating passphrase"
        $ do
    ...
    it
        "WALLETS_UTXO_02 - Sending and receiving funds updates wallet's utxo."
        $ \ctx -> runResourceT $ do
            wSrc <- fixtureWallet ctx
            wDest <- emptyWallet ctx
    ...
    it "WALLETS_SIGNATURES_01 - can verify signature" $ \ctx -> runResourceT $ do
        ...
        let key =
                unsafeXPub
                    $ fst (getFromResponse #getApiVerificationKey rKey) <> dummyChainCode
        let msgHash =
                unsafeFromHexText
    ...
    it "WALLETS_SIGNATURES_02 - invalid index for signing key" $ \ctx -> runResourceT $ do
        ...
        r <-
            rawRequest
                ...
                link
                ( Headers
                    [(HTTP.hAccept, "*/*"), (HTTP.hContentType, "application/json")]
                )
                (Json payload)
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            r1 <-
                emptyByronWalletWith ctx "random" ("byron1", m1, fixturePassphrase)
            r2 <-
                emptyByronWalletWith ctx "random" ("byron2", m2, fixturePassphrase)
            r3 <-
                emptyByronWalletWith ctx "random" ("byron3", m3, fixturePassphrase)
    ...
    it
        "BYRON_WALLETS_LIST_04, DELETE_01 - \
        ...
        $ \ctx -> runResourceT $ do
            ...
            _ <-
                request @ApiByronWallet
                    ctx
                    (Link.deleteWallet @'Byron wb2)
                    Default
                    Empty
            _ <-
                request @ApiWallet ctx (Link.deleteWallet @'Shelley ws3) Default Empty
