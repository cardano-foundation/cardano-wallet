...
spec = describe "SHELLEY_ADDRESSES" $ do
    ...
    it "ADDRESS_LIST_03 - Generates new address pool gap" $ \ctx -> runResourceT $ do
        ...
        wDest <-
            emptyWalletWith ctx ("Wallet", fixturePassphrase, initPoolGap)
    ...
    it "ADDRESS_INSPECT_05 - Address inspect OK bech32" $ \ctx -> do
        ...
        verify
            ...
            , expectField
                ...
                ( `shouldBe`
                    "addr_vkh1hwl9tuz8uuqe8cnpv387d5kcj8gyz9r9q30x395vsvue5e44fh7"
                )
            , expectField
                (Aeson.key "stake_key_hash_bech32" . Aeson._String)
                ( `shouldBe`
                    "stake_vkh1fmzmmeyrsah8nnwpj0522w2amkrpt89dyq84g9s3pwrc7dqjnfu"
                )
            , expectField
                (Aeson.key "address_type" . Aeson._Number)
                (`shouldBe` 0)
    ...
    it
        ...
        $ \ctx -> do
            ...
            let goldenAddr =
                    "addr_test1wrrqr8fmk4ulc7pgycd96fuqcg40e5ecuway0ypc2tsnteqqu6qs0"
                        :: Text
            toJSON (getResponse r1) `shouldBe` object ["address" .= goldenAddr]
            toJSON (getResponse r2) `shouldBe` object ["address" .= goldenAddr]

    it
        "ANY_ADDRESS_POST_02 - Golden tests for enterprise script address - any"
        $ \ctx -> do
            --- $ cat script.txt
            ...
            let goldenAddr =
                    "addr_test1wrjtlgneqelxxlckcsgrkd7rd6ycrgegu5th24x0f058gmqvc8s20"
                        :: Text
            toJSON (getResponse r1) `shouldBe` object ["address" .= goldenAddr]
            toJSON (getResponse r2) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        "ANY_ADDRESS_POST_04 - Golden tests for enterprise script address - some"
        $ \ctx -> do
            --- $ cat script.txt
            ...
            expectResponseCode HTTP.status202 r2
            let goldenAddr =
                    "addr_test1wqqmnmwuh85e0fxaggl6ac2hfeqncg76gsr0ld8qdjd84agpc0nuz"
                        :: Text
            toJSON (getResponse r1) `shouldBe` object ["address" .= goldenAddr]
            toJSON (getResponse r2) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        "ANY_ADDRESS_POST_05 - Golden tests for reward account script address - any"
        $ \ctx -> do
            --- $ cat script.txt
            ...
            expectResponseCode HTTP.status202 r2
            let goldenAddr =
                    "stake_test17qshpfjkgh98wumvnn9y3yfhevllp4y04u6y84q3flxcv9s2kvrnx"
                        :: Text
            toJSON (getResponse r1) `shouldBe` object ["address" .= goldenAddr]
            toJSON (getResponse r2) `shouldBe` object ["address" .= goldenAddr]
    ...
    --- script1y9c2v4j9efmhxmyuefyfzd7t8lcdfra0x3pagy20ekrpvfxxxyz
    it
        "ANY_ADDRESS_POST_06 - Golden tests for delegating script address - any"
        $ \ctx -> do
            let payload1 =
                    Json
                        [json|{
            ...
            let goldenAddr =
                    "addr_test1xqqmnmwuh85e0fxaggl6ac2hfeqncg76gsr0ld8qdjd84afpwzn9v3w2waeke8x2fzgn0jel7r2glte5g02pzn7dsctqlvxt2w"
                        :: Text
            toJSON (getResponse r1) `shouldBe` object ["address" .= goldenAddr]
            toJSON (getResponse r2) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        ...
        $ \ctx -> do
            ...
            let goldenAddr =
                    "addr_test1vpqthemrg5kczwfjjnahwt65elhrl95e9hcgufnajtp6wfgdmxm9u"
                        :: Text
    ...
    --- > | cardano-address address payment --network-tag mainnet
    it
        "ANY_ADDRESS_POST_07c - Golden tests for enterprise address - from key hash"
        $ \ctx -> do
            let payload =
                    Json
                        [json|{
    ...
    it
        ...
        $ \ctx -> do
            ...
            expectResponseCode HTTP.status202 r
            let goldenAddr =
                    "stake_test1uq6pmlvyl3wn4ca6807e26gy2gek9hqu0gastzh5tk0xx0gdfvj8f"
                        :: Text
            toJSON (getResponse r) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        ...
        $ \ctx -> do
            ...
            expectResponseCode HTTP.status202 r
            let goldenAddr =
                    "addr_test1qpqthemrg5kczwfjjnahwt65elhrl95e9hcgufnajtp6wff5rh7cflza8t3m5wlaj45sg53nvtwpc73mqk90ghv7vv7ser7yl4"
                        :: Text
            toJSON (getResponse r) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        ...
        $ \ctx -> do
            ...
            expectResponseCode HTTP.status202 r
            let goldenAddr =
                    "addr_test1zqqmnmwuh85e0fxaggl6ac2hfeqncg76gsr0ld8qdjd84af5rh7cflza8t3m5wlaj45sg53nvtwpc73mqk90ghv7vv7sq4jdnx"
                        :: Text
            toJSON (getResponse r) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        ...
        $ \ctx -> do
            ...
            expectResponseCode HTTP.status202 r
            let goldenAddr =
                    "addr_test1ypqthemrg5kczwfjjnahwt65elhrl95e9hcgufnajtp6wfgph8kaew0fj7jd6s3l4ms4wnjp8s3a53qxl76wqmy60t6snk9ahl"
                        :: Text
            toJSON (getResponse r) `shouldBe` object ["address" .= goldenAddr]
    ...
    it "ANY_ADDRESS_POST_13 - Golden tests for script with timelocks" $ \ctx -> do
        ...
        expectResponseCode HTTP.status202 r
        let goldenAddr =
                "addr_test1xq756z909yycvf5ah8j5pc4cvuedkhvhyylmgfz400t8jdwmwa0hp024gu7dm6h8n252lkgnzemp93mm9kyd48p64mjs5kkua8"
                    :: Text
        toJSON (getResponse r) `shouldBe` object ["address" .= goldenAddr]
    ...
    it
        "ANY_ADDRESS_POST_14b - at_least 0 is valid when validation is required"
        $ \ctx -> do
            let payload =
                    Json
                        [json|{
    ...
    it
        "ANY_ADDRESS_POST_15b - at_least 4 is valid when validation is required"
        $ \ctx -> do
            let payload =
                    Json
                        [json|{
    ...
    it
        "ANY_ADDRESS_POST_15 - Staking address using stake credential non-hashed"
        $ \ctx -> runResourceT $ do
            w <- emptyWallet ctx
    ...
    it
        "ANY_ADDRESS_POST_16 - Staking address using stake credential hashed"
        $ \ctx -> runResourceT $ do
            w <- emptyWallet ctx
