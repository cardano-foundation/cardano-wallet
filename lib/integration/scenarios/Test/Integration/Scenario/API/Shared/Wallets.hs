...
spec = describe "SHARED_WALLETS" $ do
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            (walAcctSameOtherScriptActive ^. #id)
                `shouldNotBe` (walActive ^. #id)
    ...
    it
        "SHARED_WALLETS_CREATE_14 - Create wallet with one change address mode on"
        $ \ctx -> runResourceT $ do
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            expectErrorMessage
                (errMsg403KeyAlreadyPresent (toText Payment))
                rPatch3
