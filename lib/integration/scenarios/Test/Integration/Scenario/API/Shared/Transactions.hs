...
spec = describe "SHARED_TRANSACTIONS" $ do
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            rSharedWal3 <-
                getSharedWallet ctx (ApiSharedWallet (Right sharedWal3))
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            (wSrc, wDest) <-
                (,) <$> fixtureSharedWallet @n ctx <*> emptyWallet ctx
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            expectResponseCode HTTP.status404 r
            decodeErrorInfo r
                `shouldBe` NoSuchTransaction (ApiErrorNoSuchTransaction (ApiT txid2))
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            ...
            verify
                rDecodedTx1
                (decodedExpectations1 ++ certExpectation1 ++ witsExp1)
            ...
            verify
                rDecodedTx2
                (decodedExpectations1 ++ certExpectation2 ++ witsExp1)
            let ApiSerialisedTransaction apiTx1 _ =
            ...
            verify
                rDecodedTx3
                (decodedExpectations1 ++ certExpectation1 ++ witsExp2)
            verify
                rDecodedTx4
                (decodedExpectations1 ++ certExpectation2 ++ witsExp2)
            ...
            verify
                rDecodedTx5
                (decodedExpectations1 ++ certExpectation1 ++ witsExp3)
            verify
                rDecodedTx6
                (decodedExpectations1 ++ certExpectation2 ++ witsExp3)
            ...
            verify
                rDecodedTx7
                (decodedExpectations2 ++ certExpectation3 ++ witsExp4)
            ...
            verify
                rDecodedTx8
                (decodedExpectations2 ++ certExpectation4 ++ witsExp4)
    ...
    it
        ...
        $ \ctx -> runResourceT $ do
            liftIO
                $ pendingWith
                    "TODO: Fix multi-delegation with shared wallets flakiness, ADP-3084"
            -- creating empty parent Shelley wallet
            m15 <- Mnemonics.generateSome Mnemonics.M15
            m12 <- Mnemonics.generateSome Mnemonics.M12
            let payloadCreateParent =
                    Json
                        [json|{
            ...
            rTx1 <-
                request @(ApiTransaction n) ctx (ep wShelley) Default payloadTx
            ...
            rTx2 <-
                request @(ApiTransaction n) ctx (ep parentWal) Default payloadTx1
    ...
    it "SHARED_TRANSACTIONS_LIST_05 - filter address output side"
        $ \ctx -> runResourceT $ do
            ...
            (wSrc, ApiSharedWallet (Right wDest)) <-
                (,)
                    <$> fixtureSharedWalletWith ctx 100_000_000
