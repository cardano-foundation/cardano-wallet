...
spec = describe "SHELLEY_CLI_WALLETS" $ do
    ...
    it "WALLETS_CREATE_03 - Cannot create wallet that exists" $ \ctx -> runResourceT $ do
        ...
        (c1, o1, e1) <-
            createWalletViaCLI ctx ["n1"] m1 m2 "secure-passphrase"
        c1 `shouldBe` ExitSuccess
        T.unpack e1 `shouldContain` cmdOk
        ...
        (c2, o2, e2) <-
            createWalletViaCLI ctx ["n2"] m1 m2 "Xsecure-passphraseX"
    ...
    describe
        "WALLETS_CREATE_05 - Can create wallet with different mnemonic sizes"
        $ do
    ...
    describe
        "WALLETS_CREATE_05 - Can't create wallet with wrong size of mnemonic"
        $ do
    ...
    it "WALLETS_LIST_01 - Can list wallets" $ \ctx -> runResourceT $ do
        ...
        j <-
            filter walFromThisTest <$> expectValidJSON (Proxy @[ApiWallet]) out
        length j `shouldBe` 2
        verify
            j
            [ expectCliListField
    ...
    describe
        "WALLETS_UPDATE_PASS_04 - Cannot update pass of wallets with false ids"
        $ do
