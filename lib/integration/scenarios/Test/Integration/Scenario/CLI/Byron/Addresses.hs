...
scenario_ADDRESS_LIST_01 walType fixture = it title $ \ctx -> runResourceT $ do
  ...
  where
    title =
        "CLI_ADDRESS_LIST_01 - "
            ++ walType
            ++ " can list known addresses on a default wallet"
...
scenario_ADDRESS_LIST_02 walType fixture = it title $ \ctx -> runResourceT $ do
    ...
    (Exit c, Stdout out, Stderr err) <-
        listAddressesViaCLI ctx (args "used")
    err `shouldBe` cmdOk
    c `shouldBe` ExitSuccess
    j <- expectValidJSON (Proxy @[ApiAddressWithPath n]) out
    let n = length j
    ...
    (Exit c2, Stdout out2, Stderr err2) <-
        listAddressesViaCLI ctx (args "unused")
    err2 `shouldBe` cmdOk
...
scenario_ADDRESS_LIST_04 walType fixture = it title $ \ctx -> runResourceT $ do
    ...
    (Exit c, Stdout out, Stderr err) <-
        listAddressesViaCLI ctx [T.unpack wid]
...
scenario_ADDRESS_CREATE_01 = it title $ \ctx -> runResourceT @IO $ do
    ...
    let wid = T.unpack (w ^. walletId)
    (c, out, err) <-
        createAddressViaCLI ctx [wid] (T.unpack fixturePassphrase)
...
scenario_ADDRESS_CREATE_03 = it title $ \ctx -> runResourceT @IO $ do
    ...
    let wid = T.unpack (w ^. walletId)
    (c, out, err) <-
        createAddressViaCLI ctx [wid] "Give me all your money."
...
scenario_ADDRESS_CREATE_05 = it title $ \ctx -> runResourceT @IO $ do
    ...
    (c, out, err) <-
        createAddressViaCLI ctx args (T.unpack fixturePassphrase)
    T.unpack err `shouldContain` cmdOk
    c `shouldBe` ExitSuccess
    j <- expectValidJSON (Proxy @(ApiAddressWithPath n)) (T.unpack out)
    verify j [expectCliField #state (`shouldBe` ApiT Unused)]
  where
    title =
        "CLI_ADDRESS_CREATE_05 - Can create an address and specify the index"
...
scenario_ADDRESS_CREATE_06 = it title $ \ctx -> runResourceT @IO $ do
  ...
  where
    title =
        "CLI_ADDRESS_CREATE_06 - Cannot create an address that already exists"
...
scenario_ADDRESS_IMPORT_01 = it title $ \ctx -> runResourceT @IO $ do
    ...
    let wid = T.unpack (w ^. walletId)
    let addr =
            T.unpack $ encodeAddress (sNetworkId @n) $ randomAddresses @n mw !! 42
    (Exit c, Stdout _out, Stderr err) <-
        importAddressViaCLI ctx [wid, addr]
    c `shouldBe` ExitSuccess
...
scenario_ADDRESS_IMPORT_02 = it title $ \ctx -> runResourceT @IO $ do
    ...
    let wid = T.unpack (w ^. walletId)
    let addr =
            T.unpack $ encodeAddress (sNetworkId @n) $ icarusAddresses @n mw !! 42
    (Exit c, Stdout _out, Stderr err) <-
        importAddressViaCLI ctx [wid, addr]
    c `shouldBe` ExitFailure 1
  ...
  where
    title =
        "CLI_ADDRESS_IMPORT_02 - I can't import an address on an Icarus wallets"
...
scenario_ADDRESS_IMPORT_03 = it title $ \ctx -> runResourceT @IO $ do
    ...
    c `shouldBe` ExitFailure 1
    err
        `shouldBe` "Unable to decode Address: not a valid Base58 encoded string.\n"
  where
