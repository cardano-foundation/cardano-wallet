...
spec = describe "BYRON_CLI_WALLETS" $ do
    describe
        "CLI_BYRON_GET_04, CLI_BYRON_DELETE_01, BYRON_RESTORE_02, BYRON_RESTORE_03 -\
        \ Deleted wallet is not available, but can be restored"
        $ do
            let matrix =
                    [ ("random", Mnemonics.generateSome Mnemonics.M12)
    ...
    describe
        "CLI_BYRON_RESTORE_01, CLI_BYRON_GET_01, CLI_BYRON_LIST_01 -\
        \Restore a wallet"
        $ do
            let scenarioSuccess style mnemonic ctx = runResourceT @IO $ do
                    ...
                    (c, out, err) <-
                        createWalletViaCLI
                            ctx
                            args
                            (T.unpack (someMnemonicToSentence mnemonic))
                            "\n"
                            "secure-passphrase"
                    T.unpack err `shouldContain` cmdOk
                    c `shouldBe` ExitSuccess
    ...
    it "CLI_BYRON_UPDATE_NAME_02 - When updated name too long" $ \ctx ->
        forM_ [emptyRandomWallet, emptyIcarusWallet]
            $ \emptyByronWallet -> runResourceT @IO $ do
                ...
                c `shouldBe` ExitFailure 1
                err
                    `shouldContain` "name is too long: expected at most 255 characters"
                out `shouldBe` mempty
    ...
    describe "CLI_BYRON_UPDATE_PASS_03 - Pass length incorrect" $ do
        ...
        let matrix =
                [ ("old pass too short", passTooShort, passOK, errMsgTooShort)
