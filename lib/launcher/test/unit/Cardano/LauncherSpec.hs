...
spec = beforeAll setupMockCommands $ do
  ...
  it "Backend process is terminated when Async thread is cancelled" $ \MockCommands {..} -> withTestLogging $ \tr -> do
    ...
    mvar <- newEmptyMVar
    let
      backend = withBackendProcess tr foreverCommand $ \_ ph -> do
        putMVar mvar ph
  ...
  it "Misbehaving backend process is killed when Async thread is cancelled" $ \_ -> withTestLogging $ \tr -> do
    ...
    mvar <- newEmptyMVar
    let
      backend = withBackendProcess tr unkillableCommand $ \_ ph -> do
        putMVar mvar ph
        forever $ threadDelay maxBound
    before <- getCurrentTime
    race_ backend (threadDelay 1000000)
    ...
    mockCommandsShell =
      MockCommands
        { mockCommand = \success before ->
            let
              exitStatus = if success then 0 else 1 :: Int
            in
              Command "sh" ["-c", "sleep 1; exit " ++ show exitStatus] before Inherit Inherit
