...
embedStore embed bstore = do
  ...
  let
    readMachine = readTVarIO machine
...
embedStore' Embedding' {load, write, update} Store {loadS, writeS, updateS} =
  ...
  in
    mkUpdateStore loadL (writeS . write) updateL
