...
-}
module Cardano.Wallet.Read.PParams.Mock
    ( mockPParamsConway
    ) where
...
import qualified Cardano.Read.Ledger.PParams as Read
    ( PParams (..)
    )
import qualified Data.Map.Strict as Map

-- | Mock protocol parameters for the 'Conway' era.
--
-- We try to use similar parameters to mainnet where it matters (in particular
-- fees, execution unit prices, and the cost model).
mockPParamsConway :: Read.PParams Conway
mockPParamsConway = Read.PParams $ unsafeWrap conwayPParams
  where
    ...
    conwayPParams = upgradeConwayPParams upgrade babbagePParams
      where
        ...
        upgrade = UpgradeConwayPParams
            ...
            , ucppGovActionDeposit = ada 1_000
            , ucppDRepDeposit = ada 2
    ...
    lovelace :: Integer -> Coin
    lovelace = Coin

    ada :: Integer -> Coin
    ada x = lovelace (1_000_000 * x)
