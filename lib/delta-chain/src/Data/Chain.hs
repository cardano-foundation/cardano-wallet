...
lookup node Chain{next, prev} =
    case (Map.lookup node next, Map.lookup node prev) of
        ...
        (after, Just (Just before)) ->
            ...
            in  Just
                    Edge{via = node, to = after, from = adjust <$> Map.lookup before next}
...
appendTip
    :: Ord node => node -> edge -> Chain node edge -> Chain node edge
...
-------------------------------------------------------------------------------}
testChain
    :: (Table (Edge Int Char), [[Table.DeltaDB Int (Edge Int Char)]])
testChain =
    liftUpdates
