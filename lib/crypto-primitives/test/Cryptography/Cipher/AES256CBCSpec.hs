...
spec = do
    describe "encrypt/decrypt roundtrip with padding"
        ...
        $ \(CipherPaddingSetup payload' key' iv' salt') -> do
            let toPayload (Left EmptyPayload) = Right (BS.empty, salt')
                toPayload res = res
            toPayload
                ( encrypt WithPadding key' iv' salt' payload'
                    >>= decrypt WithPadding key' iv'
                )
                === Right (payload', salt')
        ...
        $ \(CipherRawSetup payload' key' iv' salt') -> do
            ...
            toPayload
                ( encrypt WithoutPadding key' iv' salt' payload'
                    >>= decrypt WithoutPadding key' iv'
                )
                === Right (payload', salt')

    describe "encrypt with incorrect block size"
        $ it "encrypt payload == error"
        $ property
    ...
    describe "Golden Tests obtained by openssl" $ do
        it "golden for key and iv with salt"
            $ tohex (saltKey, saltIv)
            `shouldBe` ( "26A1A6F599173BAF863F97F2A18AF2FD8E99104E726D9EB682FCB7ED53517CF9"
                       , "131BAB9240C168CC60A9F6DFA8CA5A6D"
                       )

        it "golden for key and iv with salt"
            $ tohex (noSaltKey, noSaltIv)
            `shouldBe` ( "E11244295150E6713CD76E9A5112347093BDB6ACBF0C8021ABAE29881130B210"
        ...
        it "golden 1-byte payload no salt"
            ...
            `shouldBe` Right "ZMGELuBkqtHVw5pUA353vQ=="
        ...
        it "golden 2-byte payload no salt"
            ...
            `shouldBe` Right "KBO1WMy3NpqMmm+fEL2kbg=="
        ...
        it "golden 5-byte payload no salt"
            ...
            `shouldBe` Right "49TERtYsEVKVgpJkUzqmJw=="
        ...
        it "golden 10-byte payload no salt"
            ...
            `shouldBe` Right "XaYfTkqDajiW6hRc+SlJ8A=="
        ...
        it "golden 15-byte payload no salt"
            ...
            `shouldBe` Right "CXH8t8dSqroR4r4IiNAsfA=="
        ...
        it "golden 16-byte payload no salt"
            ...
            `shouldBe` Right "hLArzI8DbqOXiEi6HqGQh8iZSRCztrdYpMEE1aj8ES8="
        ...
        it "golden 20-byte payload no salt"
            ...
            `shouldBe` Right "hLArzI8DbqOXiEi6HqGQh96Z6cWqsqTJlbxfcWg/Nvo="
        ...
        it "golden 31-byte payload no salt"
            ...
            `shouldBe` Right "hLArzI8DbqOXiEi6HqGQh9VxdtWlCXnKS1n/vvCOwL0="
        ...
        it "golden 1-byte payload salted"
            ...
            `shouldBe` Right "U2FsdGVkX18wMDAwMDAwMJCtI/MMCZ7dqZI6TrtlLyg="
        ...
        it "golden 2-byte payload salted"
            ...
            `shouldBe` Right "U2FsdGVkX18wMDAwMDAwMKZlzmc8ZsSxwTwDKJY4J+I="
        ...
        it "golden 5-byte payload salted"
            ...
            `shouldBe` Right "U2FsdGVkX18wMDAwMDAwMCP7dvgIZ/E+vzQN+2JuGF0="
        ...
        it "golden 10-byte payload salted"
            ...
            `shouldBe` Right "U2FsdGVkX18wMDAwMDAwMDXxr84l/PtjzWRCPGb8oSY="
        ...
        it "golden 15-byte payload salted"
            ...
            `shouldBe` Right "U2FsdGVkX18wMDAwMDAwMPCoW2E+adl2BLopcz8iftA="
