...
spec_balanceTx = describe "balanceTx" $ do
    ...
    describe "change address generation" $ do
        ...
        let address = Convert.toWallet . view addrTxOutL
    ...
    it "assigns minimal ada quantities to outputs without ada" $ do
        ...
        head outs `shouldBe` (Convert.toConwayTxOut out')
    ...
    describe "stake key deposit lookup" $ do
        ...
        let partialTxWithRefund :: Coin -> PartialTx ConwayEra
    ...
    describe "when passed unresolved inputs" $ do
        it "fails with ErrBalanceTxUnresolvedInputs" $ do
            ...
            let partialTx :: PartialTx ConwayEra
...
balanceTxGoldenSpec = describe "balance goldens" $ do
        ...
        it "pingPong_2" $ do
            ...
            Golden
                ...
                , writeToFile = \fp x -> T.writeFile fp
                    ...
                    . B8.unpack
                    . hex
                    . serializeTx @ConwayEra
                    $ x
                , readFromFile =
                    fmap (deserializeConwayTx . unsafeFromHex . B8.pack)
                    . readFile
                , goldenFile = dir </> name </> "golden"
                , actualFile = Just (dir </> name </> "actual")
                , failFirstTime = False
  ...
  where
    ...
    test :: String -> PartialTx ConwayEra -> Spec
    ...
    readTestTransactions = runIO $ do
        ...
        flip foldMap paths $ \f ->
            ...
            then pure []
                ...
                pure [(f, deserializeConwayTx $ unsafeFromHex contents)]
...
prop_balanceTxUnableToCreateInput
    ...
    :: forall era. era ~ Write.ConwayEra
...
deserializeConwayTx :: ByteString -> Tx Write.ConwayEra
deserializeConwayTx
    = fromCardanoApiTx
    . either (error . show) id
...
paymentPartialTx :: [W.TxOut] -> PartialTx Write.ConwayEra
paymentPartialTx txouts =
    PartialTx (mkBasicTx body) mempty mempty (StakeKeyDepositMap mempty) mempty
  where
    body = mkBasicTxBody
        & outputsTxBodyL .~
            StrictSeq.fromList (Convert.toConwayTxOut <$> txouts)
...
pingPong_1 :: PartialTx ConwayEra
pingPong_1 = PartialTx tx mempty mempty (StakeKeyDepositMap mempty) mempty
  where
    tx = deserializeConwayTx $ unsafeFromHex $ mconcat
...
pingPong_2 :: PartialTx ConwayEra
pingPong_2 = PartialTx
    { tx = deserializeConwayTx $ mconcat
