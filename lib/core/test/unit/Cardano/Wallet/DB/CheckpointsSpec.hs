...
spec = do
    describe "sparseCheckpoints" $ do
        it "k=2160, h=42" $ \_ -> do
            ...
            getQuantity <$> sparseCheckpoints cfg h `shouldBe`
                [ 0
...
prop_sparseCheckpointTipAlwaysThere (GenSparseCheckpointsArgs cfg h) = prop
  ...
  where
    cps = getQuantity <$> sparseCheckpoints cfg (Quantity h)
...
prop_checkpointsEventuallyEqual args@(GenSparseCheckpointsArgs cfg h) =
  ...
  where
    prop_eventuallyReachesExpectedTip
        ...
        -> Property
    prop_eventuallyReachesExpectedTip tip dbs =
        last dbs === SparseCheckpointsDB (getQuantity <$> sparseCheckpoints cfg tip)
    ...
    step cps (SparseCheckpointsDB db) =
      ...
      where
        toKeep = getQuantity <$> sparseCheckpoints (cfg { edgeSize = 0 }) (Quantity h)
        cps' = last cps : (toKeep `L.intersect` cps)

    prune :: SparseCheckpointsDB -> SparseCheckpointsDB
    prune (SparseCheckpointsDB db) = SparseCheckpointsDB db'
      where
        tip = Quantity (last db)
        db' = (getQuantity <$> sparseCheckpoints cfg tip) `L.intersect` db
