...
prop_forbiddenAddreses rnd@(Rnd st rk pwd) addrIx = conjoin
    ...
    , (addr `elem` ((\(a,_,_,_) -> a) <$> knownAddresses isOursSt))
    , (changeAddr `elem` ((\(a,_,_,_) -> a) <$> knownAddresses changeSt))
    ]
  where
...
prop_oursAreUsed rnd@(Rnd st _ _) addrIx = do
    case find (\(a,_,_,_) -> a == addr) $ knownAddresses $ snd $ isOurs addr st of
        Nothing ->
