...
mkApiCoinSelection
    ...
    -> ApiCoinSelection n
mkApiCoinSelection deps mcerts metadata cs =
    ApiCoinSelection
        { inputs = mkApiInput <$> view #unsignedInputs cs
  ...
  where
    ...
    mkApiOutput :: output -> ApiCoinSelectionOutput n
    mkApiOutput (TxOut addr (TokenBundle amount assets)) =
