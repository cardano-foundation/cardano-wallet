    ...
    blockfrostLightSync
        ...
        follower = do
            ...
            getBlockHeaderAt =
              runBF . \case
                ChainPointAtGenesis -> do
                  block <- BF.getBlock $ Right $ genesisBlockHash network
                  pure . either (error . show) Just $ fromBlockfrost block
                ChainPoint (SlotNo slot) blockHeaderHash -> do
            ...
            getAddressTxs ::
              ...
              IO ChainEvents
            getAddressTxs bhFrom bhTo =
              runBF . fmap fromBlockEvents . \case
                Left address -> do
...
genesisBlockHash nd =
  case networkDiscriminantToId nd of
    Mainnet ->
      BF.BlockHash
    ...
    metadata <-
      if null metadataJSON
        then pure Nothing
          ...
          Just . TxMetadata . Map.fromList . catMaybes
            <$> for metadataJSON \BF.TransactionMetaJSON {..} -> do
              ...
              fmap (label,)
                <$> for
                  _transactionMetaJSONJSONMetadata
