...
startMetadataServer MetadataServerArgs {sourceJson, port} = do
  let
    server = queryServerReloading sourceJson
...
parserInfo =
  ...
  where
    docs =
      mconcat
        [ p
            [ "This serves only the `POST /metadata/query' endpoint of Cardano "
        ...
        , p
            [ "On every request, the server will read the file, filter and "
