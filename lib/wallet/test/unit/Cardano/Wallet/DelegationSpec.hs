...
spec = describe "Cardano.Wallet.DelegationSpec" $ do
  ...
  describe "Join/Quit Stake pool unit mockEventSource" $ do
    it "Cannot join A, when active = A" $ do
      let
        dlg = WalletDelegation {active = Delegating pidA, next = []}
    ...
    it "Cannot join A, when next = [A]" $ do
      let
        next1 = WalletDelegationNext (EpochNo 1) (Delegating pidA)
      let
        dlg = WalletDelegation {active = NotDelegating, next = [next1]}
      WD.guardJoin knownPools dlg pidA noRetirementPlanned
        `shouldBe` Left (W.ErrAlreadyDelegating pidA)
    it "Can join A, when active = A, next = [B]" $ do
      let
        next1 = WalletDelegationNext (EpochNo 1) (Delegating pidB)
      let
        dlg =
          WalletDelegation
    ...
    it "Cannot join A, when active = A, next = [B, A]" $ do
      let
        next1 = WalletDelegationNext (EpochNo 1) (Delegating pidB)
      let
        next2 = WalletDelegationNext (EpochNo 2) (Delegating pidA)
      let
        dlg =
          WalletDelegation
            { active = Delegating pidA
    ...
    it "Cannot join when pool is unknown" $ do
      let
        dlg = WalletDelegation {active = NotDelegating, next = []}
    ...
    it "Cannot quit when active: A, next = [not_delegating]" $ do
      let
        next1 = WalletDelegationNext (EpochNo 1) NotDelegating
      let
        dlg =
          WalletDelegation
            { active = Delegating pidA
    ...
    it "Cannot quit when active: A, next = [B, not_delegating]" $ do
      let
        next1 = WalletDelegationNext (EpochNo 1) (Delegating pidB)
      let
        next2 = WalletDelegationNext (EpochNo 2) NotDelegating
      let
        dlg =
          WalletDelegation
            { active = Delegating pidA
