...
balanceTransactionGoldenSpec = describe "balance goldens" $ do
    it "testPParams" $ do
        ...
        let ledgerPParams =
                Cardano.toLedgerPParams Cardano.ShelleyBasedEraBabbage
  ...
  where
    ...
    _prop_outputsSatisfyMinAdaRequirement (Cardano.ShelleyTx _ tx) = do
      ...
      where
        ...
        valid :: WriteTx.TxOut (Cardano.ShelleyLedgerEra era) -> Property
        valid out = counterexample msg $ property $
            not $ WriteTx.isBelowMinimumCoinForTxOut era ledgerPParams out
          where
    ...
    hasZeroAdaOutputs (Cardano.ShelleyTx _ tx) =
        any hasZeroAda (WriteTx.outputs era $ WriteTx.txBody era tx)
      where
        era = recentEra @era
        hasZeroAda (WriteTx.BabbageTxOut _ val _ _) =
    ...
    ledgerPParams = Write.pparamsLedger $
        mockBundledProtocolParametersForBalancing (shelleyBasedEra @era)
...
pingPong_2 = PartialTx
    ...
    , inputs = WriteTx.toCardanoUTxO $ WriteTx.utxoFromTxOutsInRecentEra RecentEraBabbage
