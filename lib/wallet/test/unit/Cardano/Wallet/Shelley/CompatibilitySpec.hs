...
import Cardano.Ledger.Shelley qualified as SL
import Cardano.Ledger.Shelley.PParams qualified as SL
...
import Cardano.Wallet.Address.Derivation qualified as Address.Derivation
...
spec = do
  describe "Conversions" $ do
    it "toTip' . fromTip' == id" $ property $ \gh tip -> do
      let
        fromTip' = fromTip gh
      let
        toTip' = toTip gh :: W.BlockHeader -> Tip (CardanoBlock StandardCrypto)
  ...
  describe "Shelley Addresses" $ do
    ...
    prop "Shelley addresses from bech32" $ \k ->
      let
        addr@(Address bytes) =
          paymentAddress @ShelleyKey @'CredFromKeyK SMainnet k
      in
        decodeAddress SMainnet (bech32 bytes) === Right addr
    ...
    prop "Shelley addresses from bech32 - testnet"
      $ \k -> withSNetworkId (NTestnet 0)
        $ \n ->
          ...
          in
            decodeAddress n (bech32testnet raw)
              === Right addr
    ...
    prop "Byron addresses from base58" $ \k ->
      ...
      in
        decodeAddress SMainnet (base58 bytes) === Right addr
  ...
  describe "decentralizationLevelFromPParams" $ do
    ...
    forM_ testCases $ \(input, expectedOutput) -> do
      let
        title = show input <> " -> " <> show expectedOutput
      let
        output =
          input
            & toRational
...
genMnemonic = do
  let
    n = fromIntegral (natVal $ Proxy @(EntropySize mw)) `div` 8
  bytes <- BS.pack <$> vector n
  let
    ent = unsafeMkEntropy @(EntropySize mw) bytes
