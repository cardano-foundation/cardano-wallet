...
import Cardano.Wallet.Address.Derivation.Shelley qualified as Seq
...
import Cardano.Wallet.Checkpoints qualified as Checkpoints
...
import Cardano.Wallet.DB.Sqlite.Types qualified as DB
import Cardano.Wallet.DB.StateMachine
  ( TestConstraints
  , prop_sequential
  , validateGenerators
  )
import Cardano.Wallet.DB.Store.Info.Store qualified as WalletInfo
...
loggingSpec = withLoggingDB @TestState $ do
    ...
    it "should not log query parameters"
      $ \(getLogs, DBFresh {bootDBLayer}) -> do
        void
          ...
          $ DBLayerParams testCpSeq testMetadata mempty gp
        let
          walletName = T.unpack $ coerce $ name testMetadata
  ...
  describe "Sqlite observables" $ do
    it "should measure query timings" $ \(getLogs, DBFresh {bootDBLayer}) ->
      do
        DBLayer {readCheckpoint, atomically} <-
...
fileModeSpec = do
  describe "Check db opening/closing" $ do
    it "Opening and closing of db works" $ do
      replicateM_ 25 $ do
        db <- temporaryDBFile
        withShelleyFileDBFresh @TestState
          db
          (\_ -> pure ())
  ...
  describe "DBFactory" $ do
    ...
    let
      withDBFactory action = withSystemTempDirectory "DBFactory" $ \dir -> do
        dbf <-
          newDBFactory
            ShelleyWallet
            nullTracer
    ...
    let
      whileFileOpened delay f action = do
        opened <- newEmptyMVar
        concurrently_
    ...
    it "removeDatabase still works if file is opened" $ do
      withDBFactory $ \dir DBFactory {..} -> do
        ...
        let
          dbfile = "she." <> show (getWalletId testWid) <> ".sqlite"
  ...
  before temporaryDBFile
    $ describe "Check db reading/writing from/to file and cleaning"
    $ do
      ...
      describe "Golden rollback scenarios" $ do
        ...
        let
          mockApply DBLayer {..} h mockTxs = do
            ...
            let
              slotA = view #slotNo $ currentTip cpA
            let
              Quantity bhA = view #blockHeight $ currentTip cpA
            let
              hashA = headerHash $ currentTip cpA
            let
              fakeBlock =
                Block
                  ( BlockHeader
                  ...
                  mempty
            let
              (FilteredBlock {transactions = txs}, (_, cpB)) =
...
attachPrivateKey DBLayer {..} = do
  let
    pwd = Passphrase $ BA.convert $ T.encodeUtf8 "simplevalidphrase"
  seed <- liftIO $ generate $ SomeMnemonic <$> genMnemonic @15
  (scheme, h) <- liftIO $ encryptPassphrase pwd
  let
    k = generateKeyFromSeed (seed, Nothing) (preparePassphrase scheme pwd)
...
manualMigrationsSpec = describe "Manual migrations" $ do
  ...
  it
    "not 'migrate' db with no passphrase scheme set."
    testMigrationPassphraseScheme2
  it
    "not 'migrate' db with no date."
    testMigrationPassphraseScheme3
  it
    "not 'migrate' db with no passphrase scheme set and no date."
...
withinCopiedFile
  :: FilePath
  -> (FilePath -> Tracer IO msg -> IO a)
  -> IO ([msg], a)
withinCopiedFile dbName action = do
  let
    orig = $(getTestData) </> dbName
...
testMigrationRole dbName = do
  ...
  let
    migrationMsg = filter isMsgManualMigration logs
...
testMigrationSeqStateDerivationPrefix dbName prefix = do
  ...
  where
    isMsgManualMigration = matchMsgManualMigration $ \field ->
      let
        fieldInDB = fieldDB $ persistFieldDef DB.SeqStateDerivationPrefix
      in
        fieldName field == unFieldNameDB fieldInDB
...
isMsgManualMigrationPw = matchMsgManualMigration $ \field ->
  let
    fieldInDB = fieldDB $ persistFieldDef DB.WalPassphraseScheme
  in
    fieldName field == unFieldNameDB fieldInDB

localTxSubmissionTableExists :: Text
localTxSubmissionTableExists =
  [i|
    SELECT EXISTS (
        SELECT
            name
    ...
    allTransactionDataIsRepresentedInTxMeta path = do
      let
        ...
        shouldBe' mtxs t = do
          ...
          liftIO
            $ Set.fromList txs
            `shouldSatisfy` \x -> x `Set.isSubsetOf` (Set.fromList mtxs)
