...
import Data.TreeDiff qualified as Expr
import Data.TreeDiff.OMap qualified as Expr
import Fmt
  ( Buildable
  )
import GHC.Generics
  ...
  where
    ...
    expectWithin delay pre action = do
      ...
      ticks <- replicateM n $ do
        ...
        getCurrentTime
      let
        times = zipWith diffUTCTime ticks (start : ticks)
      let
        avg = (sum (fromEnum <$> times)) `div` n
