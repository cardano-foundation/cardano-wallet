...
import Cardano.Api.Gen qualified as Cardano
...
spec = do
  ...
  describe "DatumHash" $ do
    it "datumHashFromBytes . datumHashToBytes == Just"
      ...
      $ \h -> do
        let
          f = datumHashFromBytes . datumHashToBytes
        f h === Just h
    ...
    describe "computeMinimumCoinForTxOut" $ do
      it
        ...
        $ \out delta perByte -> do
          let
            pp = def & ppCoinsPerUTxOByteL .~ perByte
          let
            era = RecentEraBabbage
          let
            c = delta <> computeMinimumCoinForTxOut era pp out
          isBelowMinimumCoinForTxOut
            era
            pp
