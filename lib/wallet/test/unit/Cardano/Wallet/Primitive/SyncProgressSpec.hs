...
spec = do
  let
    t0 = read "2019-11-09 16:43:02 UTC"
  let
    sp =
      SlottingParameters
  ...
  let
    ti = (mkSingleEraInterpreter (StartTime t0) sp :: TimeInterpreter Identity)
  let
    runQry = runIdentity . interpretQuery ti
  describe "syncProgress" $ do
    ...
    let
      tolerance = SyncTolerance 0

    it "unit test #1 - 0/10   0%" $ do
      let
        tip = (SlotNo 0)
      let
        ntwkTime = runQry $ slotToRelTime $ SlotNo 10
    ...
    it "unit test #2 - 10/20 50%" $ do
      let
        tip = SlotNo 10
      let
        ntwkTime = runQry $ slotToRelTime $ SlotNo 20
      runIdentity (syncProgress tolerance ti tip ntwkTime)
    ...
    it "unit test #7 - 1k/2M 0.5% (regression for overflow issue)" $ do
      let
        tip = SlotNo 1_000
      let
        ntwkTime = runQry $ slotToRelTime $ SlotNo 2_000_000
      runIdentity (syncProgress tolerance ti tip ntwkTime)
    ...
    it "syncProgress should never crash"
      ...
      $ \tip dt -> monadicIO $ do
        let
          x = runIdentity $ syncProgress tolerance ti tip dt
