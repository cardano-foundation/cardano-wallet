...
import Cardano.Slotting.Slot qualified as Cardano
...
import Ouroboros.Consensus.HardFork.History.EraParams qualified as HF
import Ouroboros.Consensus.HardFork.History.Qry qualified as HF
import Ouroboros.Consensus.HardFork.History.Summary qualified as HF
import Test.Hspec
  ( Spec
  , describe
  , it
...
spec = do
  describe "slotting" $ do
    describe "runQuery NEW mkSingleEraInterpreter == OLD . fromFlatSlot" $ do
      ...
      it "slotRangeFromTimeRange and slotRangeFromTimeRange'"
        ...
        $ \t0 sp timeRange -> do
          ...
          let
            legacy = slotRangeFromTimeRange' (slotParams t0 sp) timeRange

          let
            el = sp ^. #getEpochLength
          let
            res' = fmap (fromFlatSlot el . unSlotNo) <$> res
          res' === legacy
            ...
            res =
              runIdentity
                $ interpretQuery
                  ...
                  (firstSlotInEpoch e)
          let
            legacy = SlotNo $ flatSlot (sp ^. #getEpochLength) $ SlotId e 0
      ...
      it "Can interpret queries for multiple eras at once (regression for ADP-626)" $ do
        ...
        let
          ti = mkTimeInterpreter tr startTime (pure forkInterpreter)
    ...
    describe "TimeInterpreter conversions beyond the safe zone" $ do
      ...
      let
        failingQry = slotToUTCTime (SlotNo 100000)
