...
spec = describe "Cardano.Wallet.Primitive.Types" $ do
  ...
  describe "Buildable" $ do
    ...
    it "UTxOStatistics" $ do
      let
        txin h = TxIn (Hash h) 0
      let
        txout c = TxOut (Address "") (TokenBundle.fromCoin $ Coin c)
      let
        ada l = l * 1000 * 1000
      let
        utxo =
      ...
      let
        stats = UTxOStatistics.compute utxo
      pretty stats
  ...
  let
    slotsPerEpoch = getEpochLength sp
  describe "flatSlot" $ do
    ...
    it "flatSlot . fromFlatSlot == id" $ property $ \n -> do
      ...
      let
        maxSlotInEpoch = SlotInEpoch $ unEpochLength slotsPerEpoch - 1
      let
        maxSlotId = SlotId (EpochNo maxBound) maxSlotInEpoch
      let
        maxFlatSlot = flatSlot slotsPerEpoch maxSlotId
      let
        result = flatSlot slotsPerEpoch $ fromFlatSlot slotsPerEpoch n
      checkCoverage
  ...
  describe "Ranges" $ do
    ...
    it "Range (succ a) b `isSubrangeOf` Range a b"
      ...
      $ \nsr ->
        let
          r@(Range a b :: Range Int) = getNonSingletonRange nsr
        in
          checkCoverage
            $ cover 10 (rangeHasLowerBound r) "has lower bound"
            $ cover 10 (rangeHasUpperBound r) "has upper bound"
  ...
  describe "Epoch arithmetic: arbitrary value generation" $ do
    ...
    it "SlotParametersAndTimePoint generation covers interesting cases"
      ...
      $ \(SlotParametersAndTimePoint sps t) ->
        let
          belowMin = t < epochStartTime sps minBound
  ...
  describe "Slot arithmetic" $ do
    ...
    it "applyN (flatSlot slot) slotPred slot == Just slotMinBound"
      ...
      $ \(sps, slot) -> do
        let
          n = flatSlot (getEpochLength sps) slot
        Just slotMinBound
    ...
    it "applyN (flatSlot slot + 1) slotPred slot == Nothing"
      ...
      $ \(sps, slot) -> do
        let
          n = flatSlot (getEpochLength sps) slot + 1
    ...
    it "(applyN n slotSucc) . (applyN n slotPred) == id"
      ...
      $ \(sps, slot) (NonNegative (n :: Int)) ->
        flatSlot (getEpochLength sps) slot >= fromIntegral n ==> do
          let
            s = applyN n (slotSucc sps <$>)
          let
            p = applyN n (slotPred sps =<<)
    ...
    it "slotAt' . slotStartTime == id"
      ...
      $ \(sps, slot) -> do
        let
          f = slotAt' sps . slotStartTime sps
        Just slot === f slot
    ...
    it "slotCeiling . slotStartTime == id"
      ...
      $ \(sps, slot) -> do
        let
          f = slotCeiling sps . slotStartTime sps
        slot === f slot
    ...
    it "slotFloor . slotStartTime == id"
      ...
      $ \(sps, slot) -> do
        let
          f = slotFloor sps . slotStartTime sps
        Just slot === f slot
      ...
      let
        err =
          "name is too long: expected at most "
            ...
            <> " characters"
      let
        walName = T.pack (replicate (walletNameMaxLength + 1) 'x')
...
instance Arbitrary SlotParametersAndTimePoint where
  arbitrary = do
    ...
    let
      timeA = 0
    let
      timeB = toModifiedJulianDay $ utctDay $ epochStartTime sps minBound
    let
      timeC = toModifiedJulianDay $ utctDay $ epochStartTime sps maxBound
    let
      timeD = timeC * 2
