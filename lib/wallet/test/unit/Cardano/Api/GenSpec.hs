...
import Test.Cardano.Ledger.Alonzo.PlutusScripts qualified as Plutus
import Test.Hspec
  ( Spec
  , describe
  , it
  ...
  describe "Cardano.Api.Gen"
    ...
    $ do
      ...
      it "genValueForTxOutCoverage"
        $ property
        $ forAll genValueForTxOut genValueForTxOutCoverage
      ...
      it "genByronKeyWitness"
        $ property
        $ forAll genByronKeyWitness genByronKeyWitnessCoverage
      ...
      it "genRationalCoverage"
        $ property
        $ forAll genRational genRationalCoverage
      it "genRationalInt64Coverage"
        $ property
        $ forAll genRationalInt64 genRationalInt64Coverage
      it "genEpochNo"
      ...
      it "genCostModels"
        $ property
        $ forAll genCostModels genCostModelsCoverage
  ...
  case collateralSupportedInEra era of
    ...
    Just _ ->
      checkCoverage
        $ cover
          10
          (hasNoCollateral collateral)
          "no collateral"
        $ cover
          10
          (hasSomeCollateral collateral)
          "some collateral"
        $ cover
          2
          (collateralLength collateral == Just 0)
        ...
        $ cover
          10
          (collateralLength collateral > Just 3)
  ...
  case validityLowerBoundSupportedInEra era of
    Nothing ->
      validFrom == TxValidityNoLowerBound
        ...
        & counterexample
          ( "validity lower bound shouldn't be supported in "
              <> show era
          )
    Just _ ->
      case validFrom of
        TxValidityNoLowerBound ->
          False
            & counterexample
              ( "validity lower bound supported in "
                  <> show era
    ...
    Nothing ->
      case validFrom of
        (TxValidityNoUpperBound _) ->
          True
            & label
              ( "validity upper bound not supported in "
                  <> show era
              )
            & counterexample
    ...
    Just _ ->
      case validFrom of
        TxValidityNoUpperBound _ ->
          False
            & counterexample
              ( "validity upper bound supported in "
                  <> show era
...
genTxValidityRangeCoverage
  ...
  -> Property
genTxValidityRangeCoverage era (lower, upper) =
  checkCoverage
    $ conjoin
...
genExtraKeyWitnessesCoverage era ws =
  case extraKeyWitnessesSupportedInEra era of
    Nothing ->
      label
        "extra key witnesses are not generated in unsupported eras"
        (noWitnesses ws)
        & counterexample
          ( "key witnesses were generated in unsupported "
              <> show era
          )
...
genSimpleScriptCoverageV1 =
  forAll genSimpleScript $ \script ->
    checkCoverage
      $ cover
        10
        (requiresSignature script)
        "script has \"require signature\""
      $ cover
        10
        (requiresAllOf script)
        "script has \"require all of\""
      $ cover
        10
        (requiresAnyOf script)
      ...
      $ cover
        10
        (hasOnlyNonRecursiveScriptPrimitive script)
        "script has only non-recursive script primitives (sig, timeBefore, timeAfter)"
      $ cover
        10
        (not (hasOnlyNonRecursiveScriptPrimitive script))
...
genSimpleScriptCoverageV2 =
  forAll genSimpleScript $ \script ->
    checkCoverage
      ...
      $ cover
        10
        (requiresTimeBefore script)
        "script has \"time before\""
      $ cover
        10
        (requiresTimeAfter script)
...
genAssetNameCoverage n =
  checkCoverage
    ...
    $ cover
      0.1
      (assetNameLen == shortLength)
      "asset name is short"
    $ cover
      5
      (assetNameLen == longLength)
      "asset name is long"
    $ cover
      5
      (assetNameLen > shortLength && assetNameLen < longLength)
...
genValueForMintingCoverage val =
  checkCoverage
    $ cover
      10
      (hasMintingValue val)
...
genTxMintValueCoverage era val =
  case multiAssetSupportedInEra era of
    Left _ ->
      label
        "mint values are not generated in unsupported eras"
        (val == TxMintNone)
        & counterexample
          ( "a mint value was generated in an unsupported "
              <> show era
          )
    Right _ ->
      checkCoverage
...
genExecutionUnitsCoverage (ExecutionUnits steps mem) =
  checkCoverage
    $ conjoin
    ...
    if null supportedLangs
      then
        wit == KeyWitness KeyWitnessForStakeAddr
          ...
          & counterexample
            ( "a script witness was generated in "
                <> show era
  ...
  case withdrawalsSupportedInEra era of
    Nothing ->
      ws == TxWithdrawalsNone
        ...
        & counterexample
          ( "withdrawals were generated in unsupported era "
              <> show era
          )
    Just _ -> checkCoverage
      $ case ws of
  ...
  case auxScriptsSupportedInEra era of
    Nothing ->
      aux == TxAuxScriptsNone
        ...
        & counterexample
          ( "aux scripts were generated in unsupported "
              <> show era
          )
    Just _ -> checkCoverage
      $ case aux of
...
genTxMetadataCoverage (TxMetadata meta) =
  ...
  in
    checkCoverage
      ...
      $ conjoin
      $ fmap (metaNumberCoverage . fst) metaMap
  where
    metaNumberCoverage n =
      checkCoverage
        $ cover
          0.1
          (n == 0)
        ...
        $ cover
          10
          (n >= veryLargeMetaIndex)
          "meta index is large"
        $ cover
          10
          (n > 0 && n < veryLargeMetaIndex)
  ...
  case txMetadataSupportedInEra era of
    Nothing ->
      meta == TxMetadataNone
        ...
        & counterexample
          ( "metadata was generated in unsupported "
              <> show era
          )
    Just _ -> checkCoverage
      $ case meta of
...
genStakeAddressReferenceCoverage ref =
  checkCoverage
    $ cover
      10
      (byValue ref)
...
genPaymentCredentialCoverage paymentCred =
  checkCoverage
    $ cover
      20
      (isByKey paymentCred)
  ...
  case scriptDataSupportedInEra era of
    Nothing ->
      (datum == TxOutDatumNone)
        ...
        & counterexample
          ( "tx out datums were generated in unsupported "
              <> show era
          )
    Just _ ->
      checkCoverage
  ...
  case refInsScriptsAndInlineDatsSupportedInEra era of
    ...
    Just _ ->
      checkCoverage
        $ cover
          30
          (hasNoRefScript refScript)
          "no reference script"
        $ cover
          30
          (hasRefScript refScript)
...
genTxOutCoverage era (TxOut addr val datum refScript) =
  checkCoverage
    $ conjoin
      [ genAddressInEraCoverage era addr
      , genTxOutValueCoverage era val
...
genWitnessNetworkIdOrByronAddressCoverage witNetworkOrByron =
  checkCoverage
    $ cover
      30
      (isNetworkIdWit witNetworkOrByron)
      "is network id witness"
    $ cover
      30
      (isByronAddrWit witNetworkOrByron)
...
genShelleyWitnessSigningKeyCoverage sk =
  checkCoverage
    $ case sk of
      WitnessPaymentKey _ ->
...
genCostModelCoverage (CostModel costModel) =
  checkCoverage
    $ conjoin
      [ length costModel
  ...
  $ case certificatesSupportedInEra era of
    Nothing ->
      certs == TxCertificatesNone
        ...
        & counterexample
          ( "tx certificate was generated in unsupported "
              <> show era
          )
    Just _ ->
      case certs of
  ...
  $ case updateProposalSupportedInEra era of
    Nothing ->
      proposal == TxUpdateProposalNone
        ...
        & counterexample
          ( "tx update proposal was generated in unsupported "
              <> show era
          )
    Just _ ->
      case proposal of
  ...
  checkCoverage
    $ conjoin
      ...
      , checkCoverage
          ...
          $ cover
            30
            (denominator x > 1 && denominator x < veryLarge)
            "rational denominator is between 1 and very large"
          $ cover
            5
            (denominator x > veryLarge)
