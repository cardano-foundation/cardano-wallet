...
import Cardano.Wallet.Address.Book qualified as Sqlite
...
import Cardano.Wallet.DB.Store.Checkpoints.Store qualified as Sqlite
...
import Cardano.Wallet.Submissions.Submissions qualified as Smbs
import Cardano.Wallet.Submissions.TxStatus qualified as Sbms
...
walletUpdatePassphrase wallet new mxprv = monadicIO $ do
  ...
  where
    prop_withoutPrivateKey wl wid = do
      ...
      let
        err = ErrUpdatePassphraseWithRootKey $ ErrWithRootKeyNoRootKey wid
...
walletUpdatePassphraseDate wallet (xprv, pwd) = monadicIO $ liftIO $ do
  ...
  let
    infoShouldSatisfy predicate = do
      info <-
...
walletListTransactionsSorted wallet@(_, _, _) _order (_mstart, _mend) =
  forAll (logScale' 1.5 arbitrary) $ \history ->
    monadicIO $ liftIO $ do
      ...
      let
        times = Map.fromList [(txInfoId i, txInfoTime i) | i <- txs]
      let
        expTimes =
          Map.fromList
              ...
              txs <-
                unsafeRunExceptT
                  $ W.listTransactions
                    ...
                    Nothing
              let
                times = [(txInfoId i, txInfoTime i) | i <- txs]
              shouldBe times $ take limit $ sortOn (dir . snd) $ do
                (tx, meta) <- history'
                let
                  slot = slotNoTime (meta ^. #slotNo)
                guard $ cut slot
                pure (tx ^. #txId, slot)
            pointInRange xs f =
              slotNoTime
            ...
            monadicIO $ do
              ...
              case sort [meta ^. #slotNo | (_, meta) <- history'] of
                ...
                xs -> do
                  ...
                  let
                    l = pointInRange xs (`div` 3)
                  let
                    r = pointInRange xs ((`div` 3) . (* 2))
...
walletListsOnlyRelatedAssets txId txMeta =
  forAll genOuts $ \(out1, out2, wallet) -> monadicIO $ do
    WalletLayerFixture _ DBLayer {..} wl _ <-
      liftIO
        $ setupFixture dummyStateWithAddressesF wallet
    let
      listHistoricalAssets hry = do
  ...
  arbitrary = GenSubmissions <$> logScale' 2.7 (listOf1 genBuiltTx)
    where
      ...
      genBuiltTx = do
        sl <- genSmallSlot
        let
          bh = Quantity $ fromIntegral $ unSlotNo sl
...
instance Arbitrary TxRetryTest where
  arbitrary = do
    ...
    let
      results = fmap (,True) (builtSealedTx <$> metas)
  ...
  getMonotonicTimeNSec = do
    ...
    modifyMVar timeVar $ \t ->
      let
        t' = timeStep + coerce t
      in
        pure (coerce t', timeToNanoTime $ coerce t')
...
prop_localTxSubmission tc = monadicIO $ do
  ...
  res <- run
    ...
    $ \ctx@(WalletLayer tr _ nl _ db) -> do
      ...
      -- Run test
      let
        cfg = LocalTxSubmissionConfig (timeStep st) 10
  ...
  let
    requested x = x `elem` (builtSealedTx <$> retryTestPool tc)
  assert' "all txs in submissions pool were required"
    $ all requested
    $ resSubmittedTxs res

  let
    inPool BuiltTx {builtTx} =
...
dummyTransactionLayer =
  TransactionLayer
    { mkTransaction = \_era _stakeCredentials keystore _pp _ctx cs -> do
        let
          inps' = NE.toList $ second Just <$> view #inputs cs
        -- TODO: (ADP-957)
        let
          cinps' = []
        let
          txId = mkTxId inps' (view #outputs cs) mempty Nothing
        let
          tx =
            Tx
              { txId
        ...
        let
          wit = forMaybe (NE.toList $ view #inputs cs) $ \(_, TxOut addr _) -> do
            (xprv, Passphrase pwd) <- keystore addr
            let
              sigData = tx ^. #txId . #getHash
            let
              sig = CC.unXSignature $ CC.sign pwd (getKey xprv) sigData
            return
        ...
        let
          fakeBinary = fakeSealedTx (tx ^. #txId, wit)
  ...
  TestStateS
    $ defaultTestFeatures
      { isOwnedTest = \(TestState m) (rootK, pwd) addr -> do
          ...
          let
            accXPrv = deriveAccountPrivateKey pwd rootK minBound
            addrXPrv =
...
instance {-# OVERLAPS #-} Arbitrary (ShelleyKey 'RootK XPrv, Passphrase "user") where
  ...
  arbitrary = do
    ...
    mw <- arbitrary
    let
      key = generateKeyFromSeed (mw, Nothing) (preparePassphrase pwd)
    return (key, pwd)
