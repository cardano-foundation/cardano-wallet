  ...
  where
    ...
    prop = liftIO $ do
      forM_ pairs $ \(pool, slot) -> do
        let
          err = ErrPointAlreadyExists slot
...
prop_rollbackRegistration DBLayer {..} rollbackPoint (ManyPoolCertificates entries) =
  ...
  where
    ...
    ownerHasManyPools =
      let
        owners = concatMap (poolOwners . snd) entries
      in
        L.length owners > L.length (L.nub owners)
