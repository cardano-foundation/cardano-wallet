...
dummyByronChangeAddressGen = AnyChangeAddressGenWithState
    (defaultChangeAddressGen (byronRootK, pwd))
  ...
  where
    ...
    defaultChangeAddressGen
        :: ArgGenChange (RndState 'Mainnet)
        -> ChangeAddressGen (RndState 'Mainnet)
    defaultChangeAddressGen arg = ChangeAddressGen
        (first Convert.toLedgerAddress <$> genChange arg)
        (Convert.toLedgerAddress
            $ maxLengthAddressFor
            $ keyOfWallet (walletFlavor @(RndState 'Mainnet))
        )
