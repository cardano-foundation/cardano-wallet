...
main = withUtf8Encoding $ withLogging $ \trace -> do
  let
    tr = filterSeverity (pure . const Error) $ trMessageText trace
  defaultMain
    [ bgroupWriteUTxO tr
    , bgroupReadUTxO tr
...
bgroupWriteUTxO tr =
  ...
  where
    bUTxO n s a =
      bench lbl
        $ withCleanDB tr (fmap (,()) <$> walletFixture)
...
bgroupReadUTxO tr =
  ...
  where
    bUTxO n s a =
      bench lbl
        $ withCleanDB tr (fmap (,()) <$> utxoFixture n s a)
        $ benchReadUTxO . fst
      where
        lbl
          | a == 0 = n |+ " CP (ada-only) x " +| s |+ " UTxO"
...
bgroupReadTxHistory tr =
  ...
  where
    ...
    bTxHistory n a r o st s =
      bench lbl
        $ withCleanDB tr (fmap (,()) <$> txHistoryFixture n a r)
...
benchPutTxHistory numTxs numInputs numOutputs numAssets range DBLayer {..} = do
  let
    txs = mkTxHistory numTxs numInputs numOutputs numAssets range
...
mkTxHistory numTx numInputs numOutputs numAssets range =
  ...
  | !i <- [1 .. numTx]
  , let
      resolvedInputs = mkInputs i numInputs
  , let
      outputs = mkOutputs i numOutputs numAssets
  ]
  where
...
withCleanDB
  ...
  -> Benchmarkable
withCleanDB tr f g =
  perRunEnvWithCleanup setup (dbDown . fst)
    $ g . snd
  where
    setup = do
