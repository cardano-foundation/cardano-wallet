  ...
  where
    ...
    nFixtureWalletWithTxs n m = do
      ...
      let
        batchSize = 10
      let
        whole10Rounds = div m batchSize
      let
        lastBit = mod m batchSize
      let
        amtExp val = ((amt * fromIntegral val) + faucetAmt) :: Natural
      let
        expInflows =
          if whole10Rounds > 0
      ...
      let
        expInflows' = filter (/= 0) expInflows
    ...
    nFixtureWalletWithUTxOs n utxoNumber = do
      let
        utxoExp = replicate utxoNumber (minUTxOValue era)
    ...
    massiveFixtureWallet = do
      ...
      let
        wal1 = getFromResponse Prelude.id rWal
      pure (wal1, wal2, walMA, maWalletToMigrate)
    ...
    runScenario scenario =
      runResourceT $ scenario >>= \(wal1, wal2, walMA, maWalletToMigrate) -> liftIO $ do
        t1 <-
          measureApiLogs
            capture
        ...
        let
          coins = replicate 5 amt
        let
          payments = flip map (zip coins addresses) $ \(amount, address) ->
            [json|{
                "address": #{address},
                "amount": {
                    "quantity": #{amount},
        ...
        let
          assetsToSend = walMA ^. #assets . #total . #getApiT
        let
          val = minUTxOValue era <$ pickAnAsset assetsToSend
        payloadMA <-
        ...
        let
          assetsSrc = walMA ^. #assets . #total . #getApiT
        let
          (polId, assName) =
        ...
        let
          endpointPlan = (Link.createMigrationPlan @'Shelley maWalletToMigrate)
        t12a <-
          measureApiLogs capture
        ...
        -- Perform a migration:
        let
          endpointMigrate = Link.migrateWallet @'Shelley maWalletToMigrate
        t12b <-
          measureApiLogs capture
            $ request @[ApiTransaction n]
              ctx
...
withShelleyServer tracers action = do
  ctx <- newEmptyMVar
  let
    setupContext np baseUrl = do
      let
        sixtySeconds = 60 * 1_000_000 -- 60s in microseconds
      manager <-
        (baseUrl,)
          <$> newManager
            ( defaultManagerSettings
  ...
  where
    withServer act = withSystemTempDir nullTracer "latency" $ \dir -> do
      ...
      createDirectory db
      let
        logCfg = LogFileConfig Error Nothing Error
      let
        poolConfig = NE.head defaultPoolConfigs :| []
      withCluster
        nullTracer
        dir
