...
scenario_ADDRESS_LIST_01 fixture derPathSize = it title $ \ctx -> runResourceT $ do
  w <- fixture ctx
  r <-
    request @[ApiAddressWithPath n] ctx (Link.listAddresses @'Byron w) Default Empty
  verify r [expectResponseCode HTTP.status200]
  let
    n = length $ getFromResponse id r
...
scenario_ADDRESS_LIST_02 fixture = it title $ \ctx -> runResourceT $ do
  ...
  let
    nUsed = length $ getFromResponse id rUsed
  ...
  rUnused <-
    request @[ApiAddressWithPath n]
      ...
      Empty
  let
    nUnused = length $ getFromResponse id rUnused
...
scenario_ADDRESS_CREATE_01 = it title $ \ctx -> runResourceT $ do
  ...
  let
    payload = Json [json| { "passphrase": #{fixturePassphrase} }|]
  r <-
    request @(ApiAddressWithPath n) ctx (Link.postRandomAddress w) Default payload
  verify
    r
...
scenario_ADDRESS_CREATE_03 = it title $ \ctx -> runResourceT $ do
  w <- emptyRandomWallet ctx
  let
    payload = Json [json| { "passphrase": "Give me all your money." }|]
  r <-
    request @(ApiAddressWithPath n) ctx (Link.postRandomAddress w) Default payload
  verify
    r
...
scenario_ADDRESS_IMPORT_01 fixture = it title $ \ctx -> runResourceT $ do
  ...
  let
    addr = randomAddresses @n mw !! 42
  let
    (_, base) = Link.postRandomAddress w
  let
    link = base <> "/" <> encodeAddress (sNetworkId @n) addr
...
scenario_ADDRESS_IMPORT_04 fixture = it title $ \ctx -> runResourceT $ do
  ...
  r0 <-
    request @[ApiAddressWithPath n]
      ...
      Empty
  let
    (addr : _) = getFromResponse id r0

  -- Re-insert it
  let
    (_, base) = Link.postRandomAddress w
  let
    link = base <> "/" <> toUrlPiece (addr ^. #id)
  r1 <- request @() ctx ("PUT", link) Default Empty
  verify r1 [expectResponseCode HTTP.status204]
