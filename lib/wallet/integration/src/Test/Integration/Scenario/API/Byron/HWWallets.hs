...
spec = describe "BYRON_HW_WALLETS" $ do
  it "HW_WALLETS_01 - Restoration from account public key preserves funds" $ \ctx -> runResourceT $ do
    ...
    let
      mnemonicTxt = mnemonicToText @15 mnemonics
    let
      payldCrt =
        Json
          [json| {
    ...
    -- send funds
    let
      [addr] = take 1 $ icarusAddresses @n mnemonics
    let
      destination = encodeAddress (sNetworkId @n) addr
    let
    ...
    rTrans <-
      request @(ApiTransaction n)
        ctx
        (Link.createTransactionOld @'Byron wSrc)
        Default
        payload
    ...
    let
      accXPub = pubKeyFromMnemonics mnemonics
    wDest' <-
  ...
  describe "HW_WALLETS_03 - Cannot do operations requiring private key" $ do
    it "Cannot send tx" $ \ctx -> runResourceT $ do
      ...
      let
        pubKey = pubKeyFromMnemonics mnemonics
    ...
    it "Cannot update pass" $ \ctx -> runResourceT $ do
      ...
      -- cannot update pass
      let
        payload = updatePassPayload fixturePassphrase "new-wallet-passphrase"
      rup <-
        request @ApiByronWallet
          ctx
          (Link.putWalletPassphrase @'Byron wk)
          Default
          payload
  ...
  describe "HW_WALLETS_04 - Can manage HW wallet the same way as others" $ do
    it "Can update name" $ \ctx -> runResourceT $ do
      ...
      let
        newName = "new name"
      let
        payload = updateNamePayload newName
    ...
    it "Can get tx fee" $ \ctx -> runResourceT $ do
      ...
      rFee <-
        request @ApiFee
          ctx
          (Link.getTransactionFeeOld @'Byron wSrc)
          Default
          payload
    ...
    it "Can see utxo" $ \ctx -> runResourceT $ do
      ...
      rStat <-
        request @ApiUtxoStatistics
          ctx
          (Link.getUTxOsStatistics @'Byron wPub)
          Default
          Empty
    ...
    it "Can have address pool gap" $ \ctx -> runResourceT $ do
      ...
      let
        pubKey = pubKeyFromMnemonics mnemonics
      let
        addrPoolGap = 55 -- arbitraty but known
      let
        payloadRestore =
          Json
            [json| {
      ...
      let
        wPub = getFromResponse id rRestore
    ...
    it "Can list transactions" $ \ctx -> runResourceT $ do
      ...
      rt <-
        request @([ApiTransaction n])
          ctx
          (Link.listTransactions @'Byron wPub)
          Default
          Empty
      ...
      let
        outputs =
          zipWith3
            ...
            targetAmounts
            targetAssets
      selectCoins @n @'Byron ctx source payments
        >>= flip
          verify
  ...
  describe "HW_WALLETS_05 - Wallet from pubKey is available" $ do
    ...
    it "The same account and mnemonic wallet can live side-by-side" $ \ctx -> runResourceT $ do
      ...
      mnemonics <- liftIO $ entropyToMnemonic <$> genEntropy
      let
        mnemonicsTxt = mnemonicToText @15 mnemonics
