...
spec = describe "SHELLEY_WALLETS" $ do
  it "WALLETS_CREATE_01 - Create a wallet" $ \ctx -> runResourceT $ do
    ...
    let
      wid = getFromResponse id r
    eventually "Wallet state = Ready" $ do
      rg <-
        request @ApiWallet
          ctx
          (Link.getWallet @'Shelley wid)
          Default
          Empty
      expectField (#state . #getApiT) (`shouldBe` Ready) rg

  describe
    "OWASP_INJECTION_CREATE_WALLET_01 - \
    \SQL injection when creating a wallet"
    $ do
      let
        matrix =
          [
  ...
  it "WALLETS_CREATE_02 - Restored wallet preserves funds" $ \ctx -> runResourceT $ do
    ...
    let
      payldCrt = payloadWith "!st created" mnemonics
    ...
    -- send funds
    let
      wDest = getFromResponse id rInit
  ...
  it "WALLETS_CREATE_03,09 - Cannot create wallet that exists" $ \ctx -> runResourceT $ do
    ...
    verify
      ...
      , expectErrorMessage
          ( "This operation would yield a wallet with the\
            \ following id: "
              ++ T.unpack (getFromResponse walletId r1)
  ...
  describe "WALLETS_CREATE_04 - Wallet name" $ do
    let
      walNameMax = T.pack (replicate walletNameMaxLength 'ą')
    let
      matrix =
        [
          ( show walletNameMinLength ++ " char long"
          , "1"
          ,
            [ expectResponseCode HTTP.status201
            , expectField
  ...
  describe "WALLETS_CREATE_08 - address_pool_gap" $ do
    ...
    let
      maxDaedalusGap = (1_000 :: Word32)
    let
      addrPoolBig = fromIntegral maxDaedalusGap
    ...
    forM_ matrix $ \(title, addrPoolGap, expectations) -> it title $ \ctx -> runResourceT $ do
      m24 <- liftIO $ genMnemonics M24
      let
        payload = payloadWith' "Secure Wallet" m24 (fromIntegral addrPoolGap)
      rW <- postWallet ctx payload
      verify rW expectations

      let
        w = getFromResponse id rW
      rA <-
  ...
  it "WALLETS_LIST_01 - Created a wallet can be listed" $ \ctx -> runResourceT $ do
    ...
    let
      wid = getFromResponse walletId rp
  ...
  it "WALLETS_LIST_01 - Wallets are listed from oldest to newest" $ \ctx -> runResourceT $ do
    ...
    wids <- forM walletDetails $ \(name, mnemonics) -> do
      let
        payload = payloadWith name mnemonics
      rp <- postWallet ctx payload
      expectResponseCode HTTP.status201 rp
  ...
  it "WALLETS_UPDATE_01 - Updated wallet name is available" $ \ctx -> runResourceT $ do
    w <- emptyWallet ctx
    let
      passLastUpdateValue = w ^. #passphrase
    let
      newName = updateNamePayload "New great name"
    let
      walId = w ^. walletId
    let
      expectations =
  ...
  describe "WALLETS_UPDATE_02 - Various names" $ do
    ...
    forM_ matrix $ \(title, walName, expectations) -> it title $ \ctx -> runResourceT $ do
      w <- emptyWallet ctx
      let
        newName = updateNamePayload walName
      let
        endpoint = "v2/wallets" </> (w ^. walletId)
  ...
  it "WALLETS_UPDATE_03 - Deleted wallet cannot be updated (404)" $ \ctx -> runResourceT $ do
    ...
    let
      wid = w ^. walletId
    let
      endpoint = "v2/wallets" </> wid
    _ <- request @ApiWallet ctx ("DELETE", endpoint) Default Empty

    let
      newName = updateNamePayload "new name"
    ru <- request @ApiWallet ctx ("PUT", endpoint) Default newName
  ...
  it "WALLETS_UPDATE_PASS_01a - passphraseLastUpdate gets updated"
    $ \ctx -> runResourceT $ do
      w <- emptyWallet ctx
      let
        payload = updatePassPayload fixturePassphrase "New passphrase"
      let
        endpoint =
          "v2/wallets"
            </> (w ^. walletId)
      ...
      let
        getEndpoint = "v2/wallets" </> (w ^. walletId)
      let
        originalPassUpdateDateTime = w ^. #passphrase
  ...
  it "WALLETS_UPDATE_PASS_01b - passphraseLastUpdate gets updated, mnemonic"
    $ \ctx -> runResourceT $ do
      ...
      let
        payload = updatePassPayloadMnemonic mnemonic "New passphrase"
      let
        endpoint =
          "v2/wallets"
            </> (w ^. walletId)
  ...
  it
    "WALLETS_UPDATE_PASS_01c - passphraseLastUpdate gets updated, mnemonic \
    \using second factor"
    $ \ctx -> runResourceT $ do
  ...
  describe "WALLETS_UPDATE_PASS_02 - New passphrase values" $ do
    ...
    forM_ matrix $ \(title, passphrase, expectations) -> it title $ \ctx -> runResourceT $ do
      w <- emptyWallet ctx
      let
        payload = updatePassPayload fixturePassphrase passphrase
      let
        endpoint =
          "v2/wallets"
            </> (w ^. walletId)
    ...
    forM_ matrix $ \(title, passphrase, expectations) -> it title $ \ctx -> runResourceT $ do
      (w, mnemonic) <- emptyWalletAndMnemonic ctx
      let
        payload = updatePassPayloadMnemonic mnemonic passphrase
      let
        endpoint =
          "v2/wallets"
            </> (w ^. walletId)
  ...
  it "WALLETS_UPDATE_PASS_03 - Old passphrase incorrect" $ \ctx -> runResourceT $ do
    ...
    let
      payload = updatePassPayload "incorrect-passphrase" "whatever-pass"
    rup <-
      request @ApiWallet
        ctx
        (Link.putWalletPassphrase @'Shelley w)
        Default
        payload
  ...
  it "WALLETS_UPDATE_PASS_03 - Mnemonic incorrect" $ \ctx -> runResourceT $ do
    ...
    let
      payload = updatePassPayloadMnemonic otherMnemonic "whatever-pass"
    rup <-
      request @ApiWallet
        ctx
        (Link.putWalletPassphrase @'Shelley w)
  ...
  describe
    "WALLETS_UPDATE_PASS_03 - Can update pass from pass that's boundary\
    \ value"
    $ do
      let
        minLength = passphraseMinLength (Proxy @"user")
      ...
      forM_ matrix $ \(title, oldPass) -> it title $ \ctx -> runResourceT $ do
        ...
        let
          len = passphraseMaxLength (Proxy @"user")
        let
          newPass = T.pack $ replicate len '💘'
        let
          payload = updatePassPayload oldPass newPass
        rup <-
          request @ApiWallet
            ctx
            (Link.putWalletPassphrase @'Shelley w)
        ...
        expectResponseCode HTTP.status204 rup
        let
          payloadMnemonic = updatePassPayloadMnemonic m24 newPass
        rupMnemonic <-
          request @ApiWallet
            ctx
            (Link.putWalletPassphrase @'Shelley w)
  ...
  it "WALLETS_UPDATE_PASS_04 - Deleted wallet is not available"
    $ \ctx -> runResourceT $ do
      w <- emptyWallet ctx
      let
        payload = updatePassPayload fixturePassphrase "Secure passphrase2"
      let
        walId = w ^. walletId
      let
        delEndp = "v2/wallets" </> walId
      _ <- request @ApiWallet ctx ("DELETE", delEndp) Default Empty
      let
        updEndp = delEndp </> ("passphrase" :: Text)
  ...
  it "WALLETS_UPDATE_PASS_04 - Deleted wallet is not available, mnemonic"
    $ \ctx -> runResourceT $ do
      (w, mnemonic) <- emptyWalletAndMnemonic ctx
      let
        payload = updatePassPayloadMnemonic mnemonic "Secure passphrase2"
      let
        walId = w ^. walletId
      let
        delEndp = "v2/wallets" </> walId
    ...
    $ do
      ...
      forM_ matrix $ \(title, pass, expectations) -> it title
        $ \ctx -> runResourceT $ do
          ...
          let
            payloadUpdate = updatePassPayload oldPass newPass
          rup <-
            request @ApiWallet
              ctx
      ...
      forM_ matrix $ \(title, pass, expectations) -> it title
        $ \ctx -> runResourceT $ do
          ...
          wDest <- emptyWallet ctx
          let
            payloadUpdate = updatePassPayloadMnemonic mnemonic newPass
          rup <-
            request @ApiWallet
              ctx
              (Link.putWalletPassphrase @'Shelley wSrc)
  ...
  describe "WALLETS_UPDATE_PASS_07 - HTTP headers" $ do
    ...
    forM_ matrix $ \(title, headers, expectations) -> it title $ \ctx -> runResourceT $ do
      ...
      let
        payload = updatePassPayload fixturePassphrase "Passphrase"
      let
        endpoint = Link.putWalletPassphrase @'Shelley w
    ...
    $ \ctx -> runResourceT $ do
      ...
      rSnap <-
        request @ApiWalletUtxoSnapshot
          ctx
          (Link.getWalletUtxoSnapshot @'Shelley w)
          Default
          Empty
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      expectResponseCode HTTP.status200 rSnap
      let
        entries = getFromResponse #entries rSnap
      length entries `shouldBe` 10

  it
    "WALLET_UTXO_SNAPSHOT_03 - \
  ...
  it "WALLETS_UTXO_02 - Sending and receiving funds updates wallet's utxo." $ \ctx -> runResourceT $ do
    ...
    rStat1 <-
      request @ApiUtxoStatistics
        ctx
        (Link.getUTxOsStatistics @'Shelley wDest)
        Default
        Empty
  ...
  it "WALLETS_GET_KEY_01 - golden tests for verification key" $ \ctx -> runResourceT $ do
    ...
    let
      matrix :: [(Role, DerivationIndex, String)]
      matrix =
        [
          ( UtxoExternal
          , DerivationIndex 0
    ...
    verify r [expectResponseCode HTTP.status201]
    let
      apiWal = getFromResponse id r

    forM_ matrix $ \(role_, index, expected) ->
      counterexample (show role_ <> "/" <> show index) $ do
        let
          link = Link.getWalletKey @'Shelley (apiWal ^. id) role_ index Nothing
  ...
  it "WALLETS_GET_KEY_03 - unknown wallet" $ \ctx -> runResourceT $ do
    ...
    let
      link = Link.getWalletKey @'Shelley w UtxoExternal (DerivationIndex 0) Nothing
    r <- request @ApiVerificationKeyShelley ctx link Default Empty
  ...
  it "WALLETS_SIGNATURES_01 - can verify signature" $ \ctx -> runResourceT $ do
    ...
    let
      (role_, index) = (MutableAccount, DerivationIndex 0)
    let
      payload =
        [json|
                { "passphrase": #{fixturePassphrase}
    ...
    rKey <-
      request @ApiVerificationKeyShelley
        ctx
    ...
    --
    let
      dummyChainCode = BS.replicate 32 0
    let
      sigBytes = BL.toStrict $ getFromResponse id rSig
    let
      sig = CC.xsignature sigBytes
    let
      key =
  ...
  it "WALLETS_SIGNATURES_02 - invalid index for signing key" $ \ctx -> runResourceT $ do
    ...
    let
      link = Link.signMetadata w UtxoExternal (DerivationIndex 2_147_483_648)
    r <-
      rawRequest
        ctx
        link
  ...
  it "WALLETS_SIGNATURES_03 - unknown wallet" $ \ctx -> runResourceT $ do
    ...
    let
      link = Link.signMetadata w UtxoExternal (DerivationIndex 0)
    r <-
      rawRequest
        ctx
        link
  ...
  it
    ...
    \Shelley wallets listed only via new endpoints"
    $ \ctx -> runResourceT $ do
