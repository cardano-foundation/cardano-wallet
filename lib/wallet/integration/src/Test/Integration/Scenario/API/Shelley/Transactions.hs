...
import Cardano.Wallet.Primitive.Types.TokenPolicy qualified as TokenPolicy
...
spec = describe "SHELLEY_TRANSACTIONS" $ do
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        targetAddress = (targetAddresses !! 1) ^. #id
      let
        endpoint = Link.createTransactionOld @'Shelley sourceWallet
      let
        mkRequest = request @(ApiTransaction n) ctx endpoint Default
      let
        mkPayload :: Quantity "lovelace" Natural -> Payload
      ...
      e <- counterexample "Attempt #1" $ do
        ...
        e ^. #txOutputLovelaceRequiredMinimum
      ...
      counterexample "Attempt #2" $ do
        let
          lovelaceRequested = e ^. #txOutputLovelaceRequiredMinimum
        response <- mkRequest (mkPayload lovelaceRequested)
  ...
  it "Regression ADP-626 - Filtering transactions between eras" $ do
    \ctx -> runResourceT $ do
      w <- fixtureWallet ctx
      let
        startTimeBeforeShelley = T.pack "2009-09-09T09:09:09Z"
      currTime <- liftIO getCurrentTime
      let
        endTimeAfterShelley = utcIso8601ToText currTime
      let
        link =
      ...
      eventually "Pending tx has pendingSince field" $ do
        ...
        r <-
          postTx @n
            ...
            amt
        let
          tx = getFromResponse Prelude.id r
  ...
  it "TRANS_CREATE_01x - Single Output Transaction"
    $ \ctx -> runResourceT $ do
      let
        initialAmt = 3 * minUTxOValue (_mainEra ctx)
      ...
      let
        txid = getFromResponse #id rTx
      let
        linkSrc = Link.getTransaction @'Shelley wa (ApiTxId txid)
      let
        Quantity fee = getFromResponse #fee rTx
      ...
      let
        linkDest = Link.getTransaction @'Shelley wb (ApiTxId txid)
      ...
      eventually "wa and wb balances are as expected" $ do
        ...
        expectField
          (#balance . #available)
          (`shouldBe` Quantity (initialAmt + amt))
          rb
        ...
        expectField
          (#balance . #available)
          (`shouldBe` Quantity (initialAmt - fee - amt))
          ra2
  ...
  it "TRANS_CREATE_03 - 0 balance after transaction"
    $ \ctx -> runResourceT $ do
      ...
      (_, ApiFee (Quantity feeMin) _ _ _) <-
        unsafeRequest
          ctx
  ...
  it "TRANS_CREATE_04 - Not enough money" $ \ctx -> runResourceT $ do
    let
      minUTxOValue' = minUTxOValue (_mainEra ctx)
    let
      (srcAmt, reqAmt) = (minUTxOValue', 2 * minUTxOValue')
  ...
  it "TRANS_ASSETS_CREATE_01a - Multi-asset transaction with Ada"
    $ \ctx -> runResourceT $ do
      ...
      let
        assetsSrc = wal ^. #assets . #total . #getApiT
      assetsSrc `shouldNotBe` mempty
      let
        val = minUTxOValue (_mainEra ctx) <$ pickAnAsset assetsSrc
      ...
      rFee <-
        request @ApiFee
          ...
          payloadFee
      let
        [Quantity minCoin] = getFromResponse #minimumCoins rFee
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        coin = minUTxOValue (_mainEra ctx)
  ...
  let
    hasAssetOutputs :: [AddressAmount (ApiAddress n)] -> Bool
  ...
  it "TRANS_ASSETS_CREATE_02b - Multi-asset tx history"
    $ \ctx -> runResourceT $ do
      ...
      eventually "asset transfer is confirmed in transaction list" $ do
        -- on src wallet
        let
          linkSrcList = Link.listTransactions @'Shelley wSrc
        ...
        -- on dst wallet
        let
          linkDestList = Link.listTransactions @'Shelley wDest
  ...
  it "TRANS_ASSETS_LIST_01 - Asset list present"
    $ \ctx -> runResourceT $ do
      ...
      let
        tokenFingerprint = mkTokenFingerprint polId assName
  ...
  it "TRANS_ASSETS_GET_01 - Asset list present"
    $ \ctx -> runResourceT $ do
      ...
      let
        tokenFingerprint = mkTokenFingerprint polId assName
      let
        ep = Link.getAsset wal polId assName
  ...
  it "TRANS_ASSETS_GET_02 - Asset not present when isn't associated"
    $ \ctx -> runResourceT $ do
      wal <- fixtureMultiAssetWallet ctx
      let
        polId = TokenPolicy.UnsafeTokenPolicyId $ Hash $ BS.replicate 28 0
      let
        assName = TokenPolicy.UnsafeTokenName $ B8.replicate 4 'x'
      let
        ep = Link.getAsset wal polId assName
      r <- request @(ApiAsset) ctx ep Default Empty
      expectResponseCode HTTP.status404 r
  ...
  it "TRANS_ASSETS_GET_02a - Asset not present when isn't associated"
    $ \ctx -> runResourceT $ do
      ...
      let
        polId = TokenPolicy.UnsafeTokenPolicyId $ Hash $ BS.replicate 28 0
      let
        ep = Link.getAsset wal polId TokenPolicy.nullTokenName
      r <- request @(ApiAsset) ctx ep Default Empty
      expectResponseCode HTTP.status404 r
      expectErrorMessage errMsg404NoAsset r
  ...
  it "TRANS_TTL_04 - Large TTL"
    $ \ctx -> runResourceT $ do
      ...
      let
        amt = minUTxOValue (_mainEra ctx) :: Natural
      let
        hugeTTL = 1e9 :: NominalDiffTime

      basePayload <- mkTxPayload ctx wb amt fixturePassphrase
      let
        payload = addTxTTL (realToFrac hugeTTL) basePayload
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        txMeta = [json|{ "1": { "string": #{T.replicate 65 "a"} } }|]
      let
        payload = addTxMetadata txMeta basePayload
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        txMeta = [json|{ "1": #{T.replicate 65 "a"} }|]
      let
        payload = addTxMetadata txMeta basePayload
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        txMeta = [json|{ "1": { "string": "hello" } }|]
      let
        payloadWithMetadata = addTxMetadata txMeta payload
      ...
      let
        (Quantity feeEstMin) = getFromResponse #estimatedMin ra
      let
        (Quantity feeEstMax) = getFromResponse #estimatedMax ra
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        totalBalance = wSrc ^. #balance . #total
      let
        payload =
          Json
            [json|{
  ...
  it "TRANS_LIST_01 - Can list Incoming and Outgoing transactions"
    $ \ctx -> runResourceT $ do
      ...
      let
        link = Link.listTransactions @'Shelley wSrc
      r <- request @([ApiTransaction n]) ctx link Default Empty
      expectResponseCode HTTP.status200 r
  ...
  it "TRANS_LIST_02,03x - Can limit/order results with start, end and order"
    $ \ctx -> runResourceT $ do
      let
        minUTxOValue' = minUTxOValue (_mainEra ctx)
      let
        a1 = Quantity minUTxOValue'
      let
        a2 = Quantity (2 * minUTxOValue')
      ...
      liftIO $ forM_ matrix $ \tc -> do
        let
          link = withQuery (query tc) $ Link.listTransactions @'Shelley w
  ...
  it "TRANS_GET_02 - Deleted wallet" $ \ctx -> runResourceT $ do
    ...
    let
      link = Link.getTransaction @'Shelley w (ApiTxId txid)
    r <- request @(ApiTransaction n) ctx link Default Empty
    expectResponseCode HTTP.status404 r
    expectErrorMessage (errMsg404NoWallet $ w ^. walletId) r
  ...
  it "TRANS_GET_03 - Using wrong transaction id" $ \ctx -> runResourceT $ do
    ...
    let
      txid = Hash $ BS.pack $ replicate 32 1
    let
      link =
        Link.getTransaction @'Shelley
          wSrc
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        assertSourceTx = do
          let
            ep = Link.listTransactions @'Shelley wSrc
      ...
      eventually "transaction eventually is in target wallet" $ do
        let
          ep = Link.listTransactions @'Shelley wDest
        request @[ApiTransaction n] ctx ep Default Empty
          >>= flip
            verify
            [ expectListField
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        ep = Link.deleteTransaction @'Shelley wSrc (ApiTxId txid)
      rDel <- request @ApiTxId ctx ep Default Empty
      expectResponseCode HTTP.status403 rDel
      let
        err = errMsg403AlreadyInLedger (toUrlPiece (ApiTxId txid))
  ...
  it "SHELLEY_TX_REDEEM_02 - Can redeem rewards from other"
    $ \ctx -> runResourceT $ do
      ...
      let
        tid = getFromResponse Prelude.id rTx
  ...
  it "SHELLEY_TX_REDEEM_06 - Can't redeem rewards using byron wallet"
    $ \ctx -> runResourceT $ do
      ...
      let
        addr = encodeAddress (sNetworkId @n) (head addrs)
  ...
  it "SHELLEY_TX_REDEEM_06b - Can't redeem rewards if utxo = 0 from self"
    $ \ctx -> runResourceT $ do
      ...
      let
        ep = Link.migrateWallet @'Shelley wRewards
  ...
  it "SHELLEY_TX_REDEEM_07a - Can't redeem rewards if cannot cover fee"
    $ \ctx -> runResourceT $ do
      ...
      addr : _ <- fmap (view #id) <$> listAddresses @n ctx wSelf
      let
        amt = oneThousandAda + oneMillionAda
  ...
  it "SHELLEY_TX_REDEEM_07b - Can't redeem rewards if not enough money"
    $ \ctx -> runResourceT $ do
      ...
      addr : _ <- fmap (view #id) <$> listAddresses @n ctx wSelf
      let
        amt = oneThousandAda + oneMillionAda + oneAda
  ...
  where
      ...
      in
        it testName $ \ctx -> runResourceT $ do
          ...
          let
            minUTxOValue' = minUTxOValue (_mainEra ctx)
          let
            outputQuantities = txOutputAdaQuantities minUTxOValue'
          let
            paymentCount = length outputQuantities
          targetAddresses <-
            take paymentCount
              . fmap (view #id)
              <$> listAddresses @n ctx wb
          let
            targetAssets = repeat mempty
          let
            payments =
              NE.fromList
                $ map ($ mempty)
          ...
          let
            apiCoinSelection = getFromResponse Prelude.id coinSelectionResponse
          let
            fee = computeApiCoinSelectionFee apiCoinSelection
          let
            withinToleranceTo (Quantity expected) tol =
          ...
          eventually "metadata is confirmed in transaction list" $ do
            -- on src wallet
            let
              linkSrcList = Link.listTransactions @'Shelley wa
            rla <- request @([ApiTransaction n]) ctx linkSrcList Default Empty
            verify
              rla
              [ expectResponseCode HTTP.status200
            ...
            -- on dst wallet
            let
              linkDstList = Link.listTransactions @'Shelley wb
          ...
          eventually "metadata is confirmed in transaction get" $ do
            ...
            -- on dst wallet
            let
              linkDst = Link.getTransaction @'Shelley wb (ApiTxId txid)
    ...
    txDeleteNotExistsingTxIdTest eWallet resource =
      it resource $ \ctx -> runResourceT $ do
        ...
        let
          walId = w ^. walletId
        let
          txid = "3e6ec12da4414aa0781ff8afa9717ae53ee8cb4aa55d622f65bc62619a4f7b12"
        let
          endpoint =
            "v2/"
              <> T.pack resource
    ...
    txDeleteFromDifferentWalletTest eWallet resource =
      it resource $ \ctx -> runResourceT $ do
        ...
        let
          txid = toText $ getApiT $ getFromResponse #id rMkTx
        let
          endpoint =
            "v2/"
              <> T.pack resource
