...
spec = describe "SHARED_TRANSACTIONS" $ do
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        walShared@(ApiSharedWallet (Right wal)) =
          getFromResponse Prelude.id rPost
      ...
      let
        amt = 10 * minUTxOValue (_mainEra ctx)
      ...
      let
        txCbor1 = getFromResponse #transaction rTx2
      let
        decodePayload1 = Json (toJSON txCbor1)
      rDecodedTx1 <-
        request @(ApiDecodedTransaction n)
          ...
          decodePayload1
      let
        expectedFee = getFromResponse (#fee . #getQuantity) rTx2
      let
        metadata' =
      ...
      let
        decodePayload2 = Json (toJSON signedTx)
      rDecodedTx2 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared wal)
      ...
      let
        txid = getFromResponse #id submittedTx
      let
        queryTx = Link.getTransaction @'Shared wal (ApiTxId txid)
      ...
      eventually "Tx is in ledger finally" $ do
        ...
        let
          listTxEp = Link.listTransactions @'Shared wal
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        metadata = Json [json|{ "metadata": { "1": "hello"  } }|]
      ...
      let
        txCbor1 = getFromResponse #transaction rTx
      let
        decodePayload1 = Json (toJSON txCbor1)
      rDecodedTx1 <-
        request @(ApiDecodedTransaction n)
      ...
      -- tx
      let
        getMetadata (Cardano.InAnyCardanoEra _ tx) =
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_04a - \
    \Single Output Transaction with decode transaction - single party"
    $ \ctx -> runResourceT $ do
      wa <- fixtureSharedWallet @n ctx
      wb <- emptyWallet ctx
      let
        amt = (minUTxOValue (_mainEra ctx) :: Natural)
      ...
      let
        txCbor = getFromResponse #transaction rTx
      let
        decodePayload = Json (toJSON txCbor)
      rDecodedTxSource <-
        request @(ApiDecodedTransaction n)
          ctx
      ...
      let
        isInpOurs inp = case inp of
          ExternalInput _ -> False
          WalletInput _ -> True
      let
        areOurs = all isInpOurs
      addrs <- listAddresses @n ctx wb
      let
        addrIx = 1
      let
        addrDest = (addrs !! addrIx) ^. #id
      let
        expectedTxOutTarget =
          WalletOutput
            $ ApiWalletOutput
      ...
      let
        decodePayload1 = Json (toJSON signedTx)
      rDecodedTxSource1 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared wa)
      ...
      let
        walShared = ApiSharedWallet (Right wa)
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_04b - \
    \Single Output Transaction Shared -> Shared"
    $ \ctx -> runResourceT $ do
      wa <- fixtureSharedWallet @n ctx
      let
        srcWallet = ApiSharedWallet (Right wa)
      ...
      rAddr <-
        request @[ApiAddressWithPath n]
          ctx
          (Link.listAddresses @'Shared wb)
          Default
          Empty
      let
        addrs = getFromResponse Prelude.id rAddr
      let
      ...
      let
        txid = getFromResponse #id submittedTx
      let
        linkSrc = Link.getTransaction @'Shared wa (ApiTxId txid)
      eventually "Source wallet balance is decreased by amt + fee" $ do
      ...
      let
        linkDest = Link.getTransaction @'Shared wb (ApiTxId txid)
      eventually "Target wallet balance is increased by amt" $ do
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_04c - \
    \Cannot spend less than minUTxOValue"
    $ \ctx -> runResourceT $ do
      wa <- fixtureSharedWallet @n ctx
      wb <- emptyWallet ctx
      let
        amt = minUTxOValue (_mainEra ctx) - 1
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_04d - \
    \Can't cover fee"
    $ \ctx -> runResourceT $ do
      wa <- fixtureSharedWallet @n ctx
      wb <- emptyWallet ctx
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_04e - \
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        amt = minUTxOValue (_mainEra ctx) :: Natural
      let
        destination1 = (addrs !! 1) ^. #id
      let
        destination2 = (addrs !! 2) ^. #id
      let
        payload =
          Json
            [json|{
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_05a - \
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_05b - \
    \Single Output Transaction with decode transaction - multi party"
    $ \ctx -> runResourceT $ do
  ...
  it
    "SHARED_TRANSACTIONS_CREATE_05c - \
    \Single Output Transaction with decode transaction - multi party"
    $ \ctx -> runResourceT $ do
      ...
      rTx <-
        request @(ApiConstructTransaction n)
          ctx
          (Link.createUnsignedTransaction @'Shared sharedWal1)
          Default
          payload
      verify
        rTx
      ...
      let
        txCbor = getFromResponse #transaction rTx
      let
        decodePayload1 = Json (toJSON txCbor)
      rDecodedTx1Wal1 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared sharedWal1)
          Default
          decodePayload1
      rDecodedTx1Wal2 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared sharedWal2)
          Default
          decodePayload1
      rDecodedTx1Wal3 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared sharedWal3)
          Default
          decodePayload1
      rDecodedTx1Target <-
        request @(ApiDecodedTransaction n)
      ...
      let
        expectedFee = getFromResponse (#fee . #getQuantity) rTx
      let
        (ApiScriptTemplate scriptTemplate) =
      ...
      -- adding one witness
      let
        (ApiSerialisedTransaction apiTx1 _) =
          getFromResponse #transaction rTx
      signedTx1 <-
        signSharedTx
          ctx
          ...
          [expectResponseCode HTTP.status202]
      let
        decodePayload2 = Json (toJSON signedTx1)
      rDecodedTx2Wal1 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared sharedWal1)
      ...
      -- submission
      let
        (ApiSerialisedTransaction apiTx2 _) = signedTx1
      signedTx3 <-
        signSharedTx
          ctx
          sharedWal2
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        bal1 = wal1 ^. (#balance . #available)
      let
        bal2 = wal2 ^. (#balance . #available)
      bal1 `shouldBe` bal2
      ...
      verify
        (fmap (swapEither . view #wallet) <$> rPost1)
        [ expectResponseCode HTTP.status201
        ]
      let
        walPending1 = getFromResponse Prelude.id rPost1
      let
        payloadPatch1 =
          Json
            [json| {
      ...
      let
        walShared1 = getFromResponse Prelude.id rPatch1

      rPost2 <- postSharedWallet ctx Default payload2
      verify
        (fmap (swapEither . view #wallet) <$> rPost2)
        [ expectResponseCode HTTP.status201
        ]
      let
        walPending2 = getFromResponse Prelude.id rPost2
      let
        payloadPatch2 =
          Json
            [json| {
      ...
      let
        walShared2 = getFromResponse Prelude.id rPatch2
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        link = Link.listTransactions @'Shared wSrc
  ...
  it
    "SHARED_TRANSACTIONS_LIST_02,03x -\
    \Can limit/order results with start, end and order"
    $ \ctx -> runResourceT $ do
      let
        amt1 = minUTxOValue (_mainEra ctx)
      let
        amt2 = 2 * amt1
      ...
      rAddr <-
        request @[ApiAddressWithPath n]
          ctx
          (Link.listAddresses @'Shared walDest)
          Default
          Empty
      ...
      let
        addrs = getFromResponse Prelude.id rAddr
      let
        destAddr1 = (head addrs) ^. #id
      let
        destAddr2 = (addrs !! 1) ^. #id
      let
        payload destination amt =
          Json
            [json|{
                "payments": [{
                    "address": #{destination},
      ...
      let
        matrix :: [TestCase [ApiTransaction n]] =
          [ TestCase -- 1
              { query =
                  toQueryString
      ...
      let
        withQuery q (method, link) = (method, link <> q)
  ...
  describe
    "SHARED_TRANSACTIONS_LIST_02,03 - \
    \Faulty start, end, order values"
    $ do
      let
        orderErr =
      ...
      let
        queries :: [TestCase [ApiTransaction n]] =
          [ TestCase
  ...
  it
    "SHARED_TRANSACTIONS_LIST_02 - \
    \Start time shouldn't be later than end time"
    $ \ctx -> runResourceT $ do
      (ApiSharedWallet (Right w)) <- emptySharedWallet ctx
      let
        startTime = "2009-09-09T09:09:09Z"
      let
        endTime = "2001-01-01T01:01:01Z"
      let
        link =
          Link.listTransactions' @'Shared
            w
            Nothing
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      expectResponseCode HTTP.status200 r
      let
        txs = getFromResponse Prelude.id r
      txs `shouldBe` []

  it
    "SHARED_TRANSACTIONS_LIST_04 - \
    \Deleted wallet"
    $ \ctx -> runResourceT $ do
      (ApiSharedWallet (Right w)) <- emptySharedWallet ctx
      _ <-
  ...
  it
    "SHARED_TRANSACTIONS_LIST_RANGE_01 - \
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        (te, tl) = (utcTimePred t, utcTimeSucc t)
      txs1 <-
  ...
  it
    "SHARED_TRANSACTIONS_LIST_RANGE_02 - \
  ...
  it
    "SHARED_TRANSACTIONS_LIST_RANGE_03 - \
    \Transaction at time t is NOT selected by range (..., t - 𝛿t)"
    $ \ctx -> runResourceT $ do
      w <- fixtureSharedWalletWith ctx (minUTxOValue (_mainEra ctx))
      t <- unsafeGetTransactionTime =<< listAllSharedTransactions ctx w
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        txid = getFromResponse #id submittedTx
      let
        queryTx = Link.getTransaction @'Shared wSrc (ApiTxId txid)
      rGetTx <- request @(ApiTransaction n) ctx queryTx Default Empty
      verify
        rGetTx
        [ expectSuccess
    ...
    $ \ctx -> runResourceT $ do
      ...
      _ <-
        request @ApiWallet
          ...
          Empty
      let
        txid = ApiT $ Hash $ BS.pack $ replicate 32 1
      let
        link = Link.getTransaction @'Shared w (ApiTxId txid)
      r <- request @(ApiTransaction n) ctx link Default Empty
      expectResponseCode HTTP.status404 r
      expectErrorMessage (errMsg404NoWallet $ w ^. walletId) r

  it
    "SHARED_TRANSACTIONS_GET_03 - \
    ...
    $ \ctx -> runResourceT $ do
      ...
      submittedTx <- submitSharedTxWithWid ctx wSrc signedTx
      let
        txid1 = getFromResponse #id submittedTx
      let
        queryTx = Link.getTransaction @'Shared wSrc (ApiTxId txid1)
      rGetTx <- request @(ApiTransaction n) ctx queryTx Default Empty
      verify
        rGetTx
        [ expectSuccess
      ...
      let
        txid2 = Hash $ BS.pack $ replicate 32 1
      let
        link = Link.getTransaction @'Shared wSrc (ApiTxId $ ApiT txid2)
      r <- request @(ApiTransaction n) ctx link Default Empty
      expectResponseCode HTTP.status404 r
      expectErrorMessage (errMsg404CannotFindTx $ toText txid2) r

  it
    "SHARED_TRANSACTIONS_DELEGATION_01 - \
    \Cannot delegate when wallet is missing a delegation script template"
    $ \ctx -> runResourceT $ do
      (ApiSharedWallet (Right w)) <- emptySharedWallet ctx

      pool1 : _ <-
        map (view $ _Unwrapped . #id) . snd
      ...
      rTx <-
        request @(ApiConstructTransaction n)
          ctx
          (Link.createUnsignedTransaction @'Shared w)
          Default
          delegationJoin
  ...
  it
    "SHARED_TRANSACTIONS_DELEGATION_01a - \
    \Can join stakepool, rejoin another and quit"
    $ \ctx -> runResourceT $ do
      (party1, party2) <- fixtureSharedWalletDelegating @n ctx
      let
        depositAmt = Quantity 1_000_000
      ...
      rTx1 <-
        request @(ApiConstructTransaction n)
          ctx
          (Link.createUnsignedTransaction @'Shared party1)
          Default
          delegationJoin
      verify
        rTx1
      ...
      let
        txCbor1 = getFromResponse #transaction rTx1
      let
        decodePayload1 = Json (toJSON txCbor1)
      rDecodedTx1 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared party1)
          Default
          decodePayload1
      rDecodedTx2 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared party2)
          Default
          decodePayload1
      let
        expectedFee1 = getFromResponse (#fee . #getQuantity) rTx1
      let
        decodedExpectations1 =
          [ expectResponseCode HTTP.status202
      ...
      let
        (ApiScriptTemplate pScriptTemplate) =
        ...
        paymentScript =
          ApiT
            $ NativeExplicitScript
              ...
              ViaSpending
      let
        (Just (ApiScriptTemplate dScriptTemplate)) =
      ...
      let
        certExpectation1 =
          [ expectField
              #certificates
              ( `shouldBe`
                  [ registerStakeKeyCert stakeKeyDerPathParty1
      ...
      let
        certExpectation2 =
          [ expectField
              #certificates
              ( `shouldBe`
                  [ registerStakeKeyCert stakeKeyDerPathParty2
      ...
      let
        decodePayload3 = Json (toJSON signedTx2)
      rDecodedTx5 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared party1)
      ...
      let
        txId2 = getFromResponse #id submittedTx2
      let
        link party = Link.getTransaction @'Shared party (ApiTxId txId2)
      ...
      let
        (ApiSerialisedTransaction apiTx3 _) =
          getFromResponse #transaction rTx2
      signedTx3 <-
        signSharedTx
          ctx
          ...
          [expectResponseCode HTTP.status202]
      let
        (ApiSerialisedTransaction apiTx4 _) = signedTx3
      signedTx4 <-
        signSharedTx
          ctx
          party2
      ...
      let
        delegatingCert2 path =
          WalletDelegationCertificate $ JoinPool path (ApiT pool2)

      let
        expectedFee2 = getFromResponse (#fee . #getQuantity) rTx2
      let
        decodedExpectations2 =
          [ expectResponseCode HTTP.status202
      ...
      let
        decodePayload4 = Json (toJSON signedTx4)
      ...
      let
        coin = minUTxOValue (_mainEra ctx) :: Natural
      let
        addr = (addrs !! 1) ^. #id
      let
        payloadWithdrawal =
      ...
      rGet <-
        request @ApiWallet
          ctx
          (Link.getWallet @'Shared party1)
          Default
          Empty
      let
        previousBalance = getFromResponse (#balance . #available) rGet
      ...
      let
        (ApiSerialisedTransaction apiTx5 _) =
      ...
      signedTx5 <-
        signSharedTx
          ...
          [expectResponseCode HTTP.status202]
      let
        (ApiSerialisedTransaction apiTx6 _) = signedTx5
      signedTx6 <-
        signSharedTx
          ctx
          party2
      ...
      let
        (ApiSerialisedTransaction apiTx7 _) =
      ...
      signedTx7 <-
        signSharedTx
          ...
          [expectResponseCode HTTP.status202]
      let
        (ApiSerialisedTransaction apiTx8 _) = signedTx7
      signedTx8 <-
        signSharedTx
          ctx
          party2
      ...
      let
        delegatingCert3 path =
          WalletDelegationCertificate $ QuitPool path
      let
        decodePayload5 = Json (toJSON signedTx8)
      rDecodedTx9 <-
        request @(ApiDecodedTransaction n)
          ctx
          (Link.decodeTransaction @'Shared party1)
  ...
  it
    "SHARED_TRANSACTIONS_DELEGATION_02  \
    \Emulating multi-delegation using shared wallets"
    $ \ctx -> runResourceT $ do
      liftIO
        $ pendingWith "TODO: Fix multi-delegation with shared wallets flakiness, ADP-3084"
      ...
      verify
        ...
        [expectResponseCode HTTP.status201]
      let
        parentWal = getFromResponse Prelude.id rPostCreateParent

      -- financing the parent Shelley wallet
      rAddrShelley <-
        request @[ApiAddressWithPath n]
          ctx
          (Link.listAddresses @'Shelley parentWal)
          Default
          Empty
      expectResponseCode HTTP.status200 rAddrShelley
      let
        addrs = getFromResponse Prelude.id rAddrShelley
      let
        destination = (addrs !! 1) ^. #id
      ...
      let
        ep = Link.createTransactionOld @'Shelley
      ...
      eventually "Parent Shelley Wallet balance is as expected" $ do
        rGet <-
          request @ApiWallet
            ctx
            (Link.getWallet @'Shelley parentWal)
            Default
            Empty
        verify
          rGet
      ...
      let
        payloadCreateChild :: T.Text -> T.Text -> Payload
        payloadCreateChild ix name =
          Json
            [json| {
                "name": #{name},
                "mnemonic_sentence": #{m15},
      ...
      verify
        (fmap (view #wallet) <$> rPostCreateChild1)
        [ expectResponseCode HTTP.status201
        ]
      let
        sharedWal1 = getFromResponse Prelude.id rPostCreateChild1
      let
        (ApiSharedWallet (Right walActive1)) = sharedWal1
      ...
      let
        sharedWal2 = getFromResponse Prelude.id rPostCreateChild2
      let
        (ApiSharedWallet (Right walActive2)) = sharedWal2
      verify
        (fmap (view #wallet) <$> rPostCreateChild2)
        [ expectResponseCode HTTP.status201
        ]
      ...
      rAddrShared1 <-
        request @[ApiAddressWithPath n]
          ctx
          (Link.listAddresses @'Shared walActive1)
          Default
          Empty
      expectResponseCode HTTP.status200 rAddrShared1
      let
        addrs1 = getFromResponse Prelude.id rAddrShared1
      let
        destAddr1 = (addrs1 !! 1) ^. #id

      rAddrShared2 <-
        request @[ApiAddressWithPath n]
          ctx
          (Link.listAddresses @'Shared walActive2)
          Default
          Empty
      expectResponseCode HTTP.status200 rAddrShared2
      let
        addrs2 = getFromResponse Prelude.id rAddrShared2
      let
        destAddr2 = (addrs2 !! 1) ^. #id
      ...
      eventually "Child shared wallet 1 balance is increased by target" $ do
        rGet <-
          request @ApiWallet
            ctx
            (Link.getWallet @'Shared walActive1)
            Default
            Empty
        verify
          rGet
      ...
      eventually "Child shared wallet 2 balance is increased by target" $ do
        rGet <-
          request @ApiWallet
            ctx
            (Link.getWallet @'Shared walActive2)
            Default
            Empty
        verify
          rGet
      ...
      rTx3 <-
        request @(ApiConstructTransaction n)
          ctx
          (Link.createUnsignedTransaction @'Shared walActive1)
          Default
      ...
      rTx4 <-
        request @(ApiConstructTransaction n)
          ctx
          (Link.createUnsignedTransaction @'Shared walActive2)
          Default
      ...
      verify
        ...
        [expectResponseCode HTTP.status202]
      let
        (ApiSerialisedTransaction apiTx4 _) =
          getFromResponse #transaction rTx4
      signedTx4 <-
        signSharedTx
          ctx
      ...
      rGet1 <-
        request @ApiWallet
          ...
          Empty
      let
        (Quantity rewards1) = getFromResponse (#balance . #reward) rGet1
      rGet2 <-
        request @ApiWallet
          ctx
          (Link.getWallet @'Shared walActive2)
          ...
          Empty
      let
        (Quantity rewards2) = getFromResponse (#balance . #reward) rGet2

      let
        payloadWithdrawal amt =
          Json
            [json|
                { "payments":
                    [ { "address": #{destination}
      ...
      verify
        ...
        [expectResponseCode HTTP.status202]
      let
        expectedFee = getFromResponse (#fee . #getQuantity) rTx5a
      ...
      verify
        ...
        [expectResponseCode HTTP.status202]
      let
        (ApiSerialisedTransaction apiTx6 _) =
  ...
  it "SHARED_TRANSACTIONS_LIST_05 - filter address output side"
    $ \ctx -> runResourceT $ do
      let
        minUTxOValue' = minUTxOValue (_mainEra ctx)
      let
        a1 = minUTxOValue'
      let
        a2 = 2 * minUTxOValue'
      ...
      eventually "There are exactly 6 transactions for wDest" $ do
        let
          linkList = listTransactionsFilteredByAddress wDest Nothing
        rl <- request @([ApiTransaction n]) ctx linkList Default Empty
      ...
      let
        addr0 = (head addrs) ^. #id
      let
        linkList0 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr0))
      rl0 <- request @([ApiTransaction n]) ctx linkList0 Default Empty
      verify rl0 [expectListSize 2]
      let
        txs0 = getFromResponse Prelude.id rl0
      let
        amts0 = fmap (view #amount) txs0
      Set.fromList amts0 `shouldBe` Set.fromList (Quantity <$> [a1, a2])

      let
        addr1 = (addrs !! 1) ^. #id
      let
        linkList1 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr1))
      rl1 <- request @([ApiTransaction n]) ctx linkList1 Default Empty
      verify rl1 [expectListSize 1]
      let
        txs1 = getFromResponse Prelude.id rl1
      let
        amts1 = fmap (view #amount) txs1
      amts1 `shouldBe` (Quantity <$> [a3])

      let
        addr2 = (addrs !! 2) ^. #id
      let
        linkList2 = listTransactionsFilteredByAddress wDest (Just (apiAddress addr2))
      rl2 <- request @([ApiTransaction n]) ctx linkList2 Default Empty
      verify rl2 [expectListSize 3]
      let
        txs2 = getFromResponse Prelude.id rl2
      let
        amts2 = fmap (view #amount) txs2
  ...
  it "SHARED_TRANSACTIONS_LIST_06 - filter address input side" $ \ctx -> runResourceT $ do
    ...
    let
      a1 = minUTxOValue'
    let
      a2 = fromIntegral $ oneAda * 5_000
    let
      a3 = fromIntegral $ oneAda * 10_000
    (wSrc, ApiSharedWallet (Right wDest)) <-
      (,)
    ...
    let
      a4 = fromIntegral $ oneAda * 29_990
    let
      addr2 = (addrs !! 2) ^. #id
    let
      linkList2a = listTransactionsFilteredByAddress wDest (Just (apiAddress addr2))
    rl2a <- request @([ApiTransaction n]) ctx linkList2a Default Empty
    verify rl2a [expectListSize 3]
    let
      txs2a = getFromResponse Prelude.id rl2a
    let
      amts2a = fmap (view #amount) txs2a
    ...
    let
      addr1 = (addrs !! 1) ^. #id
    let
      linkList1a = listTransactionsFilteredByAddress wDest (Just (apiAddress addr1))
  ...
  where
    ...
    listExternalAddresses ctx w = do
      let
        link = Link.listAddresses @'Shared w
    ...
    sendAmtToAddr ctx src dest amt addrIx = do
      ...
      rTx <-
        request @(ApiConstructTransaction n)
          ctx
          (Link.createUnsignedTransaction @'Shared src)
          Default
          payload
      verify
        rTx
      ...
      let
        txid = getFromResponse #id submittedTx
      let
        queryTx w = Link.getTransaction @'Shared w (ApiTxId txid)
    ...
    fixtureTwoPartySharedWallet ctx = do
      ...
      let
        (Right m15A) = mkSomeMnemonic @'[15] m15txtA
      let
        (Right m12A) = mkSomeMnemonic @'[12] m12txtA
      let
        accXPubDerivedA =
      ...
      let
        (Right m15B) = mkSomeMnemonic @'[15] m15txtB
      let
        (Right m12B) = mkSomeMnemonic @'[12] m12txtB
      let
        accXPubDerivedB =
      ...
      verify
        (fmap (swapEither . view #wallet) <$> rPostA)
        [ expectResponseCode HTTP.status201
        ]
      let
        walShared1@(ApiSharedWallet (Right walA)) =
          getFromResponse Prelude.id rPostA

      rPostB <- postSharedWallet ctx Default (payload m15txtB m12txtB)
      verify
        (fmap (swapEither . view #wallet) <$> rPostB)
        [ expectResponseCode HTTP.status201
        ]
      let
        walShared2@(ApiSharedWallet (Right walB)) =
    ...
    fixtureTwoPartySharedWalletPatched ctx = do
      ...
      let
        walPendingA = getFromResponse Prelude.id rPostA
      let
        payloadPatchA =
          Json
            [json| {
      ...
      let
        walPendingB = getFromResponse Prelude.id rPostB
      let
        payloadPatchB =
          Json
            [json| {
    ...
    fixtureThreePartySharedWallet ctx = do
      ...
      let
        (Right m15C) = mkSomeMnemonic @'[15] m15txtC
      let
        (Right m12C) = mkSomeMnemonic @'[12] m12txtC
      let
        accXPubDerivedC =
      ...
      verify
        (fmap (swapEither . view #wallet) <$> rPostC)
        [ expectResponseCode HTTP.status201
        ]
      let
        walShared3@(ApiSharedWallet (Right walC)) =
        ...
        let
          changeRole' = \case
            ...
            ActiveUntilSlot s ->
              ActiveUntilSlot s
        in
          NativeExplicitScript (changeRole' script) scriptRole
