...
spec = describe "SHELLEY_COIN_SELECTION" $ do
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        payment = AddressAmount targetAddress amount mempty
      let
        output = ApiCoinSelectionOutput targetAddress amount mempty
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        amounts = Quantity <$> [minUTxOValue (_mainEra ctx) ..]
      let
        assets = repeat mempty
      let
        payments =
          NE.fromList
            $ take paymentCount
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        minUTxOValue' = Quantity . minUTxOValue $ _mainEra ctx
      let
        payments = AddressAmount addr minUTxOValue' mempty :| []
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        payments = AddressAmount addr minUTxOValue' mempty :| []
      let
        payload = Json [json| { "payments": #{payments} } |]
      let
        wid = toText $ getApiT $ w ^. #id
      let
        endpoints =
          ("POST",) . mconcat
  ...
  describe "WALLETS_COIN_SELECTION_04 - HTTP headers" $ do
    ...
    forM_ matrix $ \(title, headers, expectations) -> it title $ \ctx -> runResourceT $ do
      ...
      (addr : _) <- fmap (view #id) <$> listAddresses @n ctx w
      let
        amt = Quantity . minUTxOValue . _mainEra $ ctx
      let
        payments = AddressAmount addr amt mempty :| []
      let
        payload = Json [json| { "payments": #{payments} } |]
      r <-
  ...
  it "WALLETS_COIN_SELECTION_06a - can redeem rewards from self" $ \ctx -> runResourceT $ do
    ...
    let
      payment = AddressAmount addr amount mempty
    let
      transform = addField "withdrawal" ("self" :: String)
  ...
  it "WALLETS_COIN_SELECTION_06b - can redeem rewards from other"
    $ \ctx -> runResourceT $ do
      ...
      let
        payment = AddressAmount addr amount mempty
      let
        transform = addField "withdrawal" (mnemonicToText mnemonic)
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        adaQuantity = Quantity . minUTxOValue $ _mainEra ctx
      let
  ...
  it
    ...
    $ \ctx -> runResourceT $ do
      ...
      let
        tokenNames =
          UnsafeTokenName . T.encodeUtf8 . T.singleton
            <$> ['a' ..]
      let
        assetIds = AssetId policyId <$> take assetCount tokenNames
      let
        nonAdaQuantities =
