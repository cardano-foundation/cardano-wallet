...
payloadWith' name mnemonics gap =
    Json
        [json| {
     "name": #{name},
     "mnemonic_sentence": #{mnemonics},
...
updatePassPayload oldPass newPass =
    Json
        [json| {
...
updatePassPayloadMnemonic mnemonic newPass =
    Json
        [json| {
    "mnemonic_sentence": #{mnemonic},
    "new_passphrase": #{newPass}
...
updatePassPayloadMnemonicAndSndFactor mnemonic sndFactor newPass =
    Json
        [json| {
    "mnemonic_sentence": #{mnemonic},
...
updateEmptyPassPayload newPass =
    Json
        [json| {
    "new_passphrase": #{newPass}
      } |]
...
errMsg409WalletExists walId =
    "This operation would yield a wallet with the following\
    \ id: "
        ++ walId
...
errMsg403WrongPass =
    "The given encryption passphrase doesn't match the one\
...
errMsg400MinWithdrawalWrong =
    "The minimum withdrawal value must be at least \
    \1 Lovelace."
...
errMsg404CannotFindTx tid =
    "I couldn't find a transaction with the given id: "
        ++ unpack tid

errMsg403NoRootKey :: Text -> String
errMsg403NoRootKey wid =
    "I couldn't find a root private key for the given\
    \ wallet: "
        ++ unpack wid
...
errMsg500 :: String
errMsg500 =
    "That's embarrassing. It looks like I've created an invalid\
...
errMsgNotInDictionary =
    "Found an unknown word not present in the pre-defined\
    \ dictionary."
...
errMsg403KeyAlreadyPresent cred =
    mconcat
        [ "It looks like you've tried to add a cosigner key to a shared wallet's "
        , unpack cred
        , " template that is already ascribed to another cosigner. "
...
errMsg403TemplateInvalidScript reason =
    mconcat
        [ "It looks like you've tried to create a shared wallet with a template"
        , " script for payment credential that does not pass validation. The problem is: "
        , reason
...
errMsg403MissingWitsInTransaction expected got =
    mconcat
