...
getWalletUtxoSnapshot ctx = do
    ...
    let txOuts =
            availableUTxO @s pending wallet
                & unUTxO
                & F.toList
...
restoreWallet ctx =
    db & \DBLayer{..} ->
        ...
        in  catchFromIO $ case (maybeDiscover, lightSync nw) of
...
runLocalTxSubmissionPool cfg ctx =
  ...
  where
    ...
    tr =
        unliftIOTracer
            $ contramap (MsgWallet . MsgTxSubmit)
...
attachPrivateKeyFromPwdScheme
    ...
    -> IO ()
attachPrivateKeyFromPwdScheme _ ctx (xprv, (scheme, pwd)) =
    db & \_ -> do
