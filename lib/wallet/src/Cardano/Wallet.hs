...
import Cardano.Address.Style.Shelley qualified as CAShelley
import Cardano.Api
  ( serialiseToCBOR
  )
...
import Cardano.Wallet.DB.Store.Delegations.Layer qualified as Dlgs
...
import Cardano.Wallet.DB.Store.Submissions.Layer qualified as Submissions
...
import Cardano.Wallet.DB.WalletState qualified as WS
import Cardano.Wallet.DB.WalletState qualified as WalletState
...
import Cardano.Wallet.Primitive.Migration qualified as Migration
...
import Cardano.Wallet.Primitive.Slotting qualified as Slotting
...
import Cardano.Wallet.Primitive.Types.UTxOStatistics qualified as UTxOStatistics
...
import Cardano.Wallet.Write.Tx.SizeEstimation qualified as Write
...
import Data.Delta.Update qualified as Delta
import Data.Either
  ( partitionEithers
  )
...
checkWalletIntegrity db gp =
  db & \DBLayer {..} -> do
    gp' <-
      atomically readGenesisParameters >>= do
        maybe (throwIO ErrCheckWalletIntegrityNoGenesisParameters) pure
    when
      ( (gp ^. #getGenesisBlockHash /= gp' ^. #getGenesisBlockHash)
...
updateWalletPassphraseWithOldPassphrase wF ctx wid (old, new) =
  withRootKey db wid old ErrUpdatePassphraseWithRootKey
    $ \xprv scheme -> do
      ...
      -- always.
      let
        new' = (currentPassphraseScheme, new)
        xprv' =
          changePassphraseNew
            (keyOfWallet wF)
            (scheme, old)
...
listUtxoStatistics ctx = do
  ...
  let
    utxo = availableUTxO pending wal
...
restoreBlocks ctx tr blocks nodeTip =
  ...
  where
    ...
    isParentOf cp = (== Just parent) . parentHeaderHash
      where
        parent = headerHash $ currentTip cp
...
mkExternalWithdrawal netLayer txWitnessTag mnemonic = do
  let
    (_, rewardAccount, derivationPath) =
  ...
  let
    (xprv, _acct, _path) = someRewardAccount @ShelleyKey mnemonic
  pure
    $ checkRewardIsWorthTxCost txWitnessTag pp balance Nothing
...
checkRewardIsWorthTxCost txWitnessTag pp balance delegationTemplateM = do
  ...
  let
    costOfWithdrawal = _txRewardWithdrawalCost feePerByte witType balance
  ...
  where
    putCheckpoint cp =
      Delta.onDBVar walletState
        ...
        $ \_ ->
          let
            (prologue, wcp) = WalletState.fromWallet cp
...
signTransaction
  ...
  keyLookup
  mextraRewardAcc
  (RootCredentials rootKey rootPwd)
  utxo
  accIxForStakingM =
    let
      ...
      policyKey = afterByron key $ \key' ->
        ( hashVerificationKey key CA.Policy
            $ liftRawKey key'
            $ toXPub xprv
        , xprv
        , rootPwd
      ...
      $ \rootKey scheme -> lift $ do
        (BuiltTx {..}, slot) <- atomically $ do
          ...
          txWithSlot@(builtTx, slot) <- ( throwOnErr
            ...
            $ \s -> do
              let
                wallet = WalletState.getLatest s
...
buildAndSignTransactionPure
  ...
  txCtx = do
    ...
    let
      mExternalRewardAccount = case view #txWithdrawal txCtx of
      ...
      signedTx =
        signTransaction @k @'CredFromKeyK
...
buildTransaction
  ...
  paymentOuts = do
    ...
    atomically $ do
      ...
      pendingTxs <-
        Set.fromList . fmap fromTransactionInfo
          <$> readTransactions
            Nothing
            Descending
            wholeRange
      ...
      let
        utxo = availableUTxO @s pendingTxs wallet
...
buildAndSignTransaction ctx wid era mkRwdAcct pwd txCtx sel =
  db & \DBLayer {..} ->
    withRootKey db wid pwd ErrSignPaymentWithRootKey $ \xprv scheme -> do
      ...
      mapExceptT atomically $ do
        ...
        let
          keyFrom = isOwned wF (getState cp) (xprv, pwdP)
...
constructUnbalancedSharedTransaction txLayer db txCtx sel =
  db & \DBLayer {..} -> do
    cp <- lift $ atomically readCheckpoint
    let
      s = getState cp
      ...
      getScript addr = case fst (isShared addr s) of
        Nothing ->
          error
            $ "Some inputs selected by coin selection do not belong "
              <> "to multi-signature wallet"
        Just (ix, role) ->
          let
            template = paymentTemplate s
            role' = case role of
          ...
          in
            replaceCosignersWithVerKeys role' template ix
    sealedTx <- mapExceptT atomically $ do
      withExceptT ErrConstructTxBody
        $ ExceptT
        $ pure
...
constructTxMeta DBLayer {..} txCtx inps outs = atomically $ do
  ...
  let
    latestBlockHeader = currentTip checkpoint
...
submitExternalTx tr nw tl sealedTx = do
  ...
  let
    (tx, _, _, _, _, _) = decodeTx tl era AnyWitnessCountCtx sealedTx
  ...
  where
    ...
    getSlotRange = case (mStart, mEnd) of
      (Just start, Just end) | start > end -> do
        let
          err = ErrStartTimeLaterThanEndTime start end
        ...
        $ let
          ...
          in
            readTransactions
              ...
              Nothing
    let
      txAssets :: TransactionInfo -> Set TokenMap.AssetId
      txAssets =
        Set.unions
...
createMigrationPlan ctx rewardWithdrawal = do
  ...
  pp <- currentProtocolParameters nl
  let
    txConstraints = Write.txConstraints pp (transactionWitnessTag tl)
...
attachPrivateKeyFromPwdScheme
  ...
  -> IO ()
attachPrivateKeyFromPwdScheme ctx (xprv, (scheme, pwd)) =
  db & \_ -> do
...
withRootKey DBLayer {..} wid pwd embed action = do
  (xprv, scheme) <- withExceptT embed . ExceptT . atomically $ do
    ...
    let
      mScheme = passphraseScheme <$> passphraseInfo wMetadata
...
signMetadataWith ctx wid pwd (role_, ix) metadata =
  db & \DBLayer {..} -> do
...
writePolicyPublicKey ctx wid pwd =
  db & \DBLayer {..} -> do
    cp <- lift $ atomically readCheckpoint

    let
      (SeqPrologue seqState) = getPrologue $ getState cp
    ...
    let
      seqState' = seqState & #policyXPub .~ Just policyXPub
    lift
      $ atomically
      $ Delta.onDBVar walletState
      $ Delta.update
...
normalizeSharedAddress
  :: forall n k
   . (Shared.SupportsDiscovery n k, k ~ SharedKey)
  => SharedState n k
  -> Address
  -> Maybe Address
normalizeSharedAddress st addr = case Shared.ready st of
  ...
  Shared.Active _ -> do
    let
      dTM = Shared.delegationTemplate st
    fingerprint <- eitherToMaybe (paymentKeyFingerprint @k addr)
    let
      (ixM, _) = Shared.isShared addr st
    case (dTM, ixM) of
      (Just dT, Just (ix, _)) ->
...
data WalletException
  ...
  | forall level.
    ExceptionSoftDerivationIndex
      (ErrInvalidDerivationIndex 'Soft level)
  | forall level.
    ExceptionHardenedDerivationIndex
