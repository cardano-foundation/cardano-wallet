...
import Cardano.Wallet.Shelley.Compatibility.Ledger qualified as W
    ...
    do
      ...
      (extraInputs, extraCollateral', extraOutputs, s') <- do
        ...
        let
          ...
          transform sel =
            let
              (sel', s') = assignChangeAddresses genChange sel s
    ...
    where
      ...
      guardTxSize witCount tx@(Cardano.Tx body _noKeyWits) =
        withConstraints (recentEra @era) $ do
          let
            maxSize = TxSize (pp ^. ppMaxTxSizeL)
      ...
      balanceAfterSettingMinFee tx = ExceptT . pure $ do
        let
          witCount = estimateKeyWitnessCount combinedUTxO (getBody tx)
          minfee =
            W.toWalletCoin
              $ evaluateMinimumFee
                (recentEra @era)
      ...
      guardConflictingWithdrawalNetworks
        (Cardano.Tx (Cardano.TxBody body) _) = do
          ...
          let
            networkOfWdrl ((Cardano.StakeAddress nw _), _, _) = nw
...
modifyShelleyTxBody
  ...
  -> TxBody (ShelleyLedgerEra era)
modifyShelleyTxBody txUpdate era =
  withConstraints era
...
distributeSurplusDeltaWithOneChangeCoin
  feePolicy
  surplus
  fc@(TxFeeAndChange fee0 change0) =
    let
