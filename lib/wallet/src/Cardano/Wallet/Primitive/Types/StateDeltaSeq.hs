...
countTransitionsWhere
    :: ((s, d, s) -> Bool) -> StateDeltaSeq s d -> Int
...
-- A transition is empty if its initial state is equal to its final state.
countEmptyTransitionsWhere
    :: Eq s => (d -> Bool) -> StateDeltaSeq s d -> Int
...
isPrefixOf
    :: (Eq s, Eq d) => StateDeltaSeq s d -> StateDeltaSeq s d -> Bool
...
isValidM
    :: (Monad m, Eq s) => (s -> d -> m s) -> StateDeltaSeq s d -> m Bool
...
--   deltas.
toStateDeltaList
    :: StateDeltaSeq s d -> NonEmpty (StateDeltaListItem s d)
toStateDeltaList s =
    NE.fromList
        $ interleave
...
-- To verify whether the resulting sequence is valid, use 'isValid'.
applyDelta
    :: (s -> d -> s) -> d -> StateDeltaSeq s d -> StateDeltaSeq s d
...
dropEmptyTransitionWhere f s@StateDeltaSeq{head, tail} =
    StateDeltaSeq head . flip Seq.deleteAt tail
...
dropEmptyTransitionsWhere f s@StateDeltaSeq{head, tail} =
    StateDeltaSeq head
        $ F.foldl'
            (flip Seq.deleteAt)
            tail
            (reverse $ emptyTransitionsWhere f s)
