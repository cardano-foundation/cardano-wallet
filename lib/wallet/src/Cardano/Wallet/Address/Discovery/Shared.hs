    ...
    if networkTag == networkDiscriminantBits (sNetworkId @n)
      then case paymentKeyFingerprint addrRaw of
        ...
        Right addr -> case ( AddressPool.lookup addr (getPool extPool)
          ...
          (Just ix, Nothing) ->
            let
              pool' = AddressPool.update addr (getPool extPool)
            in
          ...
          (Nothing, Just ix) ->
            let
              pool' = AddressPool.update addr (getPool intPool)
            in
  ...
  knownAddresses st = case ready st of
    ...
    where
      ...
      pendingChangeAddresses intPool ixs =
        let
          internalGap = AddressPool.gap $ getPool intPool
...
instance GenChange (SharedState n k) where
  ...
  genChange mkAddress st = case ready st of
    ...
    Active (SharedAddressPools extPool intPool pending) ->
      let
        (ix, pending') = nextChangeIndex (getPool intPool) pending
        addr = mkAddress (paymentTemplate st) (delegationTemplate st) ix
      in
        (addr, st {ready = Active (SharedAddressPools extPool intPool pending')})
...
isOwned st (rootPrv, pwd) addr = case isShared addr st of
  (Just (ix, role'), _) ->
    let
      DerivationPrefix (_, _, accIx) = derivationPrefix st
...
estimateMinWitnessRequiredPerInput = \case
  ...
  RequireSomeOf m xs ->
    ...
    in
      sum $ take (fromIntegral m) smallestReqFirst
  ActiveFromSlot _ -> 0
  ActiveUntilSlot _ -> 0
...
instance AccountIxForStaking (SharedState n SharedKey) where
  getAccountIx st =
    let
      DerivationPrefix (_, _, ix) = derivationPrefix st
    in
      Just ix
