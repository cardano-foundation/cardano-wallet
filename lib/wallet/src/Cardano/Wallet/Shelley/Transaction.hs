...
import Cardano.Ledger.Keys.Bootstrap qualified as SL
...
import Cardano.Wallet.Shelley.Compatibility qualified as Compatibility
...
mkTx keyF networkId payload ttl (rewardAcnt, pwdAcnt) addrResolver wdrl cs fees era =
  do
    let
      TxPayload md certs mkExtraWits = payload
    let
      wdrls = mkWithdrawals networkId wdrl
    ...
    let
      withResolvedInputs (tx, _, _, _, _, _) =
        tx
  ...
  where
    ...
    inputResolver i =
      let
        index = Map.fromList (F.toList $ view #inputs cs)
      in
        do
    ...
    acctResolver acct = do
      let
        acct' = toRewardAccountRaw $ toXPub rewardAcnt
      ...
      wits' =
        ...
        where
          ...
          stakingScriptsKeyHashes =
            let
              (_, _, _, _, _, (WitnessCount _ nativeScripts _)) =
                fromCardanoTx witCountCtx
                  $ Cardano.makeSignedTransaction wits body
...
newTransactionLayer keyF networkId =
  TransactionLayer
    { mkTransaction = \era stakeCreds keystore _pp ctx selection -> do
        let
          ttl = txValidityInterval ctx
        let
          wdrl = view #txWithdrawal ctx
        let
          delta = selectionDelta TxOut.coin selection
        case view #txDelegationAction ctx of
          Nothing -> withShelleyBasedEra era $ do
            let
              payload = TxPayload (view #txMetadata ctx) mempty mempty
            mkTx
              keyF
              networkId
              payload
          ...
          Just action -> withShelleyBasedEra era $ do
            let
              stakeXPub = toXPub $ fst stakeCreds
            let
              certs = mkDelegationCertificates action (Left stakeXPub)
            let
              payload = TxPayload (view #txMetadata ctx) certs (const [])
            mkTx
              keyF
              networkId
              payload
    ...
    , addVkWitnesses =
        \era
         ...
         sealedTx -> do
            let
              acctMap :: Map RewardAccount (XPrv, Passphrase "encryption")
              acctMap =
                Map.fromList
                  $ map
            ...
            let
              ...
              stakingScriptResolver keyhash = case scriptStakingCredM of
                Just scriptStakingCred -> do
                  let
                    (keyhash', xprv, encP) = scriptStakingCred
                  guard (keyhash == keyhash') $> (xprv, encP)
                Nothing -> Nothing
    ...
    , mkUnsignedTransaction = \stakeCred ctx selection -> do
        ...
        let
          wdrl = view #txWithdrawal ctx
        let
          delta = case selection of
            Right selOf -> selectionDelta TxOut.coin selOf
            Left _preSel -> Coin 0
        let
          assetsToBeMinted = view #txAssetsToMint ctx
        let
          assetsToBeBurned = view #txAssetsToBurn ctx
        let
          inpsScripts = view #txNativeScriptInputs ctx
        let
          stakingScriptM =
        ...
        case view #txDelegationAction ctx of
          Nothing -> do
            let
              md = view #txMetadata ctx
            let
              ourRewardAcctM = FromScriptHash . unScriptHash . toScriptHash <$> stakingScriptM
            case wdrl of
          ...
          Just action -> do
            let
              certs = case stakeCred of
                ...
                Right Nothing ->
                  error
                    $ "stakeCred in mkUnsignedTransaction must be either "
                      <> "xpub or script when there is delegation action"
            let
              payload = (view #txMetadata ctx, certs)
            constructUnsignedTx
              networkId
              payload
              ttl
        ...
        $ Cardano.createAndValidateTransactionBody
          Cardano.TxBodyContent
            ...
            , Cardano.txValidityRange =
                let
                  toLowerBound from = case txValidityLowerBoundSupported of
            ...
            , Cardano.txMintValue =
                case txMintingSupported of
                  ...
                  Just mintedEra ->
                    let
                      mintValue = toCardanoValue (TokenBundle (Coin 0) mintData)
                      burnValue =
                        Cardano.negateValue
                      ...
                      ctx = Cardano.BuildTxWith witMap
                    in
                      Cardano.TxMintValue mintedEra (mintValue <> burnValue) ctx
            }
    where
      ...
      constructInpScriptWit inp =
        let
          script = case Map.lookup inp inpsScripts of
      ...
      inputWits = case cs of
        Right selOf ->
          if inpsScripts == Map.empty
            then
              (,buildTxCommand)
                . toCardanoTxIn
                . fst
                <$> F.toList (view #inputs selOf)
            else constructInpScriptWit . fst <$> F.toList (view #inputs selOf)
        Left _preSel ->
