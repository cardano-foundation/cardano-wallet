...
import Cardano.Wallet.Address.Derivation qualified as W
...
import Cardano.Wallet.Primitive.Types.Tx.TxOut qualified as W.TxOut
  ...
  where
    ...
    update1 (RestrictTo pts) = do
      let
        points = W.Origin : pts
      let
        pseudoSlot W.Origin = W.SlotNo 0
        pseudoSlot (W.At slot) = slot
      let
        slots = map pseudoSlot points
...
selectAllCheckpoints wid = do
  ...
  cps <- forM cpRefs $ \cp -> do
    ...
    let
      c = checkpointFromEntity @s cp utxo discoveries
    pure (getSlot c, c)
  pure $ case cps of
...
insertCheckpoint wid wallet@(WalletCheckpoint currentTip _ discoveries) = do
  let
    (cp, utxo, utxoTokens) = mkCheckpointEntity wid wallet
  let
    sl = currentTip ^. #slotNo
...
mkCheckpointEntity wid (WalletCheckpoint header wutxo _) =
  ...
  where
    ...
    utxoTokens =
      ...
      , let
          tokenList = snd (TokenBundle.toFlatList tokens)
...
instance
  ...
  loadPrologue wid = runMaybeT $ do
    ...
    let
      SeqState _ eGap iGap accountBytes policyBytes rewardBytes prefix =
        entityVal st
    let
      accountXPub = unsafeDeserializeXPub accountBytes
    let
      rewardXPub = unsafeDeserializeXPub rewardBytes
    let
      policyXPub = unsafeDeserializeXPub <$> policyBytes
    let
      intPool = Seq.newSeqAddressPool @n accountXPub iGap
    let
      extPool = Seq.newSeqAddressPool @n accountXPub eGap
...
instance
  ...
  => PersistAddressBook (Shared.SharedState n key)
  where
      ...
      insertCosigner cs cred = do
        ...
        dbChunked
          ...
          [ CosignerKey
            wid
            cred
            ( serializeXPub @(key 'AccountK)
                $ liftRawKey SharedKeyS xpub
            )
            c
  ...
  loadPrologue wid = runMaybeT $ do
    ...
    let
      SharedState _ accountBytes gap pScript dScriptM rewardAcctM prefix =
        entityVal st
    let
      accXPub = unsafeDeserializeXPub accountBytes
    pCosigners <- lift $ selectCosigners @key wid Payment
    dCosigners <- lift $ selectCosigners @key wid Delegation

    let
      prepareKeys = fmap $ second $ getRawKey SharedKeyS
      pTemplate =
    ...
    prologue <-
      lift $ multisigPoolAbsent wid <&> \case
...
selectCosigners wid cred = do
  fmap (cosignerFromEntity . entityVal)
    <$> selectList
...
instance PersistAddressBook (Rnd.RndAnyState n p) where
    ...
    st <-
      MaybeT
        $ selectFirst
          ...
          []
    let
      (RndState _ ix gen (HDPassphrase pwd)) = entityVal st
