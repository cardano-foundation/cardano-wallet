...
migrateSubmissions = mkMigration $ ReaderT $ \db -> do
    ...
    let migration = do
            ...
            liveRows <-
                query
                    conn
                    "SELECT wallet_id, tx_id, tx FROM submissions WHERE status = 0"
            forM_ liveRows $ \case
                [ PersistText walletId
                    , PersistText submissionId
...
insertLiveClaims
    :: Sqlite.Connection -> Text -> Text -> BS.ByteString -> IO ()
insertLiveClaims conn walletId submissionId bytes = do
    ...
    if toText (txId tx) /= submissionId
        then
            fail "V5 submission transaction id does not match its sealed body"
        else do
...
insertClaim
    :: Sqlite.Connection -> Text -> Text -> Int -> TxIn -> IO ()
...
sqlHex value
    ...
    | otherwise =
        error "non-hexadecimal persisted identifier in V6 migration"
  where
    isHex c =
        ('0' <= c && c <= '9')
            || ('a' <= c && c <= 'f')
