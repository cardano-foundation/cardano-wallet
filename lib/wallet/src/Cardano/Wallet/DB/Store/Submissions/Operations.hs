...
import Cardano.Wallet.Submissions.Operations qualified as Sbm
...
import Cardano.Wallet.Submissions.Submissions qualified as Sbm
import Cardano.Wallet.Submissions.TxStatus qualified as Sbm
...
syncSubmissions wid old new = do
  let
    deletes = transactions old `Map.difference` transactions new
  forM_ (Map.keys deletes) $ \k -> delete (SubmissionsKey k)

  let
    repserts = new ^. transactionsL
