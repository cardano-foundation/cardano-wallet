...
import Cardano.Wallet.Primitive.Types.Tx.Tx qualified as W.Tx
import Cardano.Wallet.Primitive.Types.Tx.TxIn qualified as W
  ( TxIn
  )
import Cardano.Wallet.Primitive.Types.Tx.TxIn qualified as W.TxIn
...
newtype TxSet = TxSet {relations :: Map TxId TxRelation}
  deriving (Generic, Eq, Show)
...
toTxCBOR (id', tx) =
  let
    r = L.build eraValueSerialize tx
  in
    (uncurry (CBOR id') $ r ^. i, r)
