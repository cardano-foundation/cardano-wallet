      ...
      pure $ mconcat $ do
        ...
        pure
          ...
          $ TxRelation
            ...
            , outs =
                fmap (fmap $ sortOn tokenOutOrd)
                  $ sortOn (txOutputIndex . fst)
                  $ (id &&& selectOutTokens k)
                    <$> orEmpty k outputs
            , collateralOuts =
                fmap (fmap $ sortOn tokenCollateralOrd)
...
selectTxCollateralOut (txid, index) = do
  ...
  let
    collateralOutputIndex = toEnum noutputs
