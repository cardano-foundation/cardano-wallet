...
import Cardano.Wallet.Address.Derivation.Byron qualified as Byron
...
import Cardano.Wallet.Address.Discovery.Shared qualified as Shared
...
import Cardano.Wallet.Api.Types qualified as Api
...
import Cardano.Wallet.DB qualified as W
import Cardano.Wallet.Delegation qualified as WD
...
import Cardano.Wallet.Network qualified as NW
...
import Cardano.Wallet.Registry qualified as Registry
...
import Cardano.Wallet.Write.Tx.Balance qualified as Write
import Control.Arrow
  ( second
  , (&&&)
  )
import Control.Concurrent.Concierge qualified as Concierge
...
import Network.Ntp qualified as Ntp
...
postWallet ctx generateKey liftKey (WalletOrAccountPostData body) = case body of
  ...
  Right body' ->
    ...
    in
      postAccountWallet ctx mkShelleyWallet liftKey action body'
  ...
  tip' <-
    liftIO
      $ getWalletTip
        ...
        cp
  let
    available = availableBalance pending cp
  let
    total = totalBalance pending reward cp
  pure
    ApiWallet
...
postSharedWalletFromRootXPrv ctx generateKey body = do
  ...
  let
    state =
      mkSharedStateFromRootXPrv
        ...
        dTemplateM
  let
    stateReadiness = state ^. #ready
  if stateReadiness == Shared.Pending
    then
      void
        $ liftHandler
        $ createNonRestoringWalletWorker @_ @s
          ctx
          wid
          (\dbf -> W.createWallet @_ @s genesisParams dbf wid wName state)
    else
      if isNothing dTemplateM
        then
          void
            $ liftHandler
            $ createWalletWorker @_ @s
  ...
  where
    ...
    pTemplate =
      scriptTemplateFromSelf (getRawKey SharedKeyS accXPub)
...
postSharedWalletFromAccountXPub ctx liftKey body = do
  ...
  where
    ...
    ix = getApiT (body ^. #accountIndex)
    pTemplate =
      scriptTemplateFromSelf accXPub
        $ body ^. #paymentScriptTemplate
    dTemplateM =
      scriptTemplateFromSelf accXPub
        <$> body ^. #delegationScriptTemplate
...
mkLegacyWallet ctx wid cp meta _ pending progress = do
  ...
  let
    available = availableBalance pending cp
  let
    total = totalBalance pending (Coin 0) cp
  pure
    ApiByronWallet
      { balance =
          ApiByronWalletBalance
...
withLegacyLayer'
  ...
  (icarus, withIcarus, deadIcarus) =
    ...
    where
      tryIcarus onMissing onNotResponding =
        withWorkerCtx @_
      ...
      tryByron onMissing onNotResponding =
        withWorkerCtx @_
          @(RndState n)
          byron
          wid
...
getWallet ctx mkApiWallet (ApiT wid) = do
  ...
  where
    ...
    whenNotResponding _ = Handler
      $ ExceptT
      $ withDatabase dbfa wid
      $ \df -> runHandler $ do
        db <- liftHandler $ loadDBLayer df
        let
          wrk = hoistResource db (MsgFromWorker wid) ctx
        (cp, (meta, delegation), pending) <-
          handler $ W.readWallet wrk
        (,meta ^. #creationTime)
    ...
    NoContent <$ case method of
      ...
      Right
        ( Api.WalletPutPassphraseMnemonicData
            (ApiMnemonicT mnemonic)
            sndFactor
            (ApiT new)
          ) -> do
          let
            encrPass = preparePassphrase currentPassphraseScheme new
            challengeKey =
              createKey
                (mnemonic, getApiMnemonicT <$> sndFactor)
                encrPass
            challengPubKey =
              publicKey (keyFlavorFromState @s)
...
putByronWalletPassphrase ctx (ApiT wid) body = do
  let
    (ByronWalletPutPassphraseData oldM (ApiT new)) = body
  withWorkerCtx ctx wid liftE liftE $ \wrk -> liftHandler $ do
    let
      old = maybe mempty (coerce . getApiT) oldM
    W.updateWalletPassphraseWithOldPassphrase
      (walletFlavor @s)
      wrk
      wid
...
getUTxOsStatistics ctx (ApiT wid) = do
  stats <- withWorkerCtx ctx wid liftE liftE $ \wrk ->
    handler
...
getWalletUtxoSnapshot ctx (ApiT wid) = do
  entries <- withWorkerCtx ctx wid liftE liftE $ \wrk ->
    handler
  ...
  where
    ...
    mkApiWalletUtxoSnapshot bundleMinCoins =
      ApiWalletUtxoSnapshot
    ...
    mkApiWalletUtxoSnapshotEntry (bundle, minCoin) =
      ApiWalletUtxoSnapshotEntry
...
selectCoins ctx@ApiLayer {..} argGenChange (ApiT walletId) body = do
  withWorkerCtx ctx walletId liftE liftE $ \workerCtx -> do
    ...
    let
      paymentOuts = NE.toList $ addressAmountToTxOut <$> body ^. #payments
    let
      txCtx =
        defaultTransactionCtx
    ...
    pure
      ApiCoinSelection
        ...
        , certificates =
            uncurry mkApiCoinSelectionCerts
              <$> delegationAction
...
selectCoinsForJoin
  ...
  walletId = do
    ...
    withWorkerCtx ctx walletId liftE liftE $ \workerCtx -> liftIO $ do
      let
        db = workerCtx ^. typed @(DBLayer IO s)
      currentEpochSlotting <- liftIO $ getCurrentEpochSlotting netLayer
      action <-
        liftIO
          $ WD.joinStakePoolDelegationAction @s
            ...
            poolStatus
      let
        changeAddrGen = W.defaultChangeAddressGen (delegationAddressS @n)

      let
        txCtx = defaultTransactionCtx {txDelegationAction = Just action}
...
getAsset ctx wid (ApiT policyId) (ApiT assetName) = do
  ...
  where
    findAsset =
      maybe (throwE ErrGetAssetNotPresent) pure
...
listAddresses ctx normalize (ApiT wid) stateFilter = do
  addrs <- withWorkerCtx ctx wid liftE liftE $ \wrk ->
    handler
    ...
    db & \W.DBLayer {atomically, readCheckpoint} ->
      W.withRootKey @s db wid pwd ErrWitnessTxWithRootKey
        $ \rootK scheme -> lift $ do
          ...
          let
            sealedTx = body ^. #transaction . #getApiT
          pure
            $ W.signTransaction
              (keyFlavorFromState @s)
              tl
...
postTransactionOld ctx@ApiLayer {..} argGenChange (ApiT wid) body = do
  let
    outs = addressAmountToTxOut <$> body ^. #payments
  let
    ...
    wdrl <- case body ^. #withdrawal of
      ...
      Just apiWdrl ->
        shelleyOnlyMkWithdrawal
          ...
          apiWdrl
    let
      outputs = F.toList $ addressAmountToTxOut <$> body ^. #payments
      minCoins =
        W.calcMinimumCoinValues walletPP txLayer
          <$> outputs
    feePercentiles <-
      liftIO
        $ W.transactionFee @s
...
constructTransaction api argGenChange knownPools poolStatus apiWalletId body = do
  ...
  withWorkerCtx api walletId liftE liftE $ \wrk -> do
    ...
    (transactionCtx2, policyXPubM) <-
      if isJust mintBurnData
        then do
          (policyXPub, _) <-
            liftHandler $ W.readPolicyPublicKey wrk
          let
            isMinting (ApiMintBurnData _ _ (ApiMint _)) = True
            isMinting _ = False
          let
            getMinting = \case
              ApiMintBurnData
        ...
        else pure (transactionCtx1, Nothing)
    ...
    let
      mintingOuts = case mintBurnData of
        Just mintBurns ->
    ...
    let
      deposits = case txDelegationAction transactionCtx2 of
        Just (JoinRegisteringKey _poolId) -> [W.getStakeKeyDeposit pp]
        _ -> []

    let
      refunds = case txDelegationAction transactionCtx2 of
  ...
  where
    ...
    parseMintBurnData tx validity = do
      let
        mbMintingBurning :: Maybe (NonEmpty (ApiMintBurnData n))
        ...
        guardOutsideValidityInterval (before, hereafter) mintBurnData =
          ...
          where
            notWithinValidityInterval (ApiMintBurnData (ApiT script) _ _) =
              not
                $ withinSlotInterval before hereafter
                $ scriptSlotIntervals script

    unsignedTx path initialOuts decodedTx =
      UnsignedTx
        { unsignedCollateral =
...
parseDelegationRequest (action :| otherActions) = except
  $ case otherActions of
    [] -> case action of
      Joining (ApiT pool) stakeKeyIdx
        | isValidKeyIdx stakeKeyIdx ->
            Right $ WD.Join pool
      Leaving stakeKeyIdx
        | isValidKeyIdx stakeKeyIdx ->
            Right WD.Quit
...
parseValidityInterval ti validityInterval = do
  ...
  let
    isThereNegativeTime = case validityInterval of
...
constructSharedTransaction
  ...
  body = do
    ...
    when isNoPayload
      $ liftHandler
      $ throwE ErrConstructTxWrongPayload
    ...
    withWorkerCtx api wid liftE liftE $ \wrk -> do
      ...
      let
        delegationTemplateM = Shared.delegationTemplate $ getState cp
      withdrawal <- case body ^. #withdrawal of
        Just SelfWithdraw ->
          liftIO
      ...
      when (isNothing delegationTemplateM && isJust delegationRequest)
        $ liftHandler
        $ throwE ErrConstructTxDelegationInvalid
      ...
      case Shared.ready (getState cp) of
        ...
        Shared.Active _ -> do
          ...
          let
            deposits = case optionalDelegationAction of
              Just (JoinRegisteringKey _poolId) ->
                [W.getStakeKeyDeposit pp]
              _ -> []
          let
            refunds = case optionalDelegationAction of
              Just Quit -> [W.getStakeKeyDeposit pp]
              _ -> []
    ...
    where
      ...
      unsignedTx initialOuts decodedTx pathM =
        UnsignedTx
          { unsignedCollateral =
              mapMaybe toUnsignedTxInp (decodedTx ^. #collateral)
          , unsignedInputs =
...
decodeSharedTransaction ctx (ApiT wid) (ApiSerialisedTransaction (ApiT sealed) _) = do
  ...
  ( txinsOutsPaths
    ...
    withWorkerCtx ctx wid liftE liftE $ \wrk -> do
      (cp, _, _) <- handler $ W.readWallet wrk
      let
        witCountCtx = toWitnessCountCtx SharedWallet (getState cp)
      let
        (decodedTx, _toMint, _toBurn, allCerts, interval, witsCount) =
      ...
      let
        delegationTemplateM = (getState cp) ^. #delegationTemplate
      let
        scriptM =
      ...
      let
        rewardAcctM = case scriptM of
          Just script ->
            ...
            in
              Just $ FromScriptHash scriptHash
          Nothing -> Nothing
      let
        certs = mkApiAnyCertificate rewardAcctM rewardAcctPath <$> allCerts
      pure
        ( inputPaths
        , collateralInputPaths
        , outputPaths
    ...
    withWorkerCtx ctx wid liftE liftE $ \wrk -> do
      ...
      case body ^. #encoding of
        Just HexEncoded ->
          pure
            $ ApiSerialisedTransaction
              (ApiT $ W.sealedTxFromCardano balancedTx)
              HexEncoded
        _ ->
          pure
            $ ApiSerialisedTransaction
...
decodeTransaction
  ctx@ApiLayer {..}
  (ApiT wid)
  (ApiSerialisedTransaction (ApiT sealed) _) = do
    era <- liftIO $ NW.currentNodeEra netLayer
    withWorkerCtx ctx wid liftE liftE $ \wrk -> do
      (k, _) <- liftHandler $ W.readPolicyPublicKey wrk
      let
        keyhash = KeyHash Policy (xpubToBytes k)
      let
      ...
      let
        certs = mkApiAnyCertificate (Just acct) acctPath <$> allCerts
      pure
        $ ApiDecodedTransaction
          { id = ApiT txId
          , fee = maybe (Quantity 0) (Quantity . fromIntegral . unCoin) fee
...
submitTransaction ctx apiw@(ApiT wid) apitx = do
  ...
  let
    sealedTx = getApiT . (view #serialisedTxSealed) $ apitx
  ...
  let
    totalNumberOfWits = length $ getSealedTxWitnesses sealedTx

  when (countJoinsQuits (apiDecoded ^. #certificates) > 1)
    $ liftHandler
    $ throwE ErrSubmitTransactionMultidelegationNotSupported
  ...
  void $ withWorkerCtx ctx wid liftE liftE $ \wrk -> do
    ...
    let
      keyhash = KeyHash Policy (xpubToBytes k)
    let
      (tx, _, _, _, _, _) = decodeTx tl era (ShelleyWalletCtx keyhash) sealedTx
    ...
    let
      wdrl = getOurWdrl acct path apiDecoded
    let
      txCtx =
        defaultTransactionCtx
          { -- TODO: [ADP-1193]
  ...
  where
    ...
    getOurWdrl rewardAcct path apiDecodedTx =
      ...
      in
        case filter isWdrlOurs generalWdrls of
...
submitSharedTransaction ctx apiw@(ApiT wid) apitx = do
  ...
  void $ withWorkerCtx ctx wid liftE liftE $ \wrk -> do
    ...
    let
      witCountCtx = toWitnessCountCtx SharedWallet (getState cp)
    let
      (tx, _, _, _, _, (WitnessCount _ nativeScripts _)) =
        decodeTx tl era witCountCtx sealedTx

    let
      numberStakingNativeScripts =
        length $ filter hasDelegationKeyHash nativeScripts
    let
      delegationScriptTemplateM = Shared.delegationTemplate $ getState cp
    let
      delegationWitsRequired = case delegationScriptTemplateM of
        Nothing -> 0
        Just (ScriptTemplate _ scriptD) ->
          if numberStakingNativeScripts == 0
            then 0
            else
              if numberStakingNativeScripts == 1
                then Shared.estimateMinWitnessRequiredPerInput scriptD
                else error "wallet supports transactions with 0 or 1 staking script"

    let
      (ScriptTemplate _ scriptP) = Shared.paymentTemplate $ getState cp
    let
      pWitsPerInput = Shared.estimateMinWitnessRequiredPerInput scriptP
    let
      witsRequiredForInputs =
        length
          $ L.nubBy samePaymentKey
...
listStakeKeys' utxo lookupStakeRef fetchRewards ourKeysWithInfo = do
  ...
  let
    ourKeys = map (\(acc, _, _) -> acc) ourKeysWithInfo

  let
    allKeys = ourKeys <> stakeKeysInUTxO
  ...
  let
    foreignKeys = stakeKeysInUTxO \\ ourKeys
...
listStakeKeys lookupStakeRef ctx@ApiLayer {..} (ApiT wid) =
  withWorkerCtx ctx wid liftE liftE $ \wrk -> handler $ do
    ...
    let
      utxo = availableUTxO @s pending wal
    let
      takeFst (a, _, _) = a
    ...
    let
      ourKeys = [(ourAccount, 0, ourApiDelegation)]
    liftIO
      $ listStakeKeys' @n
        utxo
        lookupStakeRef
...
mkApiWalletMigrationPlan s addresses rewardWithdrawal plan =
  ...
  where
    ...
    maybeUnsignedTxs = fmap mkUnsignedTx <$> maybeSelectionWithdrawals
      where
        mkUnsignedTx (selection, withdrawal) =
  ...
  mode = liftIO $ do
    ...
    where
      ...
      networkTipInfo now = do
        ...
        let
          curEpoch = tip ^. #slotId . #epochNumber . #getApiT
        (_, nextEpochStart) <- interpretQuery ti $ timeOfEpoch curEpoch
        let
          nextEpoch = EpochInfo (succ curEpoch) nextEpochStart
...
getNetworkParameters (_block0, genesisNp) nl = do
  ...
  let
    np = genesisNp {protocolParameters = pp, slottingParameters = sp}
...
postPolicyId ctx (ApiT wid) payload = do
  ...
  when (wrongMintingTemplate scriptTempl)
    $ liftHandler
    $ throwE ErrGetPolicyIdWrongMintingBurningTemplate
  ...
  case wF of
    ShelleyWallet -> case w of
      ...
      ExternalWithdrawal (ApiMnemonicT m) -> do
        let
          (xprv, _acct, _path) = W.someRewardAccount @ShelleyKey m
      ...
      $ (#time .~ tx ^. #txTime)
        <$> makeApiBlockReference
          ( neverFails
              ...
              \transactions with slots in the past"
              timeInterpreter
          )
          (tx ^. #txMeta . #slotNo)
...
mkApiFee
  ...
  -> ApiFee
mkApiFee mDeposit minCoins (Percentile estMin, Percentile estMax) =
  ApiFee
...
newApiLayer tr g0 nw tl df tokenMeta coworker = do
  ...
  let
    ctx = ApiLayer trTx trW g0 nw tl df re locks tokenMeta
...
instance IsServerError ErrGetAsset where
  toServerError = \case
    ErrGetAssetNotPresent ->
      apiError err404 AssetNotPresent
        $ mconcat
