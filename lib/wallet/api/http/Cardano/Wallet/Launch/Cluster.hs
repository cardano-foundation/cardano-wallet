...
tokenMetadataServerFromEnv =
  envFromText "TOKEN_METADATA_SERVER" >>= \case
...
cliConfigBase tr conn args = do
  ...
  env <- getEnvironment
  let
    mkEnv c = ("CARDANO_NODE_SOCKET_PATH", nodeSocketFile c) : env
  let
...
configurePool tr baseDir era metadataServer recipe = do
  let
    PoolRecipe pledgeAmt i mretirementEpoch metadata _ _ = recipe
  ...
  let
    ownerPub = dir </> "stake.pub"
  let
    ownerPrv = dir </> "stake.prv"
  genStakeAddrKeyPair tr (ownerPrv, ownerPub)

  let
    metadataURL = urlFromPoolIndex metadataServer i
  ...
  pure
    ConfiguredPool
      { operatePool = \nodeParams action -> do
          let
            NodeParams genesisFiles hardForks (port, peers) logCfg = nodeParams
          let
            logCfg' = setLoggingName name logCfg
      ...
      , registerViaTx = \(RunningNode socket _ _ _) -> do
          ...
          let
            poolRegistrationCert = dir </> "pool.cert"
          cli
            tr
            [ "stake-pool"
...
generateGenesis dir systemStart initialFunds addPoolsToGenesis = do
  ...
  let
    startTime = round @_ @Int . utcTimeToPOSIXSeconds $ systemStart
  let
    systemStart' = posixSecondsToUTCTime . fromRational . toRational $ startTime
  ...
  let
    shelleyGenesisFile = (dir </> "genesis.json")
  Aeson.encodeFile shelleyGenesisFile sg

  let
    byronGenesisFile = dir </> "genesis.byron.json"
...
withCluster tr dir LocalClusterConfig {..} faucetFunds onClusterStart = bracketTracer' tr "withCluster" $ do
  withPoolMetadataServer tr dir $ \metadataServer -> do
    ...
    if postAlonzo
      then do
        ...
        let
          pool0 :| otherPools = configuredPools
  ...
  where
    ...
    extraClusterSetupUsingNode configuredPools runningNode = do
      let
        RunningNode conn _ _ _ = runningNode
    ...
    launchPools configuredPools genesisFiles ports fallbackNode action = do
      ...
      let
        poolCount = length configuredPools
      ...
      if length (filter isRight group) /= poolCount
        then do
          cancelAll
          let
            errors = show (filter isLeft group)
          throwIO
            $ ProcessHasExited
...
_withRelayNode tr baseDir params act =
  bracketTracer' tr "withRelayNode" $ do
    ...
    let
      act' socket = act $ RunningNode socket block0 (bp, vd) []
...
withSMASH tr parentDir action = do
  let
    staticDir = parentDir </> "smash"
  let
    baseDir = staticDir </> "api" </> "v1"

  -- write pool metadatas
  forM_ defaultPoolConfigs $ \pool -> do
    let
      (poolId, _, _, _) = operatorKeys pool
    let
      metadata = poolMetadata pool

    let
      bytes = Aeson.encode metadata
  ...
  let
    toSmashId (PoolId bytes) = SMASHPoolId . T.pack . B8.unpack . hex $ bytes
  let
    poolId (PoolRecipe _ _ _ _ (pid, _, _, _) _) = toSmashId pid
  let
    delistedPoolIds = poolId <$> NE.filter delisted defaultPoolConfigs
  BL8.writeFile
    (baseDir </> "delisted")
  ...
  -- health check
  let
    health = Aeson.encode (HealthStatusSMASH "OK" "1.2.0")
...
genNodeConfig dir name genesisFiles clusterEra logCfg = do
  let
    LogFileConfig severity mExtraLogFile extraSev = logCfg
  let
    GenesisFiles {byronGenesis, shelleyGenesis, alonzoGenesis, conwayGenesis} =
      genesisFiles
  ...
  let
    (networkParameters, block0, genesisPools) = fromGenesisData genesisData
  let
...
writeOperatorKeyPair tr dir recipe = do
  let
    (_pId, pub, prv, count) = operatorKeys recipe
...
stakePoolIdFromOperatorVerKey filepath = do
  ...
  let
    bytes = serialiseToCBOR $ verificationKeyHash stakePoolVerKey
  pure
...
genMonetaryPolicyScript tr dir = do
  let
    policyPub = dir </> "policy.pub"
  let
    policyPrv = dir </> "policy.prv"
...
sendFaucet tr conn dir era what targets = do
  (faucetInput, faucetPrv) <- takeFaucet
  let
    file = dir </> "faucet-tx.raw"

  let
    mkOutput addr (TokenBundle (Coin c) tokens) =
      [ "--tx-out"
      , unwords
  ...
  let
    targetAssets = concatMap (snd . TokenBundle.toFlatList . fst . snd) targets
...
moveInstantaneousRewardsTo tr conn dir era targets = do
  ...
  let
    total = fromIntegral $ sum $ map (unCoin . snd) targets
  let
    totalDeposit = fromIntegral (length targets) * depositAmt
  ...
  let
    bftPrv = testData </> "bft-leader" <> ".skey"
  ...
  where
    ...
    mkVerificationKey xpub = do
      let
        base16 = T.unpack $ T.decodeUtf8 $ hex $ xpubPublicKey xpub
      let
        json =
          Aeson.object
    ...
    mkScript bytes = do
      let
        base16 = T.decodeUtf8 $ hex $ CBOR.toStrictByteString $ CBOR.encodeBytes bytes
      let
        json =
          Aeson.object
      ...
      let
        prefix = take 100 (T.unpack base16)
      let
        file = dir </> prefix <> ".plutus"
      BL8.writeFile file (Aeson.encode json)
      pure (prefix, file)
...
prepareKeyRegistration tr dir era = do
  ...
  let
    stakePub = dir </> "pre-registered-stake.pub"
...
genSinkAddress tr dir stakePub = do
  let
    sinkPrv = dir </> "sink.prv"
  let
    sinkPub = dir </> "sink.pub"
  cli
    tr
    [ "address"
    , "key-gen"
...
takeFaucet = do
  ...
  source <- getShelleyTestDataPath
  let
    basename = source </> "faucet-addrs" </> "faucet" <> show i
  ...
  let
    txin = B8.unpack (hex $ blake2b256 addr) <> "#0"
  let
    signingKey = basename <> ".shelley.key"
...
addMinSeverityStdout severity ob = case Aeson.lookup "setupScribes" ob of
  Just (Aeson.Array scribes) -> do
    let
      scribes' = Aeson.Array $ fmap setMinSev scribes
