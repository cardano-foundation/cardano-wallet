...
withLogging
    :: MonadUnliftIO m => ((Tracer m msg, m [msg]) -> m a) -> m a
...
captureLogging
    :: MonadUnliftIO m => (Tracer m msg -> m a) -> m ([msg], a)
...
traceSpec
    :: (HasCallStack, ToText msg) => SpecWith (Tracer IO msg) -> Spec
