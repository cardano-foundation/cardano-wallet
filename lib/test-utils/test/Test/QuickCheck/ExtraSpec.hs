...
prop_interleaveRoundRobin_coverage
    :: Testable p => [[Int]] -> p -> Property
...
genPartitionListData
    :: forall a. Arbitrary a => Gen (PartitionListData a)
genPartitionListData =
...
prop_selectMapEntry_singleton k v =
    forAll
        (selectMapEntry (Map.singleton k v))
        (=== Just ((k, v), mempty))
...
prop_shrinkWhile_coverage
    :: Arbitrary a => Fun a Bool -> a -> Property
...
prop_shrinkDisjointPair_getDisjointPair_areDisjoint s1 s2 =
    all (areDisjoint . getDisjointPair) (shrinkDisjointPair shrink p)
        === True
        & cover
            10
