...
import Test.Hspec.Extra qualified as Extra
  ...
  describe "when first attempt fails due to flakiness" $ do
    ...
    describe "when the retry also fails" $ do
      ...
      it "fails with the first error" $ do
        let
          failure = expectationFailure "failure"
        let
          noRetry = expectationFailure "test can't be retried"
...
aroundAllSpec = sequential $ do
  let
    withMockResource :: MonadUnliftIO m => a -> (a -> m r) -> m r
  ...
  describe "Extra.aroundAll" $ do
    ...
    let
      withResource = bracket (putMVar mvar ()) (`takeMVarCheck` mvar)
      ...
      let
        trySpec =
          fmap (first show)
            ...
            . flip runSpec defaultConfig
      let
        bombBefore = bracket (throwString "bomb1") (const $ pure ())
      let
        bombAfter = bracket (pure ()) (const $ throwString "bomb2")
