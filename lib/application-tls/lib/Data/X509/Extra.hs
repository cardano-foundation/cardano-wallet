...
class EncodePEM a where
    ...
    encodePEMRaw
        :: (ByteString, a -> ByteString, ByteString) -> a -> ByteString
...
failIfReasons = \case
    ...
    xs ->
        fail
            $ "Generated invalid certificate: " ++ intercalate ", " (map show xs)
...
getKeyUsagePurposes
    ...
    -> [ExtKeyUsagePurpose]
getKeyUsagePurposes =
    maybe [] (\(ExtExtendedKeyUsage ps) -> ps)
        . extensionGet
        . certExtensions
        . getCertificate
...
validateCertificateIP ip cert =
    ...
    in
        ...
        | ip `notElem` maybeToList commonName ++ altNames
        ]
