  ...
  where
    fromGenesisCmd =
        command "from-genesis"
            ...
            $ progDesc "Restore the wallet from genesis."
                ...
                <> footer
                    "This is the way to restore the wallet from the very beginning."
    fromTipCmd =
        command "from-tip"
    ...
    fromCheckpointCmdFooter =
        vsep
            ...
            , indent
                2
                "cardano-wallet wallet create from-recovery-passphrase \
                \from-checkpoint \
...
useMnemonicOption =
    flag OldPasswordCredentials MnemonicCredentials
        $ long "mnemonic"
...
cmdWalletDelete mkClient =
  ...
  where
    ...
    exec (WalletDeleteArgs wPort wId) = do
        runClient wPort (const "")
            $ deleteWallet mkClient
            $ ApiT wId
...
metadataSchemaOption =
    flag TxMetadataDetailedSchema TxMetadataNoSchema
...
cmdTransactionCreate isShelley mkTxClient mkWalletClient =
  ...
  where
    ...
    exec (TransactionCreateArgs wPort wId wAddressAmounts md ttl) = do
        wPayments <-
            either (fail . getTextDecodingError) pure
...
cmdNetworkClock mkClient =
  ...
  where
    ...
    exec (NetworkClockArgs wPort forceNtpCheck) = do
        runClient wPort Aeson.encodePretty
            $ networkClock mkClient forceNtpCheck
...
uiRandomPortOption =
    flag' False
        $ mempty
            <> long "ui-random-port"
            <> help
                "serve the personal wallet UI on any available port (conflicts with --ui-port)"
...
walletStyleOption defaultStyle accepted =
    option
        (eitherReader fromTextS)
        ( long "wallet-style"
...
withSGR h sgr action =
    hIsTerminalDevice h >>= \case
...
optionalE
    :: (Monoid m, Eq m) => (m -> Either e a) -> (m -> Either e (Maybe a))
