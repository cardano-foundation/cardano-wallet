...
spec = do
    ...
    describe "general laws, tested on sparseArithmetic" $ do
        it "prop_monotonicHeight — next heights do not fluctuate"
            $ property
            $ \ctx@(GenHeightContext epochStability tip) ->
    ...
    describe "specific policies" $ do
        ...
        it "sparseArithmetic checkpoints after genesis are close to tip"
            $ property
            $ \(GenHeightContext epochStability tip) ->
...
prop_trailingLength (GenHeightContext gap tip) =
    forAll (choose (0, 5)) $ \n ->
        fromIntegral n
            >= length (toListAtTip (CP.trailingArithmetic n gap) tip)

prop_trailingGrid :: GenHeightContext -> Property
prop_trailingGrid (GenHeightContext gap tip) =
    forAll (choose (1, 7)) $ \n ->
        all (`divisibleBy` gap)
            $ toListAtTip (CP.trailingArithmetic n gap) tip
  where
...
genBlockHeight (GenHeightContext epochStability tip) =
    oneof
        [ genNonNegative
        , choose (0, 10)
