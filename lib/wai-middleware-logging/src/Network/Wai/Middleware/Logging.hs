...
withApiLogger t0 settings app req0 sendResponse = do
  ...
  app req $ \res -> do
    ...
    readIORef builderIO
      >>= let
            fromBuilder = second (BL.toStrict . B.toLazyByteString)
          in
            uncurry (logResponse t time req) . fromBuilder
...
getRequestBody req = do
  ...
  let
    rbody = atomicModifyIORef ichunks $ \case
      [] -> ([], mempty)
      x : y -> (y, x)
...
recordChunks i = \case
  ResponseStream s h sb ->
    return . ResponseStream s h
      $ let
          capture b (ms, b') = (ms <|> Just s, b' <> b)
        in
          (\send flush -> sb (\b -> modifyIORef i (capture b) >> send b) flush)
  ResponseBuilder s h b ->
    let
      capture (ms, b') = (ms <|> Just s, b' <> b)
    in
      modifyIORef i capture >> return (ResponseBuilder s h b)
  r ->
    return r
