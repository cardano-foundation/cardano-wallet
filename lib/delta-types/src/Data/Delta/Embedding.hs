...
mkEmbedding Embedding'{load, write, update} =
    Embedding
        { inject = start . write
...
fromEmbedding
    :: (Delta da, Delta db) => Embedding da db -> Embedding' da db
fromEmbedding Embedding{inject, project} =
    Embedding'
...
replaceFromApply
    :: (Delta da, a ~ Base da) => Embedding' da (Replace a)
replaceFromApply =
    Embedding'
        { load = Right
        , write = id
