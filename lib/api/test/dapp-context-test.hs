    ...
    describe "revision-1 transaction context" $ do
        ...
        describe "DAPP_DATA_SIGNING schema" $ do
            it "accepts only the closed revision-1 request and response" $ do
                ...
                decodeDappDataSignResponse validDataSignResponse
        ...
        describe "DAPP_WITNESS_SIGNING request decoder" $ do
            ...
            forM_ invalidWitnessRequests $ \(label, request) ->
                it ("rejects " <> label)
                    $ decodeDappWitnessSignRequest request
                    `shouldSatisfy` isLeft

        it "authenticates request bindings before any context conflict" $ do
            let original =
                    [ ApiDappHex acceptedTransaction
                    , ApiDappHex $ fixtureTransactions !! 1
                    ]
        ...
        describe "DAPP_WITNESS_SIGNING" $ do
            it "accepts collateral-only and explicit required-signer proofs" $ do
                let collateral =
                        evaluateObligation mempty (Set.singleton proofHash)
            ...
            it
                "permits incomplete partial items but rejects an atomic batch failure"
                $ do
                    let inventory =
                            ProofInventory
                                []
        ...
        it "rejects unequal envelopes with the same transaction id" $ do
            let alternateEnvelope =
                ...
                decoded =
                    mapM (decodeTx . ApiDappHex) [acceptedTransaction, alternateEnvelope]
        ...
        it
            "preserves shared spending/collateral inputs and their pending claim"
            $ do
                let transaction =
                        hex
        ...
        it "recognizes supported Conway certificate constructors" $ do
            ...
            mapM_
                ...
                [
                    ( ConwayTxCertDeleg $ ConwayRegCert stakeCredential Strict.SNothing
                    , True
                    )
                ,
                    ( ConwayTxCertDeleg $ ConwayUnRegCert stakeCredential Strict.SNothing
                    , True
                    )
                ,
                    ( ConwayTxCertGov
        ...
        it "matches all frozen record and Blake2b-256 goldens" $ do
            ...
            encodeContextRecord requiredProofRecord
                `shouldBe` Right requiredProofGolden
        ...
        it "matches the token golden and rejects changed MACs and bindings" $ do
            ...
            mapM_
                ( \claims ->
                    fmap (validateContextToken key claims) encoded `shouldBe` Right False
                )
                [ changedClaims
                , ContextTokenClaims
        ...
        it "ORs requiredness across obligations sharing a frozen row key" $ do
            ...
            Map.lookup proofHash result.satisfiedWithoutCandidate
                `shouldBe` Just True
            mapM_
                ( \(existing, expected) -> do
                    let witnessResult =
                            evaluateObligation existing mempty
                                $ DirectProofObligation 0 RequiredSignerProof proofHash
        ...
        it
            "associates producible candidates only with applicable transactions"
            $ do
                let obligations =
                        [ DirectProofObligation 0 RequiredSignerProof proofHash
        ...
        it
            "evaluates all, any, threshold, and time native-script requiredness"
            $ do
                let keyA = witnessKeyHash proofHash
                    ...
                    scripts =
                        [
                            ( mkRequireAllOfTimelock $ StrictSeq.fromList [signatureA, signatureB]
                            , False
                            )
                        ,
                            ( mkRequireAnyOfTimelock $ StrictSeq.fromList [signatureA, signatureB]
                            , True
                            )
                        ,
                            ( mkRequireMOfTimelock 1 $ StrictSeq.fromList [signatureA, signatureB]
                            , True
                            )
                        ,
                            ( mkRequireAllOfTimelock
                    ...
                    validity =
                        ValidityInterval (Strict.SJust $ SlotNo 10) (Strict.SJust $ SlotNo 20)
                    evaluate script =
                        evaluateObligation
                            mempty
                ...
                mapM_
                    ( \(script, remainsSatisfied) -> do
                        let result = evaluate script
                        result.satisfied `shouldBe` True
                        Map.lookup proofHash result.satisfiedWithoutCandidate
                            `shouldBe` Just remainsSatisfied
                    )
                    scripts
        ...
        it
            "validates pending provenance in both directions and uses authority for dependencies"
            $ do
                validatePendingProvenance True [Pending] `shouldBe` Right ()
                validatePendingProvenance False [Node] `shouldBe` Right ()
                validatePendingProvenance True [Node]
                    `shouldBe` Left "pending provenance mismatch"
                validatePendingProvenance False [Pending]
                    `shouldBe` Left "pending provenance mismatch"
                ...
                mapM_
                    ( \(earlier, pending, _node, expected) ->
...
fullOutputRecord =
    FullOutputRecord
        ( ApiDappOutpoint
            ( ApiDappHex
                $ hex "35eb18701459203ce42156af66c76e3f00b9e2a1b9d48192fbbf0d233a652c9f"
            )
            0
        )
        [Earlier, Node]
        ...
        None
        ( hex
            "82581d60aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa1a000f4240"
        )
...
reviewDiscovery =
    mkSeqStateFromRootXPrv
        ShelleyKeyS
        ( RootCredentials
            ( Shelley.unsafeGenerateKeyFromSeed
...
reviewNetwork :: ApiDappContextNetwork
reviewNetwork =
    ApiDappContextNetwork
        1
        764824073
...
outputOnlyOwnership
    :: ByteString -> [(ApiDappOwnershipKind, [ApiDappProofKind])]
outputOnlyOwnership address =
...
resolvedInput =
  ...
  where
    source =
        hex
            "82581d61aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa1a000f4240"
