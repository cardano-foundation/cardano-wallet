...
instance FromJSON ApiDappDataSignRequest where
    parseJSON value = do
        ...
        unless (revision == 1) $ fail "revision must be 1"
        unless
            ( BS.length (getApiDappHex address) == 28
                || BS.length (getApiDappHex address) >= 29
                    && BS.length (getApiDappHex address) <= 59
            )
...
instance FromJSON ApiDappDataSignResponse where
    parseJSON value = do
        ...
        unless
            ( credentialKind
                `elem` [PaymentCredential, StakeCredential, DRepCredential]
            )
...
instance FromJSON ApiDappCip95KeyState where
    parseJSON value = do
        ...
        mapM_
            (requireLength "registered_stake_public_keys" 32)
            registeredStakePublicKeys
        mapM_
            (requireLength "unregistered_stake_public_keys" 32)
            unregisteredStakePublicKeys
        pure result
...
instance FromJSON ApiDappProvenance where
    parseJSON =
        parseEnum
            "provenance"
            [("earlier", Earlier), ("pending", Pending), ("node", Node)]
instance ToJSON ApiDappProvenance where
    toJSON =
        enumJson [(Earlier, "earlier"), (Pending, "pending"), (Node, "node")]
...
instance FromJSON ApiDappRequiredWalletProof where
    parseJSON value = do
        result@ApiDappRequiredWalletProof{transactionIndex, credential} <-
            genericParseJSON strictRecordTypeOptions value
        unless (transactionIndex <= 49)
            $ fail "transaction_index must be 0 to 49"
        requireLength "credential" 28 credential
...
instance FromJSON ApiDappDependency where
    parseJSON value = do
        ...
        unless (transactionIndex <= 49)
            $ fail "transaction_index must be 0 to 49"
        unless (inputRole `elem` [Normal, Collateral, Reference])
            $ fail "invalid dependency role"
        unless (source `elem` [Earlier, Pending])
            $ fail "invalid dependency source"
        unless
            ((source == Earlier) == maybe False (const True) sourceTransactionIndex)
            $ fail "invalid source transaction index"
        unless (maybe True (< transactionIndex) sourceTransactionIndex)
            $ fail "source transaction index must be earlier"
        pure result
...
instance FromJSON ApiDappConflict where
    parseJSON value = do
        result@ApiDappConflict{transactionIndex, inputRole, earlierTransactionIndex} <-
            genericParseJSON strictRecordTypeOptions value
        unless
            (transactionIndex <= 49 && earlierTransactionIndex < transactionIndex)
            $ fail "invalid conflict transaction index"
        unless (inputRole `elem` [Normal, Collateral])
            $ fail "invalid conflict role"
        pure result
...
instance FromJSON ApiDappBatchOverlay where
    parseJSON value = do
        result@ApiDappBatchOverlay{dependencies, conflicts} <-
            genericParseJSON strictRecordTypeOptions value
...
instance FromJSON ApiDappTransactionContextResponse where
    parseJSON value = do
        ...
        requireUniqueBy
            "ownership"
            ( \ApiDappOwnership{credentialKind, credential} -> (credentialKind, credential)
            )
            ownership
        ...
        requireUniqueBy
            ...
            ( \ApiDappRequiredWalletProof
                ...
                , credentialKind
                , credential
                } -> (transactionIndex, proofKind, credentialKind, credential)
            )
            requiredWalletProofs
        requireUniqueBy
            "dependencies"
            ( \ApiDappDependency{transactionIndex, inputRole, outpoint} -> (transactionIndex, inputRole, outpoint)
            )
            dependencies
        requireUniqueBy
            "conflicts"
            ( \ApiDappConflict{transactionIndex, inputRole, outpoint} -> (transactionIndex, inputRole, outpoint)
            )
            conflicts
...
encodeProvenance values = do
    ...
    pure
        $ sum
        $ map (\case Earlier -> 0x01; Pending -> 0x02; Node -> 0x04) values
...
requireUniqueBy
    :: (MonadFail m, Ord b) => String -> (a -> b) -> [a] -> m ()
requireUniqueBy label project values =
    when (hasDuplicates $ sort $ project <$> values)
        $ fail
        $ label <> " has duplicate uniqueness keys"

hasOwnedCandidate
    :: [ApiDappOwnership] -> ApiDappRequiredWalletProof -> Bool
  ...
  where
    matches
        ApiDappOwnership
            ...
            candidateKind == credentialKind
                && candidateCredential == credential
                && proofKind `elem` proofKinds
    matches _ = False
