...
scriptTemplateFromSelf
    :: XPub -> ApiScriptTemplateEntry -> ScriptTemplate
...
mkLegacyWallet ctx wid cp meta _ pending progress = do
  ...
  where
    ...
    matchEmptyPassphrase
        :: DBLayer IO s -> Handler (Either ErrWithRootKey ())
...
listWallets ctx mkApiWallet = do
    ...
    liftIO
        $ sortOn snd . catMaybes <$> mapM maybeGetWallet (ApiT <$> wids)
  where
    df = ctx ^. dbFactory @s
...
getWalletUtxoSnapshot ctx (ApiT wid) = do
  ...
  where
    ...
    mkApiWalletUtxoSnapshotEntry bundle =
        ApiWalletUtxoSnapshotEntry
    ...
    withWorkerCtx ctx walletId liftE liftE $ \workerCtx -> do
        ...
        pure
            ApiCoinSelection
                ...
                , certificates =
                    uncurry mkApiCoinSelectionDelCerts
                        <$> delegationAction
...
getAsset ctx wid (ApiT policyId) (ApiT assetName) = do
    ...
    liftIO
        $ runIdentity <$> fillMetadata client (Identity assetId) toApiAsset
  where
    findAsset =
...
postTransactionFeeOld ctx@ApiLayer{..} (ApiT walletId) body = do
    ...
    withWorkerCtx ctx walletId liftE liftE $ \workerCtx -> do
        let db = workerCtx ^. dbLayer
        ttl <-
            liftIO $ W.transactionExpirySlot (timeInterpreter netLayer) mTTL
        wdrl <- case body ^. #withdrawal of
            Nothing -> pure NoWithdrawal
            Just apiWdrl ->
                shelleyOnlyMkWithdrawal
        ...
        let outputs = F.toList $ addressAmountToTxOut <$> body ^. #payments
            minCoins =
                W.calcMinimumCoinValues pp txLayer
                    <$> outputs
        feePercentiles <-
            liftIO
                $ W.transactionFee @s
...
constructTransaction api knownPools poolStatus apiWalletId body = do
    ...
    when
        (isJust (body ^. #encryptMetadata) && isNothing (body ^. #metadata))
        $ liftHandler
    ...
    withWorkerCtx api walletId liftE liftE $ \wrk -> do
        ...
        (optionalDelegationAction, optionalVoteAction) <-
            liftIO
                $ IODeleg.handleDelegationVoteRequest
                    wrk
            ...
            Just policyXPub ->
                pure
                    $ replaceCosigner
                        ...
                        <$> mintBurnReferenceScriptTemplate
            Nothing ->
                if isJust mintBurnReferenceScriptTemplate
                    then
        ...
        let mintWithAddress mb = case mb ^. #mintBurnData of
                Left
                    (ApiMintBurnDataFromScript _ _ (ApiMint (ApiMintData (Just _) _))) ->
                        True
                Right
                    (ApiMintBurnDataFromInput _ _ _ (ApiMint (ApiMintData (Just _) _))) ->
                        True
                _ -> False
    ...
    parseMintBurnData tx validity = do
      ...
      where
        ...
        handleMissingAssetName mb = case mb ^. #mintBurnData of
          ...
          where
            updateFromScript
                :: ApiMintBurnDataFromScript n -> ApiMintBurnDataFromScript n
            updateFromScript mbd = case mbd ^. #assetName of
                Nothing -> mbd{assetName = Just (ApiT AssetName.empty)}
                Just _ -> mbd
            updateFromInp
                :: ApiMintBurnDataFromInput n -> ApiMintBurnDataFromInput n
        ...
        guardAssetNameTooLong mbs =
          ...
          where
            assetNameTooLong mb = case mb ^. #mintBurnData of
                Left
                    (ApiMintBurnDataFromScript _ (Just (ApiT (UnsafeAssetName bs))) _) ->
                        BS.length bs > AssetName.maxLength
                Right
                    (ApiMintBurnDataFromInput _ _ (Just (ApiT (UnsafeAssetName bs))) _) ->
                        BS.length bs > AssetName.maxLength
...
toUnsignedTxOut = \case
    WalletOutput o ->
        ...
        in  TxOut address (TokenBundle coin assets)
    ExternalOutput o ->
        let address = apiAddress (o ^. #address)
            coin = ApiAmount.toCoin (o ^. #amount)
            assets = ApiWalletAssets.toTokenMap (o ^. #assets)
...
toUnsignedTxInp = \case
    WalletInput i ->
        ...
        in  Just (txIn, txOut, derivationPath)
    ExternalInput _ ->
        Nothing
...
toUnsignedTxChange = \case
    WalletOutput o ->
        let address = apiAddress (o ^. #address)
            ...
            coin = ApiAmount.toCoin (o ^. #amount)
            assets = ApiWalletAssets.toTokenMap (o ^. #assets)
        in  TxChange address coin assets derivationPath
    ExternalOutput _ ->
    ...
    ( txinsOutsPaths
        ...
        withWorkerCtx ctx wid liftE liftE $ \wrk -> do
            ...
            let rewardAcctM = case scriptM of
                    ...
                    Nothing -> Nothing
            let certs =
                    mkApiAnyCertificate rewardAcctM rewardAcctPath
                        <$> certificates
            pure
                ( inputPaths
                , collateralInputPaths
...
isForeign apiDecodedTx =
    ...
    in  all isInpForeign generalInps
...
getBlocksLatestHeader
    :: NetworkLayer IO block -> Handler ApiBlockHeader
...
mkApiCoinSelectionOutput
    :: forall n. TxOut -> ApiCoinSelectionOutput n
    ...
    timeRef <-
        liftIO
            $ (#time .~ tx ^. #txTime)
                <$> makeApiBlockReference
                    ( neverFails
                        "makeApiBlockReference shouldn't fail getting the time of             \transactions with slots in the past"
                        timeInterpreter
                    )
                    (tx ^. #txMeta . #slotNo)
...
getDappCip95KeyState
    :: forall n. ApiLayer (SeqState n ShelleyKey)
    -> ApiT WalletId
  ...
  where
    decodeCertificates info = do
        ...
        DecodedTx{transaction = Read.Tx ledgerTx, valid} <-
            either (const Nothing) Just $ decodeDappTx (ApiDappHex $ BL.toStrict bytes)
        pure (valid, F.toList $ ledgerTx ^. Ledger.bodyTxL . Ledger.certsTxBodyL)
...
stakeRegistrationEffects stakeHash =
  ...
  where
    effect = \case
        ConwayTxCertDeleg (ConwayRegCert (KeyHashObj (LedgerKeys.KeyHash hash)) _)
            | Crypto.hashToBytes hash == stakeHash -> Just W.RegisterStakeKey
        ConwayTxCertDeleg (ConwayRegDelegCert (KeyHashObj (LedgerKeys.KeyHash hash)) _ _)
            | Crypto.hashToBytes hash == stakeHash -> Just W.RegisterStakeKey
        ConwayTxCertDeleg (ConwayUnRegCert (KeyHashObj (LedgerKeys.KeyHash hash)) _)
...
postDappDataSignature ctx (ApiT walletId) request = do
    ...
    (kind, credential, rawAddress) <-
        either throwDapp pure $ validateDataSignRequest expectedNetwork request
    case credential of
                ...
                result <- liftIO $ runExceptT $ do
                    ...
                    (kind', path, expectedCredential, protectedAddress) <-
                        either (const $ throwE DappDataProofGenerationError) pure
                            $ ownedDataPath kind rawAddress credentialBytes
                                (getState checkpoint)
                    let protected = encodeProtectedDataAddress protectedAddress
                        signatureStructure =
                            encodeSignatureStructure protected
...
validateDataSignRequest expected ApiDappDataSignRequest{network, address = ApiDappHex rawAddress}
    ...
    | otherwise = do
        (header, body) <- maybe (Left InvalidDappRequest) Right $ BS.uncons rawAddress
        ...
        if networkTag /= expected.networkId then Left InvalidDappRequest else case tag of
            ...
            4 | BS.length body >= 31 && canonicalPointer (BS.drop 28 body) ->
                key PaymentCredential (BS.take 28 body)
            5 | BS.length body >= 31 && canonicalPointer (BS.drop 28 body) ->
...
ownedDataPath kind rawAddress expected state =
    case classifyDRepDataCredential kind rawAddress expected drepHash of
        ...
        Right False -> case kind of
            ...
            StakeCredential ->
                let path = map getDerivationIndex
                        $ NE.toList $ stakeDerivationPath $ Seq.derivationPrefix state
                    stakeKey = deriveAddressPublicKey (Seq.accountXPub state) MutableAccount minBound
                in if blake2b224 (xpubPublicKey $ getRawKey ShelleyKeyS stakeKey) == expected
  ...
  where
    payment = do
        path <- maybe (Left ()) Right $ fst $ isOurs (Address rawAddress) state
        ...
        case indexes of
            [0x8000073c, 0x80000717, account, role, address]
                | account == getIndex accountIndex
                    ...
                    && blake2b224
                        (xpubPublicKey $ getRawKey ShelleyKeyS
                            $ deriveAddressPublicKey (Seq.accountXPub state)
    ...
    drepPath = map getDerivationIndex
        $ NE.toList $ drepDerivationPath $ Seq.derivationPrefix state
    drepKey = deriveDRepPublicKey $ getRawKey ShelleyKeyS (Seq.accountXPub state)
    drepHash = blake2b224 $ xpubPublicKey drepKey
    drep
        | expected == drepHash = Right (DRepCredential, drepPath, drepHash, drepHash)
        | otherwise = Left ()
...
encodeProtectedDataAddress address =
    CBOR.toStrictByteString
        $ CBOR.encodeMapLen 2
            ...
            <> CBOR.encodeString "address" <> CBOR.encodeBytes address
...
encodeUnprotectedData =
    CBOR.encodeMapLen 2
        <> CBOR.encodeString "hashed" <> CBOR.encodeBool False
...
mkDappDataSignResponse kind credential address payload protected signatureStructure (publicKeyBytes, signature)
    ...
    | BS.length publicKeyBytes /= 32 || BS.length signature /= 64 = Left "invalid key material"
    | blake2b224 publicKeyBytes /= credential = Left "public key hash mismatch"
    | otherwise =
        let coseSign1 =
                CBOR.toStrictByteString
                    $ CBOR.encodeListLen 4
    ...
    toServerError = \case
        ...
        ErrCreateWalletRestorationFromABlockFailed
            (ErrNoBlockAt (Read.BlockPoint (Read.SlotNo slot) block)) ->
                apiError err404 UnexpectedError
                    $ mconcat
                        [ "Restoration from a given block failed. "
