...
import Cardano.Wallet.Primitive.Types.Certificates
    ...
    , NonWalletCertificate (GenesisCertificate, MIRCertificate)
    )
...
decodeTx (ApiDappHex bytes) = do
    TxWithOutputBytes{transaction, outputsWithBytes} <-
        first show
            $ deserializeTxWithOutputBytes @Read.Conway
            $ BL.fromStrict bytes
...
decodeDappTx :: ApiDappHex -> Either DappError DecodedTx
decodeDappTx value@(ApiDappHex bytes) =
  ...
  where
    hasDeprecatedCertificate source = case deserializeTx (BL.fromStrict source)
        ...
        Right transaction ->
            any deprecatedCertificate
                $ getCertificates
                $ getEraCertificates transaction
        Left _ -> False
    deprecatedCertificate = \case
...
validVKeyWitnessHashes (Read.Tx ledgerTx) =
    Set.fromList
  ...
  where
    bodyHash =
        extractHash
            $ hashAnnotated @_ @EraIndependentTxBody
            $ ledgerTx ^. bodyTxL
...
resolveOutput available roleMap earlier pending node input = do
    let sources =
            catMaybes
                [ (\(output, bytes) -> (Earlier, output, bytes))
                    <$> Map.lookup input earlier
                , (\(output, bytes) -> (Pending, output, bytes))
                    <$> Map.lookup input pending
    ...
    (provenance, output, source) <- case sources of
        ...
        ((_, firstOutput, firstBytes) : _) -> do
            requireEither DappContextConflictError
                $ all (\(_, _, bytes) -> bytes == firstBytes) sources
            requireEither DappInternalErrorResponse
                $ all (\(_, value, _) -> value == firstOutput) sources
        ...
        case ledgerOutput ^. addrTxOutL of
            ...
            address@(Addr _ credential _) -> case credential of
                ScriptHashObj (Ledger.ScriptHash hash) ->
                    pure
                        (discovery, insert (Crypto.hashToBytes hash) ScriptOwned [] [] found)
                KeyHashObj (LedgerKeys.KeyHash hash) ->
                    ...
                    in  do
                            ...
                            pure
                                (discovery', insert credentialBytes ownership indexes proofs found)
      where
        proofs =
        ...
        insert credential ownership derivationPath proofKinds =
            Map.insertWith
                merge
                (credential, ownership, derivationPath)
                ApiDappOwnership
        ...
        merge
            ApiDappOwnership{proofKinds = newProofs}
            ApiDappOwnership
                { credentialKind
                , credential
                , ownership
                , derivationPath
                , proofKinds = oldProofs
                } =
                ApiDappOwnership
                    credentialKind
                    credential
...
verifiesPaymentPath
    :: SeqState n ShelleyKey -> [Word32] -> ByteString -> Bool
        ...
        let role = if roleIndex == 0 then UtxoExternal else UtxoInternal
            child =
                deriveAddressPublicKey
                    ...
                    role
                    (Index addressIndex)
        in  blake2b224 (xpubPublicKey $ getRawKey ShelleyKeyS child)
                == expectedCredential
  where
...
buildProofInventory configured discovery requested resolved = do
    ...
    (discoveryAfterSigners, signers) <-
        signerEvidence configured discoveryAfterPayment requested
    (native, nativeObligations) <-
        nativeEvidence configured discoveryAfterSigners requested resolved
    ...
    candidates <-
        mapM (uncurry $ candidateFromOwnership discovery)
...
proofInventory requested obligations candidates ownership =
  ...
  where
    existing =
        Map.fromList
            [ (index, validVKeyWitnessHashes tx.transaction)
            | (index, tx) <- zip [0 :: Word32 ..] requested
            ]
    producible =
        Map.fromListWith
            (<>)
            [ ( candidate.candidateTransactionIndex
              , Set.singleton candidate.candidateKeyHash
              )
    ...
    results =
        ( \obligation ->
            evaluateObligation
                ...
                ( Map.findWithDefault
                    mempty
                    (obligationTransactionIndex obligation)
                    producible
                )
                obligation
        )
    ...
    aggregate =
        Map.unionWith
            (&&)
            ( Map.fromList
                [(index, True) | (index, _) <- zip [0 :: Word32 ..] requested]
            )
            ( Map.fromListWith
                (&&)
                [ (obligationTransactionIndex result.obligation, result.satisfied)
                | result <- results
                ]
            )
...
evaluateObligation existing producible obligation =
    ProofObligationResult obligation keys baseline
        $ Map.fromSet
            ( \keyHash -> evaluate $ existingForTransaction <> Set.delete keyHash producible
            )
            producible
  where
    ...
    evaluate hashes = case obligation of
        ...
        NativeProofObligation _ _ script validity ->
            evalTimelock
                ( Set.filter (\key -> keyBytes key `Set.member` hashes)
                    $ timelockKeys script
                )
                validity
                script
...
requiredProofs ownership results =
    sort
        $ Map.elems
        $ Map.fromListWith
            combine
                    ...
                    ( result.satisfied
                        && not
                            ( Map.findWithDefault
                                result.satisfied
                                credential
                                result.satisfiedWithoutCandidate
                            )
                    )
              )
            | ApiDappOwnership
...
directProofObligations requested resolved =
    fmap concat
        $ mapM perTransaction
        ...
        fmap catMaybes
            $ mapM
                ( \input -> do
                    ...
                    fmap (DirectProofObligation transactionIndex proofKind)
                        <$> paymentKeyCredential output
                )
            $ Set.toList inputs
...
mergeOwnership
    :: [ApiDappOwnership] -> Either DappError [ApiDappOwnership]
mergeOwnership values = do
  ...
  where
    grouped =
        Map.fromListWith
            (<>)
            [ ( (value.credentialKind, value.credential)
              , [(value.ownership, value.derivationPath)]
              )
            | value <- values
            ]
    consistent [] = True
  ...
  where
    ...
    stakePath =
        map getDerivationIndex
            $ NE.toList
            $ stakeDerivationPath
            $ Seq.derivationPrefix initial
    policy =
        ( \key ->
            ( blake2b224 $ xpubPublicKey $ getRawKey ShelleyKeyS key
            , [0x8000073f, 0x80000717, 0x80000000]
            )
        )
            <$> Seq.policyXPub initial
...
nativeEvidence configured initial requested resolved = do
    (_, ownership, obligations) <-
        foldM perTransaction (initial, [], [])
            $ zip [0 :: Word32 ..] requested
  ...
  where
    ...
    perTransaction
        (discovery, ownership, obligations)
        ( transactionIndex
            , DecodedTx
            ...
            let scriptMap = Map.union (ledgerTx ^. witsTxL . scriptTxWitsL) referenceScriptMap
                mintScripts =
                    [ (PolicyCredential, PolicyProof, hash)
                    | PolicyID hash <-
            ...
            spendingScripts <-
                fmap catMaybes
                    ...
                    $ normal <> collateral
            foldM
                (perScript transactionIndex ledgerTx scriptMap)
                (discovery, ownership, obligations)
                $ mintScripts
                    <> [ (PaymentCredential, NativeScriptProof, hash) | hash <- spendingScripts
                       ]
        ...
        case getNativeScript script of
            ...
            Just timelock -> do
                (discovery', candidates) <-
                    foldM (matchLeaf proofKind) (discovery, [])
                        $ Set.toList
                        $ timelockKeys timelock
    ...
    matchLeaf proofKind (discovery, candidates) keyHash = do
        ...
        pure
            ( discovery'
            , (keyHash, paymentRows <> stakeRows <> policyRows) : candidates
            )

timelockKeys
    :: Timelock Read.Conway -> Set (LedgerKeys.KeyHash LedgerKeys.Witness)
timelockKeys script
    | Just key <- getRequireSignatureTimelock script = Set.singleton key
    | Just scripts <- getRequireAllOfTimelock script =
        Set.unions $ timelockKeys <$> toList scripts
    | Just scripts <- getRequireAnyOfTimelock script =
        Set.unions $ timelockKeys <$> toList scripts
...
validatePendingProvenance
    :: Bool -> [ApiDappProvenance] -> Either String ()
validatePendingProvenance pending provenance =
    unless (pending == (Pending `elem` provenance))
        $ Left "pending provenance mismatch"

paymentKeyCredential
    :: Output Read.Conway -> Either DappError (Maybe ByteString)
...
buildBatchOverlay requested pendingTransactions pending = do
    requireEither InvalidDappRequest
        $ duplicateTransactionsAreIdentical requested
  ...
  where
    ...
    pendingClaims =
        Set.unions
            $ concatMap (\tx -> [tx.normal, tx.collateral]) pendingTransactions
    ...
    step (priorOutputs, consumed, dependencies, conflicts) (txIndex, tx) = do
        ...
        let priorOutputs' = Map.union priorOutputs tx.outputs
            consumed' =
                foldl' (\m input -> Map.insertWith min input txIndex m) consumed
                    $ Set.toList tx.normal
    ...
    inputStep txIndex priorOutputs consumed (dependencies, conflicts) (role, input) = do
        let earlier = Map.member input priorOutputs
            ...
            dependency = case dependencySource earlier pendingSource of
                Just Earlier ->
                    Just
                        $ ApiDappDependency txIndex role (toOutpoint input) Earlier sourceIndex
                Just Pending ->
                    Just
                        $ ApiDappDependency txIndex role (toOutpoint input) Pending Nothing
                _ -> Nothing
            conflict = case (role, Map.lookup input consumed) of
                (Normal, Just earlierIndex) ->
                    Just $ ApiDappConflict txIndex role (toOutpoint input) earlierIndex
                (Collateral, Just earlierIndex) ->
                    Just $ ApiDappConflict txIndex role (toOutpoint input) earlierIndex
                _ -> Nothing
            Ledger.TxIn inputId _ = input
        requireEither InvalidDappRequest
            $ earlier || not (txIdBytes inputId `Set.member` requestedIds)
        requireEither DappContextConflictError
            $ role == Reference || input `Set.notMember` pendingClaims
        pure
            ( maybe dependencies (: dependencies) dependency
            , maybe conflicts (: conflicts) conflict
            )
...
validateTransactionContextResponseForRequest request response = do
    ...
    unless (duplicateTransactionsAreIdentical requested)
        $ Left "duplicate transaction envelopes differ"
    ...
    let outputsByOutpoint = Map.fromList [(value.outpoint, value) | value <- response.outputs]
        ...
        requestedClaims =
            Set.unions $ concatMap (\tx -> [tx.normal, tx.collateral]) requested
    unless (Set.disjoint pendingClaims requestedClaims)
        $ Left "pending spending conflict"
    mapM_
        (validateInput requested roleMap pendingOutputs outputsByOutpoint)
        $ Set.toList
        $ Set.unions
        $ concatMap (\tx -> [tx.normal, tx.collateral, tx.reference]) requested
    (_, _, dependencies, conflicts) <-
        foldM
            (step requested pendingOutputs requestedIds)
            (mempty, mempty, [], [])
            $ zip [0 :: Word32 ..] requested
    let expected = ApiDappBatchOverlay (sort dependencies) (sort conflicts)
    unless (response.batchOverlay == expected)
        $ Left "batch overlay does not match request"
  where
    validateInput
        :: [DecodedTx]
        -> Map Ledger.TxIn (Set ApiDappRole)
    ...
    validateInput requested roleMap pendingOutputs outputsByOutpoint input = do
        output <-
            maybe (Left "missing requested output") Right
                $ Map.lookup (toOutpoint input) outputsByOutpoint
        let expectedRoles = sort $ Set.toList $ Map.findWithDefault mempty input roleMap
            ...
            earlierSources =
                ...
                , any
                    ( \consumer ->
                        input
                            `Set.member` (consumer.normal <> consumer.collateral <> consumer.reference)
                    )
                    $ drop (fromIntegral index + 1) requested
                ]
        unless (actualRoles == expectedRoles)
            $ Left "output roles do not match request"
        unless
            ((not $ null earlierSources) == (Earlier `elem` output.provenance))
            $ Left "earlier provenance mismatch"
        mapM_
            ( \source ->
                unless (getApiDappHex output.sourceTransactionOutputCbor == source)
                    $ Left "earlier source bytes mismatch"
            )
            earlierSources
        validatePendingProvenance
            (Map.member input pendingOutputs)
            output.provenance
        case Map.lookup input pendingOutputs of
            Nothing -> pure ()
            Just (_, source) -> do
                unless (Pending `elem` output.provenance)
                    $ Left "pending provenance missing"
                unless (getApiDappHex output.sourceTransactionOutputCbor == source)
                    $ Left "pending source bytes mismatch"
    step
        :: [DecodedTx]
        -> Map Ledger.TxIn (Output Read.Conway, ByteString)
        -> Set ByteString
    ...
    inputStep requested pendingOutputs requestedIds txIndex priorOutputs consumed (dependencies, conflicts) (role, input) = do
        let earlier = Map.member input priorOutputs
            ...
            conflict = case (role, Map.lookup input consumed) of
                ...
                _ -> Nothing
            Ledger.TxIn inputId _ = input
        unless (earlier || not (txIdBytes inputId `Set.member` requestedIds))
            $ Left "self or forward input"
        pure
            ( maybe dependencies (: dependencies) dependency
            , maybe conflicts (: conflicts) conflict
            )
