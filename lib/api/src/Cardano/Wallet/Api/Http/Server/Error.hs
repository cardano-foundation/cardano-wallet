...
instance IsServerError ErrWalletNotInitialized where
    toServerError = \case
        ErrWalletNotInitialized ->
            apiError err404 WalletNotInitialized
                $ mconcat
...
instance IsServerError ErrWalletNotResponding where
    toServerError = \case
        ErrWalletNotResponding wid ->
            apiError err500 WalletNotResponding
                $ T.unwords
                    [ "That's embarrassing. My associated worker for"
                    , toText wid
...
instance IsServerError ErrSignPayment where
    toServerError = \case
        ...
        ErrSignPaymentWithRootKey e@ErrWithRootKeyNoRootKey{} ->
            (toServerError e)
                { errHTTPCode = 403
...
instance IsServerError ErrWitnessTx where
    toServerError = \case
        ...
        ErrWitnessTxWithRootKey e@ErrWithRootKeyNoRootKey{} ->
            (toServerError e)
                { errHTTPCode = 403
                , errReasonPhrase = errReasonPhrase err403
                }
...
instance IsServerError ErrSignTx where
    toServerError = \case
        ErrSignTxAddressUnknown txin ->
            apiError err500 KeyNotFoundForAddress
                $ mconcat
...
instance IsServerError ErrConstructTx where
    toServerError = \case
        ...
        ErrConstructTxMultidelegationNotSupported ->
            apiError err403 CreatedMultidelegationTransaction
                $ mconcat
        ...
        ErrConstructTxMultiaccountNotSupported ->
            apiError err403 CreatedMultiaccountTransaction
                $ mconcat
                    [ "It looks like I've created a transaction "
        ...
        ErrConstructTxWrongMintingBurningTemplate ->
            apiError err403 CreatedWrongPolicyScriptTemplate
                $ mconcat
        ...
        ErrConstructTxAssetNameTooLong ->
            apiError err403 AssetNameTooLong
                $ mconcat
        ...
        ErrConstructTxMintOrBurnAssetQuantityOutOfBounds ->
            apiError err403 MintOrBurnAssetQuantityOutOfBounds
                $ mconcat
        ...
        ErrConstructTxWrongValidityBounds ->
            apiError err403 InvalidValidityBounds
                $ T.unwords
        ...
        ErrConstructTxValidityIntervalNotWithinScriptTimelock ->
            apiError err403 ValidityIntervalNotInsideScriptTimelock
                $ T.unwords
        ...
        ErrConstructTxSharedWalletIncomplete ->
            apiError err403 SharedWalletIncomplete
                $ T.unwords
        ...
        ErrConstructTxDelegationInvalid ->
            apiError err403 DelegationInvalid
                $ T.unwords
        ...
        ErrConstructTxWithdrawalWithoutVoting ->
            apiError err403 WithdrawalNotPossibleWithoutVote
                $ mconcat
        ...
        ErrConstructTxFromMetadataEncryption ErrIncorrectRawMetadata ->
            apiError err403 InvalidMetadataEncryption
                $ mconcat
                    [ "It looks like the metadata does not "
                    , "have `msg` field that is supposed to be encrypted."
                    ]
        ErrConstructTxFromMetadataEncryption
            (ErrCannotEncryptMetadata cryptoError) ->
                apiError err403 InvalidMetadataEncryption
                    $ mconcat
...
instance IsServerError ErrDecodeTx where
    toServerError = \case
        ...
        ErrDecodeTxFromMetadataDecryption
            (ErrCannotDecryptPayload cryptoError) ->
                apiError err403 InvalidMetadataDecryption
                    $ mconcat
...
instance
    (Write.IsRecentEra era, IsServerError (ErrAssignRedeemers era))
    => IsServerError (ErrBalanceTx era)
    where
    toServerError = \case
        ErrBalanceTxExistingKeyWitnesses n ->
            apiError err403 BalanceTxExistingKeyWitnesses
                $ mconcat
        ...
        ErrBalanceTxAssetsInsufficient e ->
            apiError err403 (NotEnoughMoney info)
                $ mconcat
        ...
        ErrBalanceTxMaxSizeLimitExceeded _ _ ->
            apiError err403 TransactionIsTooBig
                $ T.unwords
        ...
        ErrBalanceTxUnresolvedRefunds creds ->
            apiError err500 UnresolvedRefunds
                $ T.unwords
        ...
        ErrBalanceTxInputResolutionConflicts conflicts -> do
            ...
            apiError err400 InputResolutionConflicts
                $ mconcat
        ...
        ErrBalanceTxUnableToCreateInput ->
            apiError err403 NoUtxosAvailable
                $ T.unwords
        ...
        ErrBalanceTxUnableToCreateChange e ->
            apiError err403 CannotCoverFee
                $ T.unwords
...
instance Write.IsRecentEra era => IsServerError (ErrBalanceTxInternalError era) where
    toServerError = \case
        ErrUnderestimatedFee coin' candidateTx keyWitnessCounts ->
            apiError err500 (BalanceTxUnderestimatedFee info)
                $ T.unwords
                    [ "I have somehow underestimated the fee of the transaction by"
                    , pretty (toWalletCoin coin')
        ...
        ErrFailedBalancing v ->
            apiError err500 BalanceTxInternalError
                $ T.unwords
...
instance IsServerError ErrRemoveTx where
    toServerError = \case
        ErrRemoveTxNoSuchTransaction (ErrNoSuchTransaction tid) ->
            flip (apiError err404) message
                $ NoSuchTransaction
                    ApiErrorNoSuchTransaction{transactionId = ApiT tid}
          where
            message =
                mconcat
...
instance IsServerError ErrSubmitTransaction where
    toServerError = \case
        ErrSubmitTransactionForeignWallet ->
            apiError err403 ForeignTransaction
                $ mconcat
...
instance IsServerError ErrCannotJoin where
    toServerError = \case
        ErrAlreadyDelegating pid ->
            apiError err403 PoolAlreadyJoined
                $ mconcat
        ...
        ErrAlreadyDelegatingVoting pid ->
            apiError err403 PoolAlreadyJoinedSameVote
                $ mconcat
                    [ "I couldn't join a stake pool with the given id: "
                    , toText pid
...
instance IsServerError ErrCannotVote where
    toServerError = \case
        ...
        ErrWrongEra ->
            apiError err403 VotingInInvalidEra
                $ mconcat
                    [ "I couldn't cast a vote. "
...
instance IsServerError ErrCannotQuit where
    toServerError = \case
        ErrNotDelegatingOrAboutTo ->
            apiError err403 NotDelegatingTo
                $ mconcat
        ...
        ErrNonNullRewards rewards ->
            apiError err403 NonNullRewards
                $ mconcat
                    [ "It seems that you're trying to retire from delegation "
                    , "although you've unspoiled rewards in your rewards "
                    , "account! Make sure to withdraw your "
                    , pretty rewards
                    , " lovelace first."
                    ]
...
instance IsServerError ErrFetchRewards where
    toServerError = \case
        ...
        ErrFetchRewardsMissingRewardAccount ->
            apiError err500 MissingRewardAccount
                $ mconcat
...
instance IsServerError ErrReadRewardAccount where
    toServerError = \case
        ...
        ErrReadRewardAccountMissing ->
            apiError err501 MissingRewardAccount
                $ mconcat
...
instance IsServerError ErrReadPolicyPublicKey where
    toServerError = \case
        ErrReadPolicyPublicKeyNotAShelleyWallet ->
            apiError err403 InvalidWalletType
                $ mconcat
        ...
        ErrReadPolicyPublicKeyAbsent ->
            apiError err403 MissingPolicyPublicKey
                $ T.unwords
...
instance IsServerError ErrCreateRandomAddress where
    toServerError = \case
        ...
        ErrIndexAlreadyExists ix ->
            apiError err409 AddressAlreadyExists
                $ mconcat
                    [ "I cannot derive a new unused address #"
                    , pretty (fromEnum ix)
...
instance IsServerError ErrImportRandomAddress where
    toServerError = \case
        ...
        ErrImportAddr ErrAddrDoesNotBelong{} ->
            apiError err403 KeyNotFoundForAddress
                $ mconcat
...
instance IsServerError ErrWithdrawalNotBeneficial where
    toServerError = \case
        ErrWithdrawalNotBeneficial ->
            apiError err403 WithdrawalNotBeneficial
                $ mconcat
...
instance IsServerError ErrAddCosignerKey where
    toServerError = \case
        ErrAddCosignerKeyWalletMetadataNotFound ->
            apiError err503 WalletMetadataNotFound
                $ T.unwords
        ...
        ErrAddCosignerKey WalletAlreadyActive ->
            apiError err403 SharedWalletActive
                $ T.unwords
        ...
        ErrAddCosignerKey NoDelegationTemplate ->
            apiError err403 SharedWalletNoDelegationTemplate
                $ T.unwords
        ...
        ErrAddCosignerKey (KeyAlreadyPresent cred) ->
            apiError err403 SharedWalletKeyAlreadyExists
                $ T.unwords
        ...
        ErrAddCosignerKey (NoSuchCosigner credType (Cosigner cosignerIndex)) ->
            ...
            in  apiError err403 errorInfo errorMessage
        ErrAddCosignerKey CannotUpdateSharedWalletKey ->
            apiError err403 SharedWalletCannotUpdateKey
                $ T.unwords
...
instance IsServerError ErrConstructSharedWallet where
    toServerError = \case
      ...
      where
        handleTemplateErr cred reason =
            apiError err403 SharedWalletScriptTemplateInvalid
                $ mconcat
                    [ "It looks like you've tried to create a shared wallet "
                    , "with a template script for "
                    , toText cred
                    , " credential that does not "
                    , "pass validation. The problem is: "
                    , reason
                    ]
...
instance IsServerError (ErrInvalidDerivationIndex 'Soft level) where
    toServerError = \case
        ErrIndexOutOfBound minIx maxIx _ix ->
            apiError err403 SoftDerivationRequired
                $ mconcat
...
instance IsServerError ErrCreateMigrationPlan where
    toServerError = \case
        ErrCreateMigrationPlanEmpty ->
            apiError err403 NothingToMigrate
                $ mconcat
...
instance
    ...
    where
    toServerError e =
        apiError err403 InsufficientCollateral
            $ T.unwords
...
instance IsServerError (ErrInvalidDerivationIndex 'Hardened level) where
    toServerError = \case
        ErrIndexOutOfBound (Index minIx) (Index maxIx) _ix ->
            apiError err403 HardenedDerivationRequired
                $ mconcat
                    [ "It looks like you've provided a derivation index that is "
                    , "out of bound. The index is well-formed, but I require "
                    , "indexes valid for hardened derivation only. That is, indexes "
                    , "between 0H and "
                    , pretty (Index $ maxIx - minIx)
                    , "H."
                    ]
    ...
    toServerError (req, err@(ServerError code _ body _))
        | not (isJSON body) = case code of
            ...
            406 ->
                ...
                in  apiError err NotAcceptable
                        $ mconcat
                            [ "It seems as though you don't accept '"
                            , cType
                            , "', but "
                            , "unfortunately I only speak '"
                            , cType
                            , "'! Please "
