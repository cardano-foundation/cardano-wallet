...
- [X] T001 Reproduce the RED: run `./gate.sh` unchanged, capture the
      `GATE FAIL: ... still sets a job-level LOCAL_CLUSTER_CONFIGS` line in
      `WIP.md`.
- [X] T002 Delete the `env:` block (`LOCAL_CLUSTER_CONFIGS`) from the
...
- [X] T003 GREEN: `./gate.sh` passes; capture the output in `WIP.md`.
