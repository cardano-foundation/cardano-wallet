---
name: debug
description: Debug CI failures, fetch Buildkite logs, analyze test failures
---

# Debug CI Failures

Use this skill when debugging Buildkite CI failures for cardano-wallet.
...
### Step 3: Check for process kill vs test failures

**First, check if the process was killed** (OOM or timeout):
```bash
# Exit code 137 = killed (128 + 9 SIGKILL)
grep -E "exit code 137|Killed" /tmp/bk-job.log
```

If you see `Killed` in the log, the tests didn't actually fail - the process ran out of memory or hit a timeout. Tests marked with `[✘]` may simply have never run. This is an infrastructure issue, not a code issue - retry the build.

### Step 4: Find test failures
