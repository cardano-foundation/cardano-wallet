...
- **D-106 · the #5424 macOS failure is a PERSISTENT disk condition, and the third
  leg is what makes that actionable.** `cardano-wallet: hPutBuf: resource
  exhausted (No space left on device)` in the nix builder for
  `cardano-wallet-2026.8.21.drv`. Three legs: the error class is a property of the
  **runner**, not of code; the **pushed** diff is six markdown files under
  `specs/` with 0 `.hs` / 0 `.cabal` / 0 `.nix` / 0 workflow, so no causal path
  exists; and **the control — the identical commit `beaf941e88` re-run three days
  later failed again with the same `No space left on device`.** So it is **not a
  flake a re-run clears**: every future run on that runner fails until space is
  reclaimed. **Retrying is not a strategy**, and a desk that retries after proving
  retries do not work is training itself to ignore its own evidence. Routed to
  **cardano-foundation infrastructure** — a CI runner is not this host, so it is
  not the epyc machine owner's disk.

- **D-107 · a control refused is worth more than a control stretched.** The first
  support offered for *"environmental"* was *"`master` fails too."* `master` did
  fail on 09-04 — on **Conway Integration Tests**, a different job class, which
  **licenses nothing**, so it was discarded rather than left standing as support.
  A search of ~25 recent runs then found **no other** `Build Gate (macOS)`
  failure, so **the claim "this job is flaky" was NOT made** — there is no
  evidence for it. The narrow claim actually made is the one the evidence carries.

- **D-108 · a PR's check totals are not its contents, and this desk's parent
  inferred one from the other.** *"Get #5424 green"* rested on a false premise:
  the lane paused with **seven unpushed commits**, complying exactly with *"push
  nothing"*, so **green on the pushed tree would certify six markdown files.**
  The implementation is local and unaccepted. `%107` recorded it against itself —
  *"I read a PR's check totals and inferred what was in it, having spent the week
  telling other desks that a claim must not outrun the artifact it names."*

- **D-109 · PROJECT RULE — an assertion over a self-reported claim is not an
  assertion over the fact claimed.** Adopted from auditor-2's F-3, in its words:
  *every reported self-check state and every exit branch of a published exit-code
  contract needs a **shipped** assertion binding it to an observable consequence —
  an input the report must respond to — not to the report's own text.*
  **Demonstrated, not argued:** a gate whose entire self-check apparatus is erased
  — `self_check` never called, `fixture_check`/`population_check` forced to
  `PASS`, `cells=6` printed as a **literal** — is **accepted by the shipped
  negative control at exit 0**, while the identity control in the same harness
  behaves correctly, so the acceptance belongs to the contract and not the
  harness. **Nothing shipped can tell a measured `6` from a typed `6`.**
  This is *"a correction delivered to the claim does not fix the instrument that
  produces it"* **at the code level**.

- **D-110 · #5423's own thesis, turned on #5423 — and the campaign closes rather
  than the finding.** The ticket exists to outlaw checks that cannot fail; its
  shipped control could not fail on the erasure of the self-checks it reports.
  `gate.sh` is `.gitignore:101` and **untracked**, so its assertions never ship.
  **17 of 19 invariants PASS/KILLED** on strong evidence — a purpose-built tree
  carrying a depth-3 chain, a 3-cycle, a disjoint 4-cycle, a self-edge,
  `common`-stanza attribution, whole-name isolation via `cardano-apiary` /
  `xcardano-api` / `cardano-api2`. **The second findings verdict CLOSES the
  commit-owner campaign** (the single bounce was spent at 17:09:44Z); F-3/F-4 get
  **campaign 3** and a **fresh** auditor on the new SHA. **The auditor was not
  overridden to unblock the PR** — accepting past it would ship exactly the defect
  the ticket was commissioned to prevent.

- **D-111 · measure before warning.** Last release this desk warned `%310` that
  the merged PRs were *"the ones most likely to have moved your surface"*; it
  rebased, measured, and correctly answered **no** — the warning cost it a rebase.
  This release the measurement came first: base moved **7 commits**, **0 of them
  touched `TransactionsNew.hs`**, pragma still at line **17**. So the instruction
  was *rebase for hygiene, and do **not** re-derive what you already established;
  re-measure only what a rebase could plausibly move, and say which is which.*
  A correction is made when the next instruction is different, not when it is
  quoted.
