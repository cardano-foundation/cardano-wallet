...
```mermaid
flowchart LR
  RA["🟡 #5423 closure ratchet<br/>CAMPAIGN 3 · INV-19 open"]
...
| unit | state | note |
|---|---|---|
| **closure ratchet — #5423 / PR #5424** | 🟡 **CAMPAIGN 3** — `%441` T.O., commit owner `codex`, fresh auditor `claude` (`%443` is gone; bind by ID) | 17/19 invariants killed; **INV-19 open** — `gate.sh` is gitignored so its assertions never ship, and the shipped control accepts an erased self-check at exit 0. Campaign 2 closed by its second findings verdict. PR shows markdown only: **7 commits unpushed** |
...
| **#5420 audit owed** | **TRIGGER FIRED** 2026-09-04 — live now. Audit of **shipped** code, which changes what a finding means |
| **#5424 macOS Build Gate red** | **NOT this milestone's.** `No space left on device` on the CF macOS runner; identical commit re-run three days later failed identically, so it is persistent, not a flake. Routed to cardano-foundation infrastructure. Does **not** gate campaign 3 |
