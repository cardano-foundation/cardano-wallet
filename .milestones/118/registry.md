...
**Swept 2026-09-21.** Every entry names the contract, its parties, the
invariant, and the check that enforces it — or `enforced: NONE` explicitly. An
unstated NONE is a scheduled incident.

Some entries carry project-owner rulings. **A ruling is not enforcement.**
Where a ruling is the only thing holding an invariant, the entry says so and
names the check that must replace it. Sections are dated; a later section
supersedes an earlier one's `observed` line without deleting it.

---

## Swept 2026-09-21 — one new NONE, one measurement refresh, one closed-but-disputed

```
contract:   serialised transaction bytes <-> the era the wallet decodes them as
parties:    upstream cardano-ledger (owns every era's CBOR decoder)
            readTxFromBytes in SealedTx.hs (identifies the era)
invariant:  a newer era's decoder REJECTS an older era's encoding
observed:   VIOLATED, and #5410 says it has already stopped holding.
            readTxFromBytes identifies a serialised transaction's era by TRIAL
            DECODING NEWEST-ERA-FIRST and taking the first decoder that does
            not throw. Nothing declares this invariant and nothing tests it; it
            is entirely at the ledger's discretion, AND IT IS THE OPPOSITE OF
            THE DIRECTION UPSTREAM IS MOVING - cardano-ledger-alonzo 1.16.0.0:
            "Make AlonzoTx decoder that used for Mempool backwards compatible
            with prior-eras". A deliberately lenient decoder makes the newest
            era swallow every older encoding.
enforced:   NONE. This is the highest-value NONE in the milestone as of today,
            displacing node-runtime-pin-vs-cabal-constraints, because the
            failure is SILENT: a transaction is decoded as the wrong era and
            the wallet proceeds.
            SAME CLASS AS guardsTxBodyL - code that SUCCEEDS WRONGLY rather
            than failing loudly - AND THE CENSUS CANNOT COUNT IT.
action:     COMMISSIONED to lane t5410-era-detection (#5410), pane %1880,
            branch test/5410-era-detection-invariant at 92b57304dfb5. Per
            `invariants`, the check must be proven able to fail before it is
            trusted, and it must quantify over the discovered era extent rather
            than list eras by hand.
slug:       era-detection-by-trial-decode
```

```
contract:   released wallet <-> the cardano-node it is declared compatible with
parties:    #5416 (states the blocker), cardano-node release process,
            ouroboros-consensus #2251
invariant:  no release is cut against a node whose pinned consensus lacks #2251
observed:   DISPUTED. #5416 was CLOSED COMPLETED on 2026-09-16 and TWO
            INDEPENDENT MEASUREMENTS DISAGREE WITH THE CLOSE.
            (1) This desk, marker-file route, WITH A POSITIVE CONTROL: the
                marker is ABSENT at ouroboros-consensus 4.2.1.0's pinned rev
                82ecba329d; CHaP carries nothing newer than 4.2.1.0; wallet
                master pins ==4.2.1.0; the same query FINDS the marker on main.
                Limit: the instrument's subject is CHaP.
            (2) Project owner %107, constraint route, closing that gap:
                cardano-node 11.1.2 pins ouroboros-consensus ^>= 4.2.0.1 with
                index-state cardano-haskell-packages 2026-09-16T23:53:07Z, and
                A CARET AT ^>= 4.2.0.1 CANNOT REACH 4.3 while the newest
                consensus in CHaP is 4.2.1.0. Limit, named by it: verified the
                CONSTRAINT not the resolved plan; a source-repository-package
                grep finding none is inconclusive, not confirming.
enforced:   NONE. Nothing mechanical gates a release on consensus carrying
            #2251; the issue body's two-query procedure is an instrument a
            human must choose to run.
            THE FAILURE IT GUARDS IS THE SILENT ONE: the wallet connects,
            negotiates, APPLIES ZERO BLOCKS, AND LOGS NO ERROR. A release cut
            against that is indistinguishable from a healthy one.
action:     STANDS AS A RELEASE BLOCKER AT PROJECT LEVEL until someone shows
            #2251 in the pinned set. The burden has moved to whoever closed it.
            ROUTED TO THE OPERATOR with both measurements. NOT REOPENED -
            reversing another party's decision is outward. Do not reopen, do
            not comment, do not touch #5435 or #5444.
slug:       release-vs-node-consensus-2251
```

```
contract:   Dijkstra stub census <-> every PR from every milestone
            [MEASUREMENT REFRESH of the 2026-09-07 entry below]
parties:    M6 (owns the census and the outcome), every desk that edits lib/
invariant:  the census never rises; a PR that retires a stub lowers the ratchet
            in the same PR
observed:   32 at origin/master 92b57304dfb5 - 26 error + 6 pendingWith across
            10 FILES, measured 2026-09-21, gate GREEN at MAX=32 with
            "controls: positive=PASS negative=PASS".
            TREND: 44/15 at open -> 39/15 on 09-08 -> 32/10 today.
            FIVE of the 32 are in lib/wallet/src/Cardano/Api/Extra.hs, the shim
            M1's #5290 deletes. M6 DELIVERS 27 AND OWES ONE VERIFICATION -
            confirm they died with their owner, take back any that owner drops.
            A census falling BY AGREEMENT BETWEEN TWO DESKS is what epic
            criterion 3 forbids.
enforced:   PARTIAL, unchanged - one direction only. See the 2026-09-07 entry
            for the exact measured asymmetry and the deliberate waiver.
```
...
## Swept 2026-09-07 — two entries enforced, one correction, one new NONE
...
invariant:  the census never rises; a PR that retires a stub lowers the ratchet
            in the same PR
observed:   39 on master 6b42c36b58, 33 error + 6 pendingWith, 15 files
enforced:   PARTIAL — scripts/ci/dijkstra-stub-gate.sh, wired at
            .github/workflows/dijkstra-census.yml:47, MAX=39, merged with
            #5407 on 2026-09-04. The gate self-checks, printing
            "controls: positive=PASS negative=PASS" at every invocation.
            ONE DIRECTION ONLY, measured with $? read directly and not
            through a pipe:
              add a stub                    -> GATE RED, exit 1   (enforced)
              retire one, leave MAX at 39   -> RATCHET SLACK, exit 0 (advisory)
            So "the ratchet moves in the same PR" is checked BY THE MILESTONE
            OWNER AT ACCEPTANCE, not by CI. Never reported as enforced.
action:     whether slack should exit non-zero is #5406's owner's call, not
            M6's. There is a real argument for advisory: a hard failure turns
            every cross-desk retirement into a red build for a desk that did
            nothing wrong. Recorded as a deliberate waiver, not an oversight.
```

```
contract:   the epic's outcome claim ↔ the census that measures it
parties:    #5209 (claims Dijkstra readiness), the census (measures announced stubs)
invariant:  nobody reads a green census as a delivered outcome
observed:   the census population is code that ANNOUNCES it is unimplemented —
            error stubs naming Dijkstra, and pendingWith. The Dijkstra arm of
            setRequiredSigners (guardsTxBodyL) announces nothing and is
            unverified code that may be wrong when activated.
            A STUB FAILS LOUDLY; THAT SUCCEEDS WRONGLY.
enforced:   NONE, and deliberately so — widening the population to "code that
            might be wrong" would destroy the property that makes the criterion
            mechanical. A census that requires judgement is not a ratchet.
            Held instead by a written statement in #5209's body: "this issue's
            outcome claim is wider than its instrument — do not read zero as
            done", plus criteria 2 and 3 (every path exercised; no stub closed
            by suppression), which are not automatable in the same way.
action:     none. A green census is a NECESSARY condition for the outcome, not
            a sufficient one, and that is now on the record where the claim lives.
```

```
contract:   node runtime pin <-> Haskell dependency pin-set   [CORRECTED 2026-09-07]
parties:    flake.nix cardano-node-runtime input; cabal.project constraints
invariant:  the node the wallet is COMPILED against equals the node it is
            TESTED against; a bump moves both or neither
observed:   ADVANCED. The 08-25 entry below records ref=11.0.1 with
            cardano-api ==11.0.0.0; #5399 merged 2026-09-01 as 4a2227a725 and
            moved the pin set to node 11.1.0. The entry below is preserved as
            written but its "observed" line is superseded by this one.
enforced:   NONE — still a comment in cabal.project, not a check. Unchanged by
            the bump; the bump is exactly the operation the missing check
            would have guarded.
action:     still the highest-value NONE in this milestone.
```

---

## Entries as swept 2026-08-25
